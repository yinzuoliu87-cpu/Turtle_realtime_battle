...
## 双路布阵默认: 3统领+3小将分上/下(各3). top=统领0,1+前排小将; bottom=统领2+前排小将+后排小将.
func default_dual_lineup() -> Dictionary:
	var lead: Array = season_leaders.duplicate() if season_leaders is Array else []
	while lead.size() < 3:
		lead.append(str(lead[0]) if lead.size() > 0 else "basic")
	return {
		"top": [{"kind": "leader", "id": str(lead[0])}, {"kind": "leader", "id": str(lead[1])}, {"kind": "minion", "role": "front"}],
		"bottom": [{"kind": "leader", "id": str(lead[2])}, {"kind": "minion", "role": "front"}, {"kind": "minion", "role": "back"}],
	}

## 取双路布阵(缺/无效/换队后重置为默认). 校验统领集合==season_leaders.
func get_dual_lineup() -> Dictionary:
	if dual_lineup is Dictionary and dual_lineup.has("top") and dual_lineup.has("bottom"):
		var ids: Array = []
		for lane in ["top", "bottom"]:
			for u in dual_lineup[lane]:
				if u is Dictionary and str(u.get("kind", "")) == "leader":
					ids.append(str(u.get("id", "")))
		ids.sort()
		var sl: Array = (season_leaders.duplicate() if season_leaders is Array else [])
		var slc: Array = []
		for x in sl: slc.append(str(x))
		slc.sort()
		if ids == slc:
			return dual_lineup
