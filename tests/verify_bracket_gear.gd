...
func _ready() -> void:
	...
	## ★★★这里**曾经**是「窗口往上/往下真的开着」两条。它们立了大功: 入参改了含义
	##   而函数体还在读 `GameState` 时, 「往上 0 次 / 往下 360 次」就是唯一的线索。
	##   但对手池按场次重做之后(SEED_VER 12, 0~24 每格 12 支), 第 ① 级「等场次」
	##   **垄断了全部抽样** ⇒ 那两条永远满足不了 —— 而那是想要的结果不是缺陷。
	##   ⇒ 窗口对称性搬去 `verify_exact_match` ⑤b 用**合成池**直接量窗口函数
	##     (端到端走不到的那一级, 判据就不该放在端到端)。
	##   ⇒ 这里换成量「同场次命中率」: 它守着同一个 bug —— 入参被忽略而照读全局时
	##     每一抽都会抽到 0 场次的快照, 命中率掉到 0 当场红(硬边界守不住: 0 ≤ N+1 恒真)。
	var same := draws - above - below - over.size()
	var rate := float(same) / float(maxi(1, real_snaps))
	_ok("★★★同场次命中率 ≥ 0.90(池子按场次重做买到的; 入参被忽略时它会掉到 0)",
		rate >= 0.90,
		"等场次 %d / 往上 %d / 往下 %d ⇒ 命中率 %.3f" % [same, above, below, rate])
