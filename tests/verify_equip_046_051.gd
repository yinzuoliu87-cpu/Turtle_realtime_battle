...
func _ready() -> void:
	...
	## ★★这一节是**探针**。产品那一行是 `u["atk"] += u["maxHp"] / HP_MULT * hammer_pct`,
	##   **除了 HP_MULT**。而 HP_MULT 自己的注释写着「龟/装备 hp **已写最终值**」——
	##   若当真, 这一除就等于只发文案的 1/3。**必须量, 不能推理**(数值我不擅自改)。
	...
	for si in [0, 2]:
		...
		## ★★★【已确诊的分歧·钉住现状不擅自改】
		##   文案(玩家看得到): 「获得 4/6/15% 自身最大生命值的攻击力」
		##   实发(本条量出来): maxHp ÷ HP_MULT(3) × 4/6/15% = **1.33/2/5%** —— 只有文案的 1/3。
		##   拿龟血中位 945 算: ★3 文案承诺 +141.8 ATK, 实给 +47.2(龟基础 ATK 才 40)。
		##   ★为什么三个文案审计器全绿: 它们比的是「文案三元组 ↔ 代码三元组」(4/6/15 两边一致),
		##     **盲在消费那一步**(`u["maxHp"] / HP_MULT * hammer_pct`)。
		##   ★`equip_stats.gd` 自己的注释也写着「hammer_pct 是 ATK += maxHp×15%」—— 注释也以为是 15%。
		##   ⇒ 这是**数值**问题(3 倍), 按铁律不擅自改; 钉进断言 = 谁改了这个数这条会当场红并读到上面这段。
		##   ⇒ 已登记进方案书「已知风险与未决点」等用户拍板。
		_ok("② ★★★%d 星实发 %.1f = maxHp ÷ HP_MULT × %.0f%%(= %.2f%% 最大生命) —— **文案写的是 %.0f%%**"
			% [si + 1, from_hp, EXP_HAMMER_PCT[si] * 100.0,
			   100.0 * EXP_HAMMER_PCT[si] / _s.HP_MULT, EXP_HAMMER_PCT[si] * 100.0],
			absf(from_hp - want_div) < 1.0,
			"钉住现状: 文案口径应是 %.1f, 实发 %.1f(1/%.0f)" % [want_final, want_div, _s.HP_MULT])
