...
##   效果 每次普攻 +5 龟能; 每次普攻 +3/5/7% 攻速, ★不设上限, 换路重置
##   ★2026-09-06 用户「鲨肝油加强为每次攻击提供3/5/7%攻击速度」⇒ 原 1/2/4%。
##     产品侧已抽成常量 `EqPotionBatch.OIL_ASPD_PER_HIT`；**这里故意不引它** ——
##     门禁引产品常量就是拿被测对象证明自己(恒真式)，所以下面写死用户拍板的数。
func _t065_shark_oil() -> void:
	...
	for si in range(3):
		var per: float = [0.03, 0.05, 0.07][si]   # ★写死用户 2026-09-06 拍板值, 不引产品常量
		_s._units.clear()
		...
		_ok("065 si=%d 一次普攻 +%.0f%% 攻速(需求 3/5/7%%)" % [si, per * 100.0],
	...
	## ★2026-09-06: si=2 每次普攻 4% → 7% ⇒ 新携带者打 1 次应是 1.07
	_ok("065 ★换路重置: 新建的携带者从 1.00 起算(1 次普攻 = 1.07, 不接上一只的累计值)",
		absf(float(b2.get("aspd_perm", 1.0)) - 1.07) < 0.0005,
		"新单位 aspd_perm=%.4f 期望 1.0700; 老单位 %.4f" % [float(b2.get("aspd_perm", 1.0)), float(a2.get("aspd_perm", 1.0))])
	_s._units.clear()
...
## 068 深海气压罐(药水·5费):
##   受伤 ×60/100/200% 进充能条(上限 = 最大生命 30/50/100% ★2026-09-06, 原 20/40/100%)
...
func _t068_pressure_can() -> void:
	...
	#    | 星 | 充满需实际挨打 | 法力护盾 | 激光伤害 |
	#    ★2026-09-06 用户「加强深海气压罐, 储存的上线为最大生命值的30/50/100%」
	#      ⇒ 上限 20/40/100% → 30/50/100%。★3 本来就是 100% ⇒ **该档四个数全不变**,
	#      只有 1★/2★ 跟着涨。下表按 maxHp=3000 重算(存入 60/100/200% · 护盾 100/120/300%)。
	#    | 星 | 充满需实际挨打 | 上限 | 法力护盾 | 激光伤害 |
	#    | 1★ |     1500       |  900 |   900    |   900    |
	#    | 2★ |     1500       | 1500 |  1800    |  1800    |
	#    | 3★ |     1500       | 3000 |  9000    | 18000    |
	var hit_need: Array = [1500.0, 1500.0, 1500.0]
	var want_sh: Array = [900.0, 1800.0, 9000.0]
	var want_beam: Array = [900.0, 1800.0, 18000.0]
	var want_cap: Array = [900.0, 1500.0, 3000.0]
	for si in range(3):
		_s._units.clear()
		_s._spec.clear_all()
		...
		_ok("068 si=%d 挨打 %.0f → 充能条满 %.0f(转化 60/100/200%%, 上限 = 最大生命 30/50/100%%)"
