...
func _ready() -> void:
	...
	_s = RB.new()
	add_child(_s)
	## ★★开**确定性模式**: 它让每帧恰跑**一步** sim(SIM_DT), 于是本文件里
	##   所有按帧数的等待都变成了确切的**游戏秒数**, 与机器快慢无关。
	##   ★探针实测(2026-09-24, --max-fps 15): 不开的话 `_wait(120)` 要走掉8 游戏秒,
	##   20 秒的时停在两次等待之间就结束了(剩 0.00 秒/active=0),
	##   于是 ⑧⑨④⑪ 四条全红 —— 看着像"时停冻不住东西", 实际是时停已经过去了。
	##   (高帧率下同一句还剩 17.18 秒 ⇒ 本地永远看不到。)
