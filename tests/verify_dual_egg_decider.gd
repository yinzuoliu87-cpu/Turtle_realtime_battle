...
func _ready() -> void:
	...
	_ok("终极路 wiped=left → 定局", sc._dl_sys._dl_is_decider("left") == true)
	_ok("终极路 wiped=right → 定局", sc._dl_sys._dl_is_decider("right") == true)

	gs.current_lane = "top"; gs.lane_results = {}
	_ok("上路 wiped=left → 非定局", sc._dl_sys._dl_is_decider("left") == false)
	_ok("上路 wiped=right → 非定局", sc._dl_sys._dl_is_decider("right") == false)

	# 下路: 横扫 = 本路胜方(wiped的对手) 上路也赢
	gs.current_lane = "bottom"; gs.lane_results = {"top": "left"}
	_ok("下路 上路left赢·wiped=right(本路left赢) → 横扫定局(2-0)", sc._dl_sys._dl_is_decider("right") == true)
	_ok("下路 上路left赢·wiped=left(本路right赢) → 非定局(1-1)", sc._dl_sys._dl_is_decider("left") == false)

	gs.current_lane = "bottom"; gs.lane_results = {"top": "right"}
	_ok("下路 上路right赢·wiped=left(本路right赢) → 横扫定局(0-2)", sc._dl_sys._dl_is_decider("left") == true)
