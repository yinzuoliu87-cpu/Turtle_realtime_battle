...
func _test_form_chip_on_real_panel() -> void:
	...
	## ★★2026-08-17 判据改了口径, 原因是**用户推翻了旧要求**, 不是门禁坏了:
	##   旧要求(我当初写这条时): "玩家得知道攒满怒气会变身" ⇒ 判"面板可见文本里有 攒满+火山"。
	##   新指令(用户 2026-08-17):「**攒满变火山形态这种 ai 味的废话给我去掉啊**」
	##   ⇒ 怒气条上那句一个数都没给(条上已写 30/100), 是把同一件事说的**第三遍**, 已删。
	## ★但**要求本身没被推翻** —— "玩家得知道" 仍然要成立。查证: 它写在**被动描述**里,
	##   而且比那句全得多:「怒气积满 100 时, 熔岩龟变身为火山龟, 转为近战形态、全属性提升
	##   并获得火山技能组, 持续 15 秒。」被动就在面板第一格、点开即见。
	## ⇒ 判据从"面板可见文本"改成**"被动描述里讲清楚了"**: 保住要求, 不逼回那句废话。
	##   (真把被动里也删了, 这条照样红 —— 反向验证过。)
	var _pv: Dictionary = DataRegistry.pet_by_id.get("lava", {}).get("passive", {})
	var _ptxt := str(_pv.get("brief", "")) + str(_pv.get("desc", ""))
	_ok("★常态也说清楚(被动描述里讲了攒满怒气会变身 —— 不必在条上再说第三遍)",
		_ptxt.find("怒气") >= 0 and _ptxt.find("火山") >= 0 and _ptxt.find("100") >= 0,
		"被动文本长 %d, 怒气=%s 火山=%s 100=%s" % [_ptxt.length(),
			str(_ptxt.find("怒气") >= 0), str(_ptxt.find("火山") >= 0), str(_ptxt.find("100") >= 0)])
...
func _test_rage_and_star_denominator() -> void:
	...
	v["star_energy"] = 400.0
	_ip._refresh_info_panel()
	## ★★同 lava 那条: 用户 2026-08-17 点名「攒满放强化版」这类不带数的结论文字是废话, 已删。
	##   要求"玩家得知道星能满了会怎样"仍在, 而它写在**被动描述**里且更全:
	##   「当星能充满时,【星波】与【扭曲空间】会打出强化变体并消耗全部星能。」
	##   ⇒ 判据落到被动文本上, 不逼回条上那句。
	var _sv: Dictionary = DataRegistry.pet_by_id.get("space", {}).get("passive", {})
	var _stxt := str(_sv.get("brief", "")) + str(_sv.get("desc", ""))
	_ok("★星能攒满时明说会怎样(在被动描述里, 不必在条上再说一遍)",
		_stxt.find("星能") >= 0 and (_stxt.find("充满") >= 0 or _stxt.find("攒满") >= 0)
			and _stxt.find("强化") >= 0,
		"被动文本长 %d" % _stxt.length())
	_s._hud._close_info_panel()
