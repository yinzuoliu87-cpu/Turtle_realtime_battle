...
##
## ⚠ 实测记录: `BLACKMAP=1` 【没有】把地图变黑, 所以这张图里
##   ★2026-09-18 更正根因: 当时写的解释是"它管的是环境背景那一层" —— 只对了一半。
##   `BLACKMAP` 确实有一处管环境背景(`battle_world_builder.gd:460`, 活的), 但它**还有**
##   一处专门把地面刷黑的分支, 那处落在 `MAP_V2` 恒真 return 之后 ⇒ **永远跑不到**。
##   也就是说"地面没变黑"不是设计如此, 是**一段到不了的代码**。那 135 行已于 v0.19.405 删除。
##   ★这条留着当样本: 撞见反常先写下解释、解释听着通就停止追查 —— 死代码就是这么活两个月的。
