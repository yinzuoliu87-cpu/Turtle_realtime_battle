...
##   ② ★最亮像素必须是 255 —— 贴图必须**用满 0..255 这段量程**。
##      ★★2026-09-18 这条的【理由换了】, 判据没换, 记一下免得下一个人照旧注释推断:
##        旧理由是"最亮处 = 纯调色板色, 贴图只做减法" —— 那时 ground_land 写的是
##        `c = base_col.rgb * det`, det≤1 ⇒ 贴图确实只能减。
##        v0.19.403 把它改成了 `c = base_col.rgb * mix(0.80, 1.30, det)`(绕基色上下调制,
##        修"占地面 37% 的主地面被从明度 36 单向压到 19"), **det=1 现在是 1.30 倍基色**,
##        "只做减法"已经不成立了。
##      ★现在的理由: det 不到 1 就永远够不到调制区间的上端 ⇒ 整片地面被系统性压暗。
##        v0.19.404 换新地砖时正是这么踩的: stone 最亮只有 0.902 ⇒ 石台白白暗掉 9.8%,
##        而那一轮的目的恰恰是【提亮地面】。修法是把贴图线性拉伸到最亮=255(×1.11),
##        不是把几个像素钉死成 255 —— 那是糊弄判据, 结构一点没改善。
