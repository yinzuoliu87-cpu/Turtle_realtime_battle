...
func _ready() -> void:
	...
	# ── ④.5 暴击率溢出 100% 要转成暴伤显示(用户 2026-08-14 实测看到"124%") ────
	##   ★内部计算本来就对(`_resolve_dmg` 钳到 100% + 溢出转暴伤), 错的是面板原样显示。
	##   判据: 把 crit 设成 1.24, 面板必须读作「暴击 100%」而不是「暴击 124%」,
	##   且暴伤那一行要涨 24%×1.5 = 36 个百分点。
	u["crit"] = 1.24
	u["crit_dmg"] = 1.5
	var crit_row := ""
	var cd_row := ""
	for rc in ip._info_stat_rows(u):
		var tx := str((rc as Array)[1])
		if tx.begins_with("暴击 "): crit_row = tx
		if tx.begins_with("暴伤 "): cd_row = tx
	_ok("★★暴击率 124%% ⇒ 面板显示【暴击 100%%】(不是 124%%, 那作为概率没意义)",
		crit_row == "暴击 100%", "实得 %s" % crit_row)
	_ok("★★溢出的 24%% ×1.5 并进暴伤 ⇒ 150%% + 36%% = 186%%",
		cd_row == "暴伤 186%", "实得 %s" % cd_row)
	u["crit"] = 0.25
	u["crit_dmg"] = 1.5
