...
func _t_energy_real() -> void:
	...
	## ★★**一次只喂一步 sim, 采样也一步一次**(2026-09-24 改)。
	##   原来是 `await process_frame` 推进 + 每帧采一次 —— 本地一帧≈1 步 sim,
	##   所以每一步都被采到; 而 CI 一帧 ~0.066 秒 = **4 步**, 龟能能从 70 直接跨到 5,
	##   `prev_e >= 满-1` 与 `e_now < 1` 两头都不满足 ⇒ **整个跃迁看不见**,
	##   循环跑到 13 秒超时, 量出一个「攒到一半」的数(CI 实测 33.6, 本地用
	##   `_advance_sim_accum(0.20)` 当场复现出 36.2 / -1.00 / 护盾 -1.0, 三条一样)。
	##   ⇒ 尺子与被测量的东西要同一个时钟
	##   (memory `fb-headless-one-step-per-frame-hides-frame-keyed-bugs` 的镜像面)。
	## ★不靠 `await` 推进: `_advance_sim_accum` 本来就是「可测种子」
	##   (`verify_interactive_determinism` 直接驱动它), 与帧率无关。
	while w2 < 3600 and float(_s._t) - t1 < 13.0:
		_s._advance_sim_accum(_s.SIM_DT)     # 恰一步
		w2 += 1
		## ★★**一步 await 都不要**: 只要让出一帧, `_process` 就会自己再推几步,
		##   那几步是**采不到**的 —— 跃迁刚好落在里面就又看不见了(CI 上就是这么红的),
		##   只是概率从“必然”降成“偶尔”。整个循环在**同一帧**里跑完, 一步不漏。
