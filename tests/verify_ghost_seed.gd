extends Node
## verify_ghost_seed.gd — 守卫: 内置 ghost 种子池 (10 支策划队, 按档分桶, 冷启动/老档并入)
...
func _ready() -> void:
	...
	_ok("种子池 = 10 支队", total == 10, "实际 %d" % total)
	...
	Backend._ensure_seeded(pool)
	var c2 := _count(pool)
	_ok("_ensure_seeded 空池并入 10", c1 == 10, "%d" % c1)
	_ok("_ensure_seeded 幂等(重并不重复)", c2 == 10, "%d" % c2)
	...
	for sp in top_specs:
		if str((sp as Dictionary).get("kind", "")) == "leader":
			...
			if (sp as Dictionary).has("equips") and ((sp as Dictionary)["equips"] as Array).size() > 0:
				equipped_count += 1
	_ok("对手上路 = ghost lane_assign 的 leaders", leader_ids == ["diamond", "cyber"], str(leader_ids))
	_ok("★对手 leaders 都挂上了 equipped(按档装备)", equipped_count == 2, "带装备的 leader 数=%d" % equipped_count)
	# 高档件数 > 空档: 抽个低档对照
...
func _done() -> void:
	print("")
	if _fail == 0:
		print("ALL PASS — ghost 种子池(10策划队/按档) 守卫通过")
	else:
		print("FAIL x", _fail)
