...
const WANT_COUNT := {
	...
	## 2026-08-31: 096 小木斧单独成一个类型「斧头」(用户「为木斧专门设一个羁绊」),
	##   照香火的形状: 单档·1 件即激活 ⇒ 顶档 1 == 件数 1, D5 那条不变式仍然成立。
	"斧头": 1,
}
## 方案书 §4.5.1 新阈值
const WANT_TIERS := {
...
## p2eq-types.json 只映射【上架的 95 件】—— 羁绊赠送的圣光护盾(p2eq_095)【故意没有类型】:
## 给它"盾"就会【送盾 → 盾件数+1 → 档位涨 → 再送盾】无限循环。
const WANT_ITEMS := 95   # 2026-08-31: +1 = 096 小木斧(方案书三期)
