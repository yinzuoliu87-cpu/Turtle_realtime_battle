...
func _t079_pos() -> void:
	...
	## ★★2026-08-26: 这条原来写的是 `_ok(..., true, "裸落点就是礁石中心")` —— **什么都没验**。
	##   分母断言恰恰是别的断言的安全网, 它自己不可能红就等于整组没有安全网:
	##   若哪天障碍尺寸改小到裸落点已经在外面, 上面那条"推到边上"会因为"本来就在外面"而恒绿。
	##   ⇒ 拿同一个椭圆公式**真算一遍裸落点**, 断言它 < 1(在里面)。
	## ⚠ 我改这条时第一版写成 `Vector2(0,0).length() < 1.0` —— **那还是恒真式**, 只是拐了个弯。
	##   真判据必须**读两个各自独立算出来的量**: 拿【同一个产品函数、空障碍列表】算出裸落点,
	##   再拿它对礁石算椭圆参数。哪天礁石挪了位置或半径改小, 这条会红, 上面那条才有意义。
	## ★两个量必须【各自独立算出来】才判别得动:
	##     · `po_free` = 同一个产品函数、**空障碍列表** ⇒ 真正的无障碍落点
	##     · `ob_c`    = 障碍**自己存的中心**(不是测试里那个 `raw` 变量)
	##   我第一版拿 `po_free - raw` 算, 而 `raw` 按构造就等于 `po_free` ⇒ 恒为 0, 还是恒真式。
	##   现在挪一下礁石(改 `ob` 里的 c)这条就会红 —— 那正是"这组用例已经失去意义"的信号。
	var po_free: Vector2 = EqGunBatch.tower_pos(c, "left", arena, [], 28.0)
	var ob_c: Vector2 = (ob[0] as Dictionary)["c"]
	var d_raw: Vector2 = po_free - ob_c
	var ell_raw: float = Vector2(d_raw.x / (60.0 + 28.0), d_raw.y / (36.0 + 28.0)).length()
	_ok("③ ★分母: 不推的话裸落点【真的】在障碍里(椭圆参数 %.3f < 1)" % ell_raw,
		ell_raw < 1.0,
		"无障碍落点 (%.0f,%.0f) vs 礁石心 (%.0f,%.0f) ⇒ t=%.3f"
			% [po_free.x, po_free.y, ob_c.x, ob_c.y, ell_raw])
...
func _t_vfx_nodes() -> void:
	...
	# ★★2026-08-07: 演出现在是**两段**的 —— 弹飞到终点时会再生成一次**命中火花**
	#   (用户:「子弹打到哪里」, 原来打中什么都没有)。一次 `tick(2.0)` 会把弹一步推到寿终、
	#   同一帧生出火花, 而那一帧已经没有剩余时间去推火花 ⇒ 断言"全退场"就红。
	#   ⇒ 喂**两段**: 第一段推完弹(并生出火花), 第二段推完火花。
	#   ⚠ 这不是"放宽断言" —— 断言本身没动(仍然要求全部退场), 只是承认演出多了一级链。
	vf.tick(2.0)                    # ① 弹/带/环等第一级效果寿终, 并在终点生出命中火花
	vf.tick(2.0)                    # ② 命中火花(0.16~0.22 秒)也退场
