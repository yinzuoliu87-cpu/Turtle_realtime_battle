...
func _t_fits_landscape() -> void:
	...
	print("  ④ 32 人桶镜像后高度 = %.0f px (视口 %.0f)" % [h32, vp.y])
	_ok("④ ★★32 人桶镜像之后【放得下】720 高(单向排要 ≈1136 ⇒ 必须拖)",
		h32 <= vp.y, "%.0f" % h32)
	_ok("④ ★分母: 而且它确实不小(>400) —— 否则「放得下」可能只是因为算成了 0",
		h32 > 400.0, "%.0f" % h32)
...
func _t_ratios() -> void:
	...
	_ok("⑤ 节点条高 = 屏高 5.4%（≈39px）",
		absf(L.row_h() - 720.0 * 0.054) < 0.5, "%.1f" % L.row_h())
