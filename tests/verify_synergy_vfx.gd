...
# ── ⑥ 形态E: 跨阈值只放一次 ─────────────────────────────────────────────────
## 纯函数, 不烘任何数字 —— 四条滚雪球的具体档位是方案书 U4, 用户还没拍板。
func _g6_tier() -> void:
	print("")
	print("  ⑥ 形态E — 跨阈值只放一次(阈值由调用方传, 本层不烘数字):")
	var th := [100.0, 300.0, 600.0]
	for cs in [[0.0, 0], [99.0, 0], [100.0, 1], [299.0, 1], [300.0, 2], [599.0, 2], [600.0, 3], [9999.0, 3]]:
		var got: int = _syn.tier_of(float(cs[0]), th)
		_ok("⑥ tier_of(%.0f) = %d" % [cs[0], int(cs[1])], got == int(cs[1]),
			"实得 %d" % got)
