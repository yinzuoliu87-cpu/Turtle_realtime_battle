...
func _g16_perf() -> void:
	...
	##
	## ★为什么不改判据（试过两条，都不对）：
	##   · 分 N 批取最小批 —— 毒雾会随 tick 消散，后面的批**负载天然更轻**，
	##     min 会选中"最轻负载"而不是"噪声最小"，等于作弊。
	##   · 归一化到参照负载 —— 代码真变慢 2 倍而机器同时降频 2 倍时会被约掉，判据变松。
	##   ⇒ 阈值语义保持不变，只补证据。
	...
	_ok("⑯ ★★满载每帧开销 < 1.0 ms(触手 4 根是 0.33 ms, 这是同一量级的预算)",
		us / 1000.0 < 1.0,
		"实测 %.4f ms; **机器参照负载 ×%.2f**(冷机=1.00) ⇒ %s"
			% [us / 1000.0, slow,
				"参照也慢了同一量级 ⇒ 多半是 CPU 降频不是回归。让机器凉几分钟再【单跑这一条】确认，别急着改阈值"
				if slow >= 1.5 else
				"参照正常 ⇒ 这是**真的变慢了**，去查代码"])
	for e2 in extra:
		_s._arr_erase_unit(_s._units, e2)
