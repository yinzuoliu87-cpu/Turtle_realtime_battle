...
func _g16_perf() -> void:
	...
	## ★★2026-09-05 加的诊断（判据没动，只是让失败信息说清"是不是机器的锅"）
	##
	## 这条断言是**绝对毫秒数**，对 CPU 降频极敏感。实测**同一份代码**：
	##   冷机 0.33 / 0.33 / 0.33 ms   连跑一晚门禁后的热机 1.04 / 1.18 / 1.19 ms  ⇒ **3.6 倍**
	## 而把三个改动文件**回退到 HEAD** 后热机一样红（1.12 / 1.19 / 1.25 ms）——
	## 我因此差点把它判成回归，花了 40 分钟做背对背 A/B 才排除。
	## （本机 CPU 有确诊硬件故障，见 CLAUDE.md §2：WHEA 20 条 · 两次无转储关机。）
	##
	## ⇒ 顺手测一段**参照负载**：纯 GDScript 浮点循环，与被测同为 CPU-bound ⇒ **一起被降频**。
	##   失败时把倍数一并打印：参照也慢了同一量级 = 机器的锅；参照正常而这里慢了 = 真回归。
	##
	## ★★2026-09-23 判据改了一次（原来是**绝对毫秒**）。
	##   起因: **提交门禁在 CI 上红了至少两个提交**，红的只有这一条 ——
	##   实测 1.52 ms 而阈值 1.0 ms，同时它自己量出**机器参照 ×7.09**：
	##   CI runner 比我的冷机慢七倍，绝对毫秒数**怎么都过不去**。
	##   后果跟 CLAUDE.md §3.6 那次一样：门禁从那时起等于没在 CI 上跑过。
	##
	## ★★顺带纠正我自己原来写在这里的一句错话。原文说「归一化到参照负载 ——
	##   代码真变慢 2 倍而机器同时降频 2 倍时会被约掉，判据变松」。**算一遍就知道不成立**：
	##     代码 2 倍 + 降频 2 倍 ⇒ 实测 = 0.33×2×2 = 1.32 ms，参照 = 8800×2 = 17600 us
	##     比值 = 1320/17600 = 0.075，冷机比值 = 330/8800 = 0.0375 ⇒ **正好 2 倍，抓得到**。
	##   归一化只约掉**机器**那一项，那正是要约掉的。我当时推错了，还照着错结论
	##   把判据留成了一条 CI 过不去的绝对值。
	##
	## ★那条**确实**不对的仍然不做：分 N 批取最小批 —— 毒雾会随 tick 消散，
	##   后面的批负载天然更轻，min 会选中"最轻负载"而不是"噪声最小"，等于作弊。
	const REF_COLD_US := 8800.0     # 冷机标定: 400k 次 sqrt，三次 8711/8863/8911（±1%）
	var r0: int = Time.get_ticks_usec()
	var acc := 0.0
	for i in range(400000):
		acc += sqrt(float(i) + 1.0)
	var ref_us: float = float(Time.get_ticks_usec() - r0)
	var slow: float = ref_us / REF_COLD_US
	print("     %d 帧平均 %.4f ms/帧 (触手当年 4 根实测 0.33 ms/帧); 机器参照 %.0f us / 冷机 %.0f us = ×%.2f (校验和 %.0f)"
		% [frames, us / 1000.0, ref_us, REF_COLD_US, slow, acc])
	## 冷机上 1.0 ms / 8800 us = 0.1136。
	## ★说准一点: 这**不是**与原来那条绝对判据逐位等价 —— 冷机上语义相同,
	##   机器慢了的时候预算**按实测的机器倍率跟着放宽**(这正是 CI 上要的)。
	##   反向验证(2026-09-23, 往产品 tick 里真塞额外开销):
	##     ≈2 倍(1.0511 ms/帧) → 绿 · ≈4 倍(3.1201 ms/帧) → 红。
	##   余量实测 2.85 倍(基线 0.346 ms/帧), 与旧判据同一量级。
	const RATIO_BUDGET := 1000.0 / REF_COLD_US        # 每帧微秒数 ÷ 参照微秒数
	const HARD_CEIL_MS := 8.0                         # 参照本身失真时的硬顶
	var ratio: float = us / maxf(1.0, ref_us)
	_ok("⑯ ★★满载每帧开销 / 机器参照 < %.4f(= 冷机上的 1.0 ms; 触手 4 根是 0.33 ms)"
			% RATIO_BUDGET,
		ratio < RATIO_BUDGET,
		"实测 %.4f ms ⇒ 比值 %.4f; 机器参照 ×%.2f(冷机=1.00) ⇒ %s"
			% [us / 1000.0, ratio, slow,
				"机器慢了同一量级但比值仍超标 ⇒ **真的变慢了**"
				if slow >= 1.5 else
				"机器正常 ⇒ 这是**真的变慢了**，去查代码"])
	_ok("⑯ ★硬顶: 每帧 < %.0f ms(万一参照负载本身失真, 还有这一道)" % HARD_CEIL_MS,
		us / 1000.0 < HARD_CEIL_MS, "实测 %.4f ms" % (us / 1000.0))
	for e2 in extra:
		_s._arr_erase_unit(_s._units, e2)
	_v.clear_all()
