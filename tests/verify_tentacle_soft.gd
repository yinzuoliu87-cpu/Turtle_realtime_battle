...
func _ready() -> void:
	...
	_ok("★分母: 抬起期真的采到帧(否则下面全是空的)", lmove.size() >= 40,
		"只采到 %d 帧" % lmove.size())
	## ══ 【已删】抬起"全程不许定格" ══════════════════════════════
	## 这条是给**运动学模型**定的(姿态公式直接决定位移, 基线 0.055 / 修好后 0.925)。
	## 2026-08-22 换成链式模拟后**它反转了** —— 实测:
	##   模拟 + 旧的【定格】曲线 → 比值 **0.323**
	##   模拟 + 现行【匀速】曲线 → 比值 **0.154**
	## 定格的那版反而"更流动"。根因: 模拟里链条**永远有惯性余动**, 姿态冻不冻住它都在动,
	## "整条每帧位移"这把尺子已经测不到姿态层的定格了。
	## ⇒ 不留一条量不到需求的判据(那正是我今晚反复栽的空判据)。
	##   "抬起要一直在动"现在由下面【姿势几乎不动时触手仍在动】那条守 —— 它反向验证会红。
	var lpeak := 0.0
	for mv in lmove:
		lpeak = maxf(lpeak, float(mv))
	print("     【抬起】每帧位移峰值 %.4f (%d 帧) —— 仅作记录, 不再当判据" % [lpeak, lmove.size()])
	## ══ 再量【下砸】 ══════════════════════════════════════════
