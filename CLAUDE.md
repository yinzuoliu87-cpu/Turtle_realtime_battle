...
`大版本.功能版本.改动序号`，当前 `0.11.3`。第 3 位每次改动 +1；第 2 位加新玩法时 +1。
...
**正解**：演出多由 tween 驱动，tween 走真实时间 → 用 `Time.get_ticks_msec()`：

```gdscript
var ms0 := Time.get_ticks_msec()
while Time.get_ticks_msec() - ms0 < 5000 and 效果还没落地:
    await get_tree().process_frame
```

且**必须在 `frames_for()` 里登记足够帧预算**（CI 帧率高，5 秒墙钟可能要几千帧）。
