...
**★门禁是并行跑的（2026-08-11），别再以为它天生要七八分钟。** 实测时间去向：
一个只读四个文件的空测试 2.14 秒、一个建整套战斗场的重测试 2.03 秒 ——
**耗时几乎 100% 是 Godot 进程启动，测试逻辑本身不要钱**，串行等于把这 2 秒重复 168 次。
量过但**不是**原因的（免得再查一遍）：CPU 降频（连跑 40 次只慢 4%）、
音频驱动（带不带 `--audio-driver Dummy` 一模一样）、审计器（六个共 1.5 秒）。
⇒ 测试池有界并行 + 冒烟提前与测试池并发。
⚠ **「2:34」是 2026-08 那会儿 123 项时量的，早就不成立了**：现在自动发现 **300 个测试 / 判定 346 项**。
2026-09-17 实测（日志起止时间戳，不是估的）：**JOBS=2 跑 7 轮 1007~1026 秒（16.8~17.1 分，±1%）；
JOBS=4 跑 2 轮均为 737 秒（12.3 分）** ⇒ 4 比 2 省 273 秒（−27%）。
**报耗时前先量**——这一行自己就漂了一个月没人发现。
默认 **`JOBS=4`**（2026-09-17 用户拍板调回来；2026-08-12 曾因这台机器 CPU 有确诊硬件故障从 4 降到 2）。
⚠ **调回时没有确认机器已送修/换板** —— 蓝屏一旦重现，第一件事就是改回 2，并且**跑门禁前先提交**
（最坏情况只是重跑一轮，而不是丢掉未提交的工作）。当初降下来的实测证据（材料在
`桌面\电脑故障-保修材料\`）：WHEA `Internal parity error` 累计 20 条，其中 08-11 22:04:12
**一秒内 16 条**且首次出现**第二个 APIC ID**（此前 4 条全是 APIC 16）；08-12 一天两次异常关机
且**连转储都没写出来**（此前 9 次蓝屏每次都有 `.dmp`）。**多进程编译正是最容易触发的负载。**
