...
```bash
bash run-tests.sh          # 自证测试(自动发现) + 全流程冒烟 + 三方对账 —— 约 2.5 分钟
...
⇒ 测试池有界并行 + 冒烟提前与测试池并发：**7:30 → 2:34**。
默认 **`JOBS=2`**（2026-08-12 从 4 降下来）是**因为这台机器 CPU 有确诊硬件故障且仍在恶化**，
不是因为这个数最优；**送修/换板之后调回 4 或更高**。恶化的实测证据（材料在
