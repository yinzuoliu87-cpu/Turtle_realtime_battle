...
if not revtxt:
    ...
    ## ══════════════════════════════════════════════════════════════
    ##  ★★★2026-09-06 补:「多出来的旧内容」以前一律放过
    ## ══════════════════════════════════════════════════════════════
    ## 上面那条判据是 `v not in L` —— **子串包含**, 只挡得住"表里少了/写错了",
    ## 挡不住"表里在 json 那段后面**还挂着一堆旧垃圾**"。
    ## 用户 2026-09-06:「这些旧东西不弄标志和日期吗, 这怎么乱搞呢这些文件？」
    ## 实测当时 **96 行里有 37 行**被裸换行截成两个物理行, 第二行装着**写死数字的旧版文案**
    ## (例 004 挂着「每6秒…回复相当于该伤害100%的生命」, 而 json 里是带占位符的新版),
    ## 而本审计器**报 ALL OK** —— 因为 json 那段确实"包含在"第一个物理行里。
    ##
    ## ⇒ 补一条形状判据: **每条装备行必须是完整的一行**(以 `|` 结尾)。
    ##   markdown 表格单元格不能跨物理行, 一旦跨了就说明有人手写塞了东西进去,
    ##   而生成器逐物理行走、根本看不见那半行 ⇒ 残留会被一遍遍抄下去。
    ##   (memory `fb-self-claiming-authority-docs-rot`: 这张表 2026-07-29 就"把逐行对账焊进门禁"了,
    ##    但判据只卡了一半 —— 门禁在, 漏照旧。同族: 判据要刚好卡住那个形状。)
    torn = [rid for rid, L in rows.items() if not L.rstrip().endswith('|')]
    ...
    chk('★每条装备行都是完整一行(跨物理行 = 有手写残留, 生成器清不掉)',
        ['%s 行尾不是 |' % r for r in sorted(torn)[:8]])
