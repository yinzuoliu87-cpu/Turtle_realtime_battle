...
def audit(path, pad_lr=185, pad_top=60, pad_bot=10, step=2):
    ...
    vals, cnt = [], Counter()
    bright_sat = []          # 亮部(Y>130)像素的 HSV 饱和度 —— 判据③
    n_ramp_lo = 0            # Y∈[100,140) 的像素数 —— 判据④
    n_ramp_hi = 0            # Y∈[150,195) 的像素数
    n_neutral = 0            # 近中性(S<0.25) —— 只报不判(见文件头)
    for y in range(0, crop.height, step):
        for x in range(0, crop.width, step):
            ...
            mx = max(c[0], c[1], c[2]); mn = min(c[0], c[1], c[2])
    ...
    out = {
        ...
        # ★2026-09-18 新增的两条(标定见文件头第二张表)
        "bright_pct": round(100.0 * len(bright_sat) / n, 1),
        "bright_sat_p50": (round(sorted(bright_sat)[len(bright_sat) // 2], 3)
                           if len(bright_sat) >= 200 else None),
...
##
## ═══════════════════════════════════════════════════════════════════
## ★★第二轮标定(2026-09-18·用户「都要仔细重做」后): 34 张 / 9 款真实游戏内画面
##   (46 张候选剔掉 12 张 key art/菜单/被模态框压住的; 逐张目视判过, 剔除清单在
##    scratchpad/lightcal/sheet_*.png)。
##
## ★为什么要第二轮: **上面那两条是必要不充分的** —— v0.19.405 实拍 2/2 全过,
##   而用户仍然说「战斗内的背景很烂」。只好回参考里再量, 找能区分的维度。
##
##   判据③ 亮部饱和度中位  参考中位 0.394 (四分位 0.294~0.465) | 本项目 0.696
##   判据④ 亮坡比          参考中位 2.45  (四分位 1.86~4.10)   | 本项目 0.57
##   (只报不判) 近中性占比   参考中位 14.2% (四分位 7.4~25.8)    | 本项目 2.1%
##
## ★判据③「亮部饱和度」抓的是: **最亮的东西到底是「光」还是「一块纯色」**。
##   真实光源到高亮度会往白里跑(饱和度掉); 一块自身很艳的表面被调亮则不会。
##   可区分性: 参考组这条量从 0.00 跨到 0.75, 阈值 0.60 只有 **1/34** 踩中 ——
##   那一张是 Hades_0「整屏被一团巨大橙色火焰占据」, 即"最亮的真的是一团有色的火"的合法例外。
##   排除它后参考组最高 0.57。本项目 4/4 全部踩中。
##
## ★判据④「亮坡比」抓的是: **亮区周围有没有明度过渡**。
##   正常场景越亮的像素越少 ⇒ 这个比值天然 >1; <1 说明亮的那块是【硬贴上去的一片亮板】。
##   可区分性: 参考 0.34~199.6, 阈值 1.0 只有 **1/34** 踩中(BrawlStars_3 白烟爆炸帧),
##   而那一张判据③ 只有 0.53 —— **两条合用: 参考 0/34 同时踩中, 本项目 4/4 同时踩中。**
##
## ★★实测指认: 本项目全屏亮部里 **86% 是色相 160~200 的青色** —— 就是那条水带。
##   它同时是"全屏最大的连通亮区"(占窗口 17.2%, 参考中位 4.2%)。
##   ⇒「背景很烂」的根因不是太暗/太平, 是**一条高饱和青色亮带霸占了全部亮部且边缘无过渡**。
##   ⚠ 这条**推翻了一个刻意的旧设计**: `ground_water.gdshader:9-10` 写着
##     「深色不能压太狠…『亮青潟湖框住暗岛』的构图会被反转」。实测说那个构图本身就是病,
##     参考组 34 张里**没有任何一款**是"最亮的东西是一块饱和色"。
##
## ★这一轮又证伪了六条我可能会去追的假说(记下来免得重查):
##   ✗「明度直方图中间被掏空」—— 空洞宽度本项目 0, **34/34 参考都比我们大**。
##      (⚠ 文件上方 `gap_p50_p75` 的注释说"这个空档就是本项目的病", 那是改前的事实,
##        **v0.19.405 之后已不成立**。)
##   ✗「一大坨死平的大色块」—— 最大平坦连通区本项目 7.1%, 14/34 参考更大。
##   ✗「没有大尺度灯光布局」—— 12×12 宏格 sd 本项目 40.0, 只有 6/34 更高。
##   ✗「亮处不够暖」—— Lab b* 亮暗差本项目 10.35, 参考中位 10.65, 正好压在中位。
##   ✗「局部对比低 = 地面死平」—— 本项目 10.62 落在参考四分位内; Brotato sd 中位 0.00 而它是好游戏。
##   ✗「最大亮块细长 = 带子不是光池」—— 周长²/4πA 本项目 65.5 vs 参考中位 58.1, 不可区分。
## ═══════════════════════════════════════════════════════════════════
def verdict(a):
    ...
    ## ★③④ 拿不到分母时【不判】而不是判过 —— 分母不足要说出来, 不能静默放行。
    if a.get("bright_sat_p50") is None:
        rules.append(("③ ★分母不足: 亮部(Y>130)不到 200 个采样点, 本条不判", False, "亮部 %.1f%%" % a["bright_pct"]))
    else:
        rules.append(("③ 亮部饱和度中位 ≤ 0.60(最亮的该是【光】不是一块纯色)",
                      a["bright_sat_p50"] <= 0.60, "%.3f" % a["bright_sat_p50"]))
    if a.get("ramp_ratio") is None:
        rules.append(("④ ★分母不足: Y∈[150,195) 不到 50 个采样点, 本条不判", False, ""))
    else:
