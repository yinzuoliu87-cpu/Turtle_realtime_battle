...
## 这些是**真的复用了别人的机制**, 不是接错:
EXEMPT = {
    'p2eq_096': {'AxeEvolution': '小木斧的全部数值(装备属性/召唤物公式/进化阈值/售价/配色)'
                                 '单独抽成一张纯数据表 AxeEvolution —— 它要被装备层、商店、'
                                 '图鉴、调色板工具四处共用, 塞回 AxeSystem 就成了跨层依赖',
                 'AxeFinalStats': '四个最终造物的数值(亡灵/炽天使/全息/余烬)同理单独成表 —— '
                                  'AxeEvolution 管的是"五档进化", 造物是它之后的另一层, '
                                  '两张表分开才不会让进化逻辑背上四个造物的常量'},
    'p2eq_040': {'GremlinGun': 'FPGA板【登场】塞出去的是【古灵精怪枪】这件别的东西 —— '
                                '枪的三项加成与自伤比例本来就该住在 gremlin_gun.gd, 不该在 FPGA 里再存一份'},
