...
★由来(2026-08-20): CLAUDE.md 白纸黑字写着"装备属性的真事实源是 EquipStats.STATS,
  ...
  ★★2026-09-06 订正: 原来这里写着「玩家在商店和图鉴看到的属性行就是这个 baseStats1」——
    **已经不是了**。`detail_views.gd:679` 明写「不再打印 data 里手写的 baseStats1」，
    而 `tests/verify_equip_stats_source.gd` 的 A 条是**源码级防复发**：
    「图鉴/背包【不准】再回退到直接打印 baseStats1」。
    玩家看到的属性行由 `EquipStats.lines_of()` 从 STATS **现算**。
    ⇒ `baseStats1` 如今**只有工具在读**（本审计器 / 评审表生成器 / review_table_sync），
      是个纯内部对账镜像，**改它的措辞玩家一个字都看不到**。
    本审计器仍然要跑 —— 镜像漂了说明有人改了 STATS 没同步，那是真信号；
    但别再拿"玩家看得到"当它的理由，那句话会让下一个人跑去改错地方（我 09-06 就差点）。
