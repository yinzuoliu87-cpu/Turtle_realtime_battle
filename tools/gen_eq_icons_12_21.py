...
NAME_OF = {
    12: "龟苓膏块", 13: "炙烤海胆", 14: "深海堡垒甲", 15: "荆棘海胆", 16: "铁壁盾",
...
# ── 012 龟苓膏块: 圆角方块果冻 + 顶面高光 ──────────────────────────────
def ic12(u, v):
    ## 圆角方块(超椭圆): |u|^4 + |v|^4 <= r^4
    def sq(r):
        return abs(u) ** 4 + abs(v) ** 4 <= r ** 4
    if not sq(0.80):
        return None
    if not sq(0.70):
        return OL                      # 描边
    ## 顶面: 上 1/3 亮一档(切出一条水平分界, 读成"一块方的"而不是球)
    if v < -0.28:
        ## 左上角一块方高光 —— 果冻的反光
        if -0.52 < u < -0.16 and -0.60 < v < -0.36:
            return HI
        return LT
    if v > 0.34:
        return DP                      # 底面阴影
