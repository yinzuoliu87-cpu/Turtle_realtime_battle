...
VERIFIED_OK = {
    # 2026-08-20 补「倍/发/枚」三个单位后浮出 3 处, 逐条看过上下文: 1 真 2 假。
    # 真的那条(赌神 3.3倍 → 6倍)已按代码 MULTI_ASPD=0.1667(≈6×) 改掉了。
    # 下面两条是**同一件事的两种记法**, 不是分歧:
    ("pirate", "skillPool", 1, "6发"):
        "brief 写「6 发炮弹」, 详细写「分**六**发扣血」—— 汉字数字, 同一个数",
    ("shell", "skillPool", 0, "1倍"):
        "brief 写「1 倍攻击力」, 详细写「100%×攻击力」—— 1倍 == 100%, 同一个量",
}

TAG = re.compile(r'<[^>]*>')
PLACEHOLDER = re.compile(r'\{[^{}]*\}')
# 带单位的数值。单位表按项目文案实际用到的收: 百分比/码/秒/层/段/点/级/次
# ★2026-08-20 补「倍」「发」「枚」: 赌神的 brief 写「约 3.3 倍攻击速度」而详细写「约 6 倍」,
#   代码 MULTI_ASPD=0.1667 ⇒ 约 6 倍(2026-07-28 从 0.30 改过来的, brief 停在改动前)。
#   这条一直没被抓到, **只因为单位表里没有「倍」** —— 单位表是白名单, 白名单天生会漏。
NUMU = re.compile(r'(\d+(?:\.\d+)?)\s*(%|码|秒|层|段|点|级|次|倍|发|枚)')
