# -*- coding: utf-8 -*-
"""blender_truefire.py — 022 余烬燃油瓶【真火】的持续燃烧(8 帧循环)。

跑法(无窗口):
  blender --background --python tools/blender_truefire.py -- --out C:/tmp/tf --frames 8 --px 320
  python tools/pixelize_sheet.py C:/tmp/tf --dirs 8 --frames 1 --cell 80 --art-h 80
      --palette truefire -o assets/sprites/vfx/true-fire.png
  "<godot>" --headless --path . --import        # ★换了 png 必须 import

════════════════════════════════════════════════════════════════════════
 ★由来 (2026-09-12)
════════════════════════════════════════════════════════════════════════
用户:「**022只需要做一个真火的特效**」「**真火是个buff你明白吗**」
⇒ 它是**挂在目标身上的状态**(EMBER_TRUEFIRE_SEC = 5 秒), 不是命中那一下的爆点。
   期间该目标受到的灼烧**改判真实伤害**(battle_damage: 蓝字 → 白字)。
   在此之前**画面上完全没有表现** —— 只有飘字颜色变了。

★参考(他:「**去网上找燃烧例子**」):
  5db7V-YGslo「Burning knight - Real time VFX (UE5)」, **上传 2026-02-20**, 32 秒。
  一个骑士整个人在烧、镜头近、暗背景 —— 和我们的黑场台子同类, 正好能量。
  ★★先看上传日期: 上一轮我拿了 2013 年的卡尔玛视频, 被用户当场抓。
...
第 2 版: 一把细树枝(线性收尖)。
第 3 版(**它进了游戏, 实拍才看出来**): **一丛橙色的草** —— 并排直杆、白尖在顶、暗褐在根。

实拍一崩, 回去把前三版**漏量的三条**量了(同一把尺子, 扣背景):

| 量 | 参考实测 | 第 3 版做出来的 |
|---|---|---|
| 填充率(火占包围盒) | **0.44** | 0.37, 而且是并排细杆不是团块 |
| 亮度 内/外(离边 3px vs 0px) | **1.26 —— 芯亮边暗** | **反的**: 白在尖、暗红在根 |
| 舌片厚度 / 火高 | **0.11~0.15**(最大内深 11px / bbox 高 143~214) | 细得多 |
| 火星(脱离本体的碎块) | 连通块 38 个, **82% 是 ≤12px 的碎星** | **0 个** |

★★**「芯为什么亮」不是画上去的渐变, 是视线穿过的火焰体积更厚。**
  所以这一版改用 **Cycles 体积发光**(Principled Volume): 亮度 = ∫ 发光 × 路径长,
  外缘薄 ⇒ 暗橙, 舌片中轴厚 ⇒ 亮橙, 两根舌片交叠处最厚 ⇒ 近白。
  内/外梯度是**渲出来的**不是我调出来的 —— 判据(内/外 ≈ 1.26)因此不是恒真式。
  同理 alpha = 1-exp(-密度×路径) ⇒ 薄的舌尖天然被 pixelize 的 alpha<24 切掉,
  剪影自带毛边, 不用手画锯齿。

════════════════════════════════════════════════════════════════════════
 ★实测剖面(帧 11.5~16.0s 每 0.1s; 未燃烧帧量本体, 峰值帧量火焰)
════════════════════════════════════════════════════════════════════════
  骑士本体            : 41 × **108 px**
  火焰峰值(f24~f36)   : w 170~225 · h 163~186
  ⇒ **火高 / 角色高 = 1.5~1.7**   **火宽 / 角色高 = 1.6~2.1**(火把人整个吞掉)
  面积剖面(上→下)     : 25 / 1578 / 3348 / 4375 / 4671 / 2775 ⇒ **下宽上尖**
  最亮处颜色          : RGB (231,164,138) 橙白

这条面积剖面**直接驱动**火舌的高度分布 —— 不是我手调的系数
(memory [[fb-match-reference-by-measured-curve]]: 手调缓动补不出真实曲线)。

★尺寸: 龟高 2.0 m ⇒ 火 1.6 × 2.0 = 3.2 m 高。1 屏幕像素 = 0.0426 m(染色实测)
  ⇒ 一格 80 px ⇒ 世界 3.41 m, pixel_size 0.0426 = **1 texel : 1 屏幕像素**。
  实拍复核(第 3 版素材): 屏幕上火 **91 宽 × 80 高**、龟 45 高 ⇒ 1.78 龟高 ——
  **尺寸这条是对的**, 崩掉的只有形状与明暗, 所以这一版**不动尺寸**。

★8 帧循环: 火是**持续**状态, 必须无缝循环(第 8 帧接回第 1 帧)。
  所有起伏都是 sin(2π·frame/8 + 相位) ⇒ 天然首尾相接。
"""
import argparse
import math
import os
...
Y0 = -0.98             # 火焰底(脚下)
H_ALL = 1.90           # 火焰总高(到 +0.92, 不许顶满画布)
W_MAX = 0.98           # 最宽那一带的半宽(火宽/火高 ≈ 1.03; 台子泛光再撑到实测的 1.15)
...
JET_H = [1.00, 0.55, 0.88, 0.42, 0.72, 0.62, 0.34]
JET_X = [-0.10, 0.58, 0.26, -0.66, -0.40, 0.74, 0.06]   # 根部横位(相对包络半宽)
