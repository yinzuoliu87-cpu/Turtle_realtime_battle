...
VERIFIED_FAR = {
    ...
    #   ⇒ 300/600/750 → 120/250/400（旧条目已删，留着会被 [STALE] 标出来）。
    #   注意这仍是**面板值**：这两段走 `_phys_after_armor`，落地约 ×0.6685。
    ('p2eq_055', '120/250/400'): 'HookBombSystem.BLAST_FLAT [120.0,250.0,400.0] 聚爆固定段 (2026-09-06 用户主动削弱; 面板值, 落地吃护甲)',
    ...
    # 它此前之所以判"近处命中", 是因为【026 雷电法杖】的 `var dmg: int = [40, 60, 90][si]`
    #   恰好落在 023 分支下方 2779 字符处(窗口 2500 的边缘), 数值撞车而已 —— 与 023 无关。
    # 023 的主动是由 `_eq_charge(..., func(): _eq_fire_coral_active(src, si))` 闭包触发的,
    #   不是 `"id": _fn(` 形状的分派 ⇒ 工具拿不到函数锚点, 本来就该走这条白名单。
    ('p2eq_023', '40/60/90'): '_eq_fire_coral_active 灼热火珊瑚主动: 接触敌施 [40,60,90] 层灼烧(闭包触发·无 id 分派锚点)',
    # ── 2026-08-11 on-hit 普攻闸(015/038/056 各插一行 `if not basic: continue`)把 026 的
    #   数组推出 ±2500 窗口边缘(023 那条已记过 026 本来就在 2779 的边上)。数值本体
