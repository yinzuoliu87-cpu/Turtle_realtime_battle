...
## ★判据放宽了一档: 原来只认 "00=" 这种紧跟等号的写法, 但 FPGA 文案里还有
...
## 同族的第二种写法: `{N:某类.某常量*100+…}` —— 常量存的是比例(0.05), 文案要的是百分数(5),
##   那个 `*100` 同样是单位换算。★判据同样窄: **紧跟在类名.常量名之后**的 `*100`。
##   (石头龟反弹公式 `REFLECT_BASE*100 + (…)*REFLECT_PER_DEF*100` 就是这个形状。)
CONST_UNIT_CONV = re.compile(r'[A-Z][A-Za-z0-9_]*\.[A-Z][A-Z0-9_]{2,}\s*\*\s*100(?![\d.])')
NOISE = {'0', '1', '2', '3'}

# 只降不升。改动后如果这个数涨了, 说明又添了没人验的数字。
BASELINE = 48   # 2026-08-25 第 68 批(1722→144)。台账见 docs/plans/20260820-文案数字根除.md。**只降不升**。
    ...
    for src, who, t in texts:
        ...
        for ph in PLACEHOLDER.findall(t):
            ...
            if UNIT_CONV.fullmatch(ph):
                ...
                n_ph += 1
                continue
            ph2, k_unit = CONST_UNIT_CONV.subn(' ', ph)
            n_unit += k_unit
            lits = [x for x in NUM.findall(ph2) if x not in NOISE]
            if lits:
