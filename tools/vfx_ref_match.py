...
SPEC = {
    ...
    "eq084-wave.png": {
        ...
        ## ★★【扇形】那条是我凭空拍的, 已改成按参考实测。
        ##   2026-08-30 量用户的两张参考图: 尾宽/前宽 = **0.56 与 1.41** ——
        ##   参考根本不是"从一点张开的扇形", 而是**两头都有宽度的湍流气墙**。
        ##   而我原来写的 0.45 等于要求"必须是三角楾" ⇒ 把用户要的形状判死。
        ##   判据错不是判据松 —— 本日第三次同一形状。
        "wedge_max": 1.60,          # 尾宽/前宽 —— 参考实测 0.56~1.41, 留一点余量
...
def main():
    ...
    ## ★圆心跟着素材镜像过: 0.664 → 0.336(2026-08-30)。
    ##   不改这里的话, 拿旧圆心去量镜像后的图 ⇒ 报 279°。全套门禁抳出来的。
    for fn, want, PIVOT in (("eq084-slash-wide.png", 120.0, (0.336, 0.728)),):
