...
## ★★偏心【随帧变强】—— **第二个参考样本逼出来的**(2026-09-14)。
##   用户:「这个是第一次时停, 这一集里应该还有多次时停, **你找另一段时停过来**」——
##   一次不算样本。用量出来的特征(一帧内亮度大涨+饱和塌陷 **且** 闪后紧跟零运动)
##   自动扫两段长视频, 从 65+49 处命中收到 3 处, 其中 430.9~431.6 秒那段是真的。
##   拿**同一把尺子**量它, 三条结构全对上, 但第三条的量程比样本① 宽得多:
##     核心掏空  样本① 189→76   样本② 183→**55**
##     峰值外移  样本① 0→7      样本② 0→**9**
##     角向变异  样本① 0.15→0.23  样本② 0.15→**0.46**
##   **两个样本都是「越扩张越不均匀」**, 而我第一版几乎不涨(0.16→0.19) ⇒ 做窄了。
##   改成随帧加强, 让末帧落在两个样本的中段。
LOPSIDE_0 = 0.34      # 首帧偏心强度(0=完全对称)
LOPSIDE_1 = 0.82      # 末帧偏心强度 —— 越扩张越偏
...
def _draw(im, fi):
    ...
    for y in range(CELL):
        for x in range(CELL):
            ...
            ## 强度随帧从 LOPSIDE_0 涨到 LOPSIDE_1 —— 两个参考样本都是「越扩张越不均匀」。
            var_lop = LOPSIDE_0 + (LOPSIDE_1 - LOPSIDE_0) * t
            a0 = math.atan2(dy, dx)
            v *= (1.0 - var_lop) + var_lop * (0.5 + 0.5 * math.cos(a0 - LOPSIDE_DIR))
            if v < 0.30:
                continue
...
def main():
    ...
    print("  ★② 角向变异系数 %.2f~%.2f (两个参考样本合起来 0.15~0.46; 均匀闭合环会接近 0)"
          % (min(cvs), max(cvs)))
    assert cvs[-1] > cvs[0] + 0.05, "角向不均匀度没有随扩张变强 —— 两个参考样本都是越扩张越偏"
