...
frames_for () {
  case "$1" in
    ...
    # 复制不留长命锁: 逐个抄【80 个可抄技能】, 每个建一场 3 单位的合成局 + 6 帧结算。
    ...
    verify_sword_storm_chain) echo 14000 ;;
    ...
    verify_eq_blood_combo)    echo 20000 ;;
    ...
    # 死技能棘轮: **逐个**由它自己的龟放 84 个技能, 每个都要建一场合成局 + 推 150 帧窗口。
    #   实测 91 秒 / 约 3 万帧。默认 500 帧只够跑前一两个 ⇒ 没打 ALL PASS, 看着像断言失败。
    ...
    # B 阶段逐步指纹: 9 个场景 × 各跑两遍真实帧(200/600/600/1000/1400/800/700/1000/1800 步)
    #   + 换种子反证 240×2 ⇒ 实测 16680 帧 / 本机 123 秒。
    #   ★判据就是"每一步都比", 帧数即分母 —— 掐断 = 分母被悄悄砍掉(比红了更危险)。
    verify_determinism_b) echo 40000 ;;
    # 跨平台确定性: 与 _b 同一张场景表(9 个场景), 但**只跑一遍**每个场景 +
    #   ①跨进程复跑第一个场景 + ③反证两遍 ⇒ 帧数约为 _b 的六成, 给同一个预算兜底。
    #   ★帧不够的表现是「没打 ALL PASS」而 rc=0(CLAUDE.md §2 那个坑) —— 实测 12000 帧
    #   时只跑到 28 条就被掐断, 日志里一条 FAIL 都没有。
    verify_determinism_cross) echo 40000 ;;
    # ★一次性测量门禁(24 个子场景 × 600 帧), 把场景 ③ 的跨平台分叉二分到具体装备。测完删。
    verify_determinism_bisect) echo 40000 ;;
    ...
    # 096 第十批(2026-09-15 加全息法阵一节): 要按【墙钟】等本体释放 + 脉冲 0.45 秒播完(约 0.75 秒),
...
# ★默认 4(2026-09-17 用户拍板调回来; 2026-08-12 曾从 4 降到 2)。
#   降到 2 的原因是这台机器 CPU 已确诊硬件故障: 08-11 一秒内 16 条 WHEA Internal parity
#   error(且首次出现第二个 APIC ID), 08-12 一天两次异常关机且【连转储都没写出来】;
...
# ★「逐帧研究过」必须有逐帧凭据 (2026-09-09 新增)。由来: 用户「我这样子强调让你拿 100 帧看,
#   你才一张张完整的看…不然你这个撒谎精是会直接跳着看几张, 然后告诉用户你学会了怎么参考的」。
#   同一天我就拿缩略接触印相扫一眼给 009 旧特效下了结论, 方向整个判反。memory 靠我想起来, 门禁自己会红。
run_audit "tools/frame_study_audit.py" "ALL OK" "frame_study (逐帧研究的行数/帧号必须对得上声明的帧数)"
