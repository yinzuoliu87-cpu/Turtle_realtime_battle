...
# 特殊占位插入顺序: back-2→front-0 (PoC syncMarkSlot [5,4,3,2,1,0])
const MARK_SLOT_ORDER := [5, 4, 3, 2, 1, 0]
...
var team: Array = [null, null, null, null, null, null]   # 6 槽, 各 pet_id 或 null
...
func _build_ui() -> void:
	...
	# 前/后排标签 (PoC .ts-fb-label: 17px, 前=#8ef0a0 后=#66d0ff, 阴影)
	...
	front_lbl.add_theme_font_size_override("font_size", _sf(17))
	front_lbl.add_theme_color_override("font_color", Color("#8ef0a0"))
	_add_text_shadow(front_lbl, Vector2(0, 2), Color(0, 0, 0, 0.75))
	root.add_child(front_lbl)
	_place(front_lbl, "frontLabel")
	...
	back_lbl.add_theme_font_size_override("font_size", _sf(17))
	back_lbl.add_theme_color_override("font_color", Color("#66d0ff"))
	_add_text_shadow(back_lbl, Vector2(0, 2), Color(0, 0, 0, 0.75))
	root.add_child(back_lbl)
	_place(back_lbl, "backLabel")
...
func _sync_special_slots() -> void:
	_sync_mark_slot("hiding", SUMMON_MARK, false, "")
	_sync_mark_slot("crystal", CRYSTAL_BALL_MARK, true, "crystalBall")
	_sync_mark_slot("candy", CANDY_BOMB_MARK, true, "candyBombPassive")
...
func _refresh_slots() -> void:
	for i in range(6):
		...
		var id = team[i]
		if id == null:
			# 空槽: active(待填, PoC .fg-active 黄亮边)/ 否则透明露出背景画好的 "+"
			panel.add_theme_stylebox_override("panel", _slot_box("active") if i == _active_slot_idx else StyleBoxEmpty.new())
			continue
...
func _on_start() -> void:
	...
	var slots: Array = []
	for i in range(6):
		# 特殊占位 (随从/水晶球/糖果炸弹) 不进 left_team — 战斗端运行时自动找槽 (1:1 PoC startBattle:1932)
	...
	# 对手: V2 优先用后端快照 (dual_ghost.leaders, 异步 ghost/bot); 无则现场随机抽 3 (兜底老路)
	var right_team: Array[String] = []
	var _ghost_leaders: Array = GameState.dual_ghost.get("leaders", []) if GameState.dual_ghost is Dictionary else []
	if _ghost_leaders.size() >= 1:
		for gid in _ghost_leaders:
			right_team.append(str(gid))
	else:
		var available_ids: Array = []
		for pet in DataRegistry.launch_pets:
			if not (pet["id"] in picked):
				available_ids.append(pet["id"])
		for i in range(available_ids.size() - 1, 0, -1):
			var j := rng.randi_range(0, i)
			var tmp = available_ids[i]
			available_ids[i] = available_ids[j]
			available_ids[j] = tmp
		for i in range(min(3, available_ids.size())):   # min 守卫防越界
			right_team.append(available_ids[i])
	...
	GameState.season_leaders = left_typed.duplicate()   # V2: 记录本赛季统领 (背包/ghost 用; 锁定强制待UI)
	GameState.right_team = right_team

	_write_last_lineup()

	if GameState.mode == "duallane":
		# 推塔(双路龟蛋): 选完3统领 → 大地图分路 (不走规则之日). 敌方=right_team(随机野生).
		get_tree().change_scene_to_file("res://scenes/DualLaneMap.tscn")
	elif GameState.mode == "single":
		# pve野生局: 先弹规则之日选择 (G3, 1:1 PoC mode==='pve' showRulePickModal), 选完写 battle_rule 再进战斗
		_show_rule_pick_modal(func(rule_name: String) -> void:
			GameState.battle_rule = rule_name
			get_tree().change_scene_to_file("res://scenes/Battle.tscn"))
	else:
		# test/其它模式: 直进战斗, 不弹规则之日 (1:1 PoC startBattle 末else → BattleScene 无规则; 原test误入else被弹窗)
		get_tree().change_scene_to_file("res://scenes/Battle.tscn")
...
# ══════════════════════════════════════════════════════════════
# 规则之日选择 modal (G3, 1:1 PoC showRulePickModal TeamSelectScene.ts:2008)
#   pve/自定义局 确认后弹: 7 规则 4列网格 + 🎲随机; 选完写 GameState.battle_rule(规则名) 再进战斗.
#   (之前完全没弹 → battle_rule 永空 → 规则系统在 pve 从不触发)
# ══════════════════════════════════════════════════════════════
func _rule_color(c: int) -> Color:
	return Color8((c >> 16) & 0xff, (c >> 8) & 0xff, c & 0xff)


func _show_rule_pick_modal(on_pick: Callable) -> void:
	var layer := CanvasLayer.new()
	layer.layer = 80
	layer.name = "RulePickModal"
	add_child(layer)
	var veil := ColorRect.new()
	veil.color = Color(0, 0, 0, 0.7)
	veil.set_anchors_preset(Control.PRESET_FULL_RECT)
	veil.mouse_filter = Control.MOUSE_FILTER_STOP   # 拦截背景 (PoC 必须选规则, 不点空白关)
	...
	bsb.content_margin_left = 28; bsb.content_margin_right = 28
	bsb.content_margin_top = 24; bsb.content_margin_bottom = 24
	bsb.shadow_color = Color(1.0, 0.851, 0.42, 0.3)   # PoC box-shadow:0 0 32px rgba(255,217,107,.3) 金光
	bsb.shadow_size = 16
	...
	box.add_child(vb)
	var title := Label.new()
	title.text = "🎯 选择本局规则"   # PoC TeamSelectScene.ts:2026 (原漏🎯)
	title.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	...
	vb.add_child(grid)
	var finish := func(rule: Dictionary) -> void:
		layer.queue_free()
		on_pick.call(str(rule.get("name", "")))
	for r in DataRegistry.battle_rules:
		grid.add_child(_make_rule_card(r, finish))
	var rand_btn := Button.new()
	rand_btn.text = "🎲 随机一个"   # PoC TeamSelectScene.ts:2059 (emoji 在前)
	rand_btn.add_theme_font_size_override("font_size", 16)
	rand_btn.size_flags_horizontal = Control.SIZE_SHRINK_CENTER   # PoC btnRow text-align:center
	# PoC randBtn 样式: bg #2a1a40 / 边 2px #ffd93d / radius8 / padding 8px 28px (TeamSelectScene.ts:2060-2064)
	var rsb := StyleBoxFlat.new()
	rsb.bg_color = Color("#2a1a40")
	rsb.border_color = Color("#ffd93d")
	rsb.set_border_width_all(2)
	rsb.set_corner_radius_all(8)
	rsb.content_margin_left = 28; rsb.content_margin_right = 28
	rsb.content_margin_top = 8; rsb.content_margin_bottom = 8
	for st in ["normal", "hover", "pressed", "focus"]:
		rand_btn.add_theme_stylebox_override(st, rsb)
	for fc in ["font_color", "font_hover_color", "font_pressed_color", "font_focus_color"]:
		rand_btn.add_theme_color_override(fc, Color("#ffd93d"))
	rand_btn.mouse_default_cursor_shape = Control.CURSOR_POINTING_HAND
	rand_btn.pressed.connect(func() -> void:
		var rs: Array = DataRegistry.battle_rules
		if rs.is_empty():
			return
		finish.call(rs[rng.randi_range(0, rs.size() - 1)]))
	vb.add_child(rand_btn)


func _make_rule_card(r: Dictionary, finish_cb: Callable) -> Control:
	var card := PanelContainer.new()
	card.custom_minimum_size = Vector2(150, 150)
	card.mouse_filter = Control.MOUSE_FILTER_STOP
	var col := _rule_color(int(r.get("color", 0x888888)))
	var csb := StyleBoxFlat.new()
	csb.bg_color = Color("#12202a")
	csb.border_color = col
	csb.set_border_width_all(2)
	csb.set_corner_radius_all(10)
	csb.content_margin_left = 12; csb.content_margin_right = 12   # PoC padding:14px 12px (L/R 12)
	csb.content_margin_top = 14; csb.content_margin_bottom = 14   # (T/B 14)
	card.add_theme_stylebox_override("panel", csb)
	# hover: 描边转金 + scale1.04 (PoC TeamSelectScene.ts:2048-2049)
	card.pivot_offset = Vector2(75, 75)
	card.mouse_entered.connect(func() -> void:
		csb.border_color = Color("#ffd93d"); card.scale = Vector2(1.04, 1.04))
	card.mouse_exited.connect(func() -> void:
		csb.border_color = col; card.scale = Vector2.ONE)
	card.gui_input.connect(func(ev: InputEvent) -> void:
		if ev is InputEventMouseButton and ev.pressed and ev.button_index == MOUSE_BUTTON_LEFT:
			finish_cb.call(r))
	...
	nm.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	nm.add_theme_font_size_override("font_size", 15)   # PoC TeamSelectScene.ts:2045 font-size:15px
	nm.add_theme_color_override("font_color", col)
	nm.mouse_filter = Control.MOUSE_FILTER_IGNORE
	v.add_child(nm)
	var desc := Label.new()
	desc.text = str(r.get("desc", ""))
	desc.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	desc.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	desc.custom_minimum_size = Vector2(130, 0)
	desc.add_theme_font_size_override("font_size", 11)
	desc.add_theme_color_override("font_color", Color("#bbccdd"))
