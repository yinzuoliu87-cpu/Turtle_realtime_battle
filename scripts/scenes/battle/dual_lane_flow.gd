...
func _dl_build_lane_field() -> void:
	...
	## ★换路把六件的累积余额清零 —— **用户 2026-08-26 拍板「清零」**, 覆盖全部六件:
	##   064 溺者的浮囊(幽灵盾) / 068 深海气压罐(法力充能) / 070 压舱咸鱼砖(灰血条)
	##   / 071 炼乳罐(奶油盾) / 072 铁皮蛋糕盒(终极盾) / 087 盗令潜水钟(压载舱)。
	## ⚠ 这里原来注的是「这就是用户定的『每路一次』口径」—— **出处引错了**(2026-08-26 翻聊天记录查实):
	##   用户那句原话(2026-08-05「…每路一次」)是在回答**专门关于 072 变身**的一问,
	##   当时被推广到了六件身上, 而另外五件从没被问过。现在由 08-26 那次拍板正式覆盖。
	## ★六件里 064/071/087 的**玩家文案本身就写着"每路"**, 072 也是用户定的 ⇒
	##   真正需要拍板的只有 068 与 070 两件, 已拍: 清零。
	battle._spec.clear_all()
	battle._hpl.clear_all()       # 换路: 血线阈值重置(同一口径)
