...
## 毒雾寿命(秒)。它同时是浓度阈值 C_min 的定义点 ⇒ R(FOG_LIFE) 精确为 0。
##
## ★★2026-08-20 修的一个**真 bug**(不是文案): 这里原本写死 `4.0`, 而逻辑侧
## `eq_venom_drone.gd` 的 `FOG_LIFE` 早在 2026-08-13 就被用户从 4 秒**加强到 6 秒**。
## 中毒判定走的是本文件的 `fog_radius(t)`, 而它在 t >= FOG_LIFE 时返回 0
## ⇒ **每团毒雾只毒 4 秒**, 逻辑侧却把它留到 6 秒, 最后 2 秒是个不毒人的空团 ——
## 等于那次加强**从来没生效过**, 而两边注释都写着"门禁要能验两侧一致"。
## ⇒ 现在**不再自己定义**, 直接引用逻辑侧那一份, 从结构上消掉这个副本。
const FOG_LIFE := VenomDroneSystem.FOG_LIFE
## 释放时的高斯烟团标准差(码)
const FOG_SIG0 := 26.0
