...
const SLAM_BURST_PX := 420.0      ## 水滴最远抛到多少码(不是 SLAM_R_PX = 500 —— 那是伤害范围, 水泼不了那么远)
                                  ## ★这里原写"不是 1000": 那是 2026-08-09 用户拍板 1000→500 之前的旧半径
...
##   ⚠ **诚实记录**: 它不是 AOE 的预告圈 —— 当时半径还是 1000 码(= 2000 码直径),
##   在 1280 px 的屏幕上根本装不下(实拍确认整圈在画面外)。
##   ★半径 2026-08-09 已由用户拍板 1000→**500**(`EqArcaneBatch.PESTLE_RADIUS`),
##     所以"装不下"这条理由现在只对**纵向**成立(直径 1000 > 战场 728, 横向 1596 放得下)。
##     真正的预告圈另有其人 —— 见下面的雷电预警圈, 它用的就是真实半径。所以这里画的是
...
##   旧版收拢到 `LEAP_WINDUP_PX = 110` 码, 而真正挨砸的当时是 1000 码, **差九倍**。
##   (今天 `SLAM_R_PX = PESTLE_RADIUS = 500` ⇒ 仍差约 4.5 倍, 结论不变。)
func _warn_batch(root: Node3D, center: Vector2, radius_px: float, col: Color) -> void:
	...
	for i in range(WARN_ARCS):
		...
		##   为什么: 当时是 1000 码半径 ⇒ 直径 2000 码, 而战场只有 1596×728 码 —— **圈比战场还大**。
		##   ★2026-08-09 半径已改 500(直径 1000): 横向进得了 1596, **纵向仍超出 728**
		##     ⇒ 圆照样会被上下切平, 这段裁边逻辑仍然必要。
...
##   ② **冲击波环** —— 半径就是真实的伤害半径(`SLAM_R_PX ≡ EqArcaneBatch.PESTLE_RADIUS` = **500** 码,
##      由 `pestle_slam(c, PESTLE_RADIUS)` 传进来), 从 0 扩到满。
##      ★这里原写"1000"是 2026-08-09 用户拍板 1000→500 之前的旧半径(1000 是现在的**直径**)。
##   ③ **落点水花** —— 一圈贴地的溅水
##   ④ **震屏** —— 7 米砸下来得有分量
