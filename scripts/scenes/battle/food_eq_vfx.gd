...
## ★不自己写一份 550: 判定用 `EqFoodBatch.BOX_TAUNT_PX`(072 的嘲讽范围本身)。
##   两个名字、两个文件、同一个数 = 改一个不改另一个就是环和真嘲讽范围对不上,
##   而 `twin_const` 只查**同名**常量、`const_leftover_audit` 只查**裸数字** ⇒ 没人守得住。
const TAUNT_RANGE_PX := EqFoodBatch.BOX_TAUNT_PX
