...
##    ⇒ 换成**真像素素材**(PixelLab 生成, 见下), 尺寸压到 **22 码宽 × 44 码高**
##      (`STICK_W_PX = 22.0` × `STICK_TEX_ASPECT = 2.0`; 这里原写"20 × 40"是 22 之前的旧值)。
##
...
##   贴图 32×64 ⇒ 实际高 = 宽 × 2 = 44 码(22×2), 与龟(47 码)同量级, 不再是 76 码的巨柱。
...
## 火头的光晕(码): 让 22 码宽(STICK_W_PX)的香在实战镜头下也有一点"亮"
const GLOW_PX := 13.0
