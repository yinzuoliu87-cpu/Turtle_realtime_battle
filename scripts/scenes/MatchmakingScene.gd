...
## MatchmakingScene — 匹配动画 (在线/野生): 匹配几秒 → 显示「我 vs 对手」(头像+ID) → 进选龟.
## 野生 = 模拟真人对手 (随机昵称/头像/ID); 在线 = 真人 (网络层 P3, 现也模拟).
...
func _ready() -> void:
	...
	var opp := _gen_opponent()
	GameState.dual_opponent = opp
	# V2 异步匹配: 后端抽同档对手快照 (ghost/池空→bot 兜底); 战斗用其阵容 (TeamSelect 读 dual_ghost.leaders)
	var _rng := RandomNumberGenerator.new(); _rng.randomize()
	GameState.dual_ghost = Backend.find_opponent(Backend.bracket_for_battles(int(GameState.season_total_battles)), [], _rng)
	_build_searching()
...
func _gen_opponent() -> Dictionary:
	var pets: Array = DataRegistry.launch_pets
	var pid := "basic"
	if not pets.is_empty():
		pid = str(pets[randi() % pets.size()].get("id", "basic"))
	return {
		"name": FAKE_NAMES[randi() % FAKE_NAMES.size()],
		"avatar": pid,
		"id": "#%06d" % (randi() % 1000000),
	}
