...
func _sk_star_gravity_warp(u: Dictionary) -> void:             # 星际龟·扭曲空间「奇点」(封板数值不动: 普通=敌阵中心500码全体0.8A魔法; 强化星能满=+拽向中心min(d×0.6,200)+清空星能; 120龟能)
	...
	# 用户2026-07-16批准: 伤害挪到爆发帧结算+施法锁(技2先例); 同日四条整改: ①普通版施法可积攒星能·强化版才锁星能 ②强化拉拽改真抛物击飞+滞空拉向中心落球心60码环("要拖到空中换位") ③特效范围=伤害范围500码对齐 ④节奏再减慢30%→吸入1.03s/爆发帧1.24s/拉拽0.57s
	var es: Array = []
	for o in _enemies_of(u):
	...
	var do_burst := func() -> void:
		...
		if not charged:                                          # 普通版收尾: 奇点缩小沉回地里(与来处对称)+淡紫残痕
			...
			return
		# ── 强化锚点③(用户2026-07-16整改"要拖到空中换位"): 真抛物击飞~0.57s+滞空拉向中心落球心60码环(大熊_pull_airborne先例) 头顶紫螺旋跟飞+尾迹+沿途白火花
		...
		for o3 in victims:
			...
			var d0: float = from3.distance_to(center)
			var oref3: Dictionary = o3
			o3["_slam"] = true                                   # 定身贯穿拉拽+落地定坐(分离pass/假人归位都吃_slam门→换位不被碰撞顶散·用户2026-07-16"碰撞体积不能完成换位")
			_knockback(uu, o3, 0.0, 1.05, 0.0)                   # 竖直击飞~0.57s(vy=6.3·无横推; 横向交给_pull_airborne)
			_pull_airborne(o3, center, maxf(0.0, d0 - 60.0), 0.55)   # 滞空平滑拉到球心60码环(照发条R卷进球心·留24px由函数兜底)
			var rel := _reg_tween()                              # 落地定坐0.55s后解定身(记得清_slam)
			rel.tween_interval(1.15)
			rel.tween_callback(func() -> void: oref3["_slam"] = false)
			_beam_vfx("res://assets/sprites/vfx/fx-trail.png", from3, center, 26.0, Color(0.72, 0.55, 1.0, 0.55), 0.64)
