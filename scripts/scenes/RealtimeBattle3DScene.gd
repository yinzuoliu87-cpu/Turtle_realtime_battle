...
func _laser_blade_sweep(u: Dictionary, origin: Vector2, dir: Vector2, rng: float, half_deg: float) -> void:   # 像素激光扇形斩·扫过动画(用户2026-07-12: 刃沿扇形从一边扫到另一边+拖尾+斩痕闪, 非静态淡入淡出)
	var base_ang: float = atan2(dir.y, dir.x)
	var wh: float = clampf(rng * WS * 0.5, 2.5, 4.2)
	# ① 亮能量刃沿120°扇形扫过(留拖尾轨迹=挥砍动作)
	var blade := Sprite3D.new()
	blade.texture = load("res://assets/sprites/vfx/laser-cleave.png")
	...
	blade.pixel_size = wh / float(maxi(1, int(blade.texture.get_height())))
	blade.modulate = Color(1.0, 0.55, 0.55, 1.0)
	_world.add_child(blade)
	var swp := _reg_tween()
	swp.tween_method(_laser_sweep_step.bind(blade, origin, base_ang, rng, wh), 0.0, 1.0, 0.15).set_trans(Tween.TRANS_SINE)
	swp.tween_callback(blade.queue_free)
	# ② 扫到位→斩痕crescent闪一下(切中的印)
	var mark := Sprite3D.new()
	mark.texture = load("res://assets/sprites/vfx/laser-slash.png")
	mark.texture_filter = BaseMaterial3D.TEXTURE_FILTER_NEAREST
	mark.billboard = BaseMaterial3D.BILLBOARD_ENABLED; mark.shaded = false; mark.transparent = true
	mark.flip_h = dir.x < 0.0
	mark.pixel_size = (wh * 1.5) / float(maxi(1, int(mark.texture.get_height())))
	mark.position = _world_pos(origin + dir * (rng * 0.55), wh * 0.5)
	mark.modulate = Color(1.0, 1.0, 1.0, 0.0)
	_world.add_child(mark)
	var mt := _reg_tween()
	mt.tween_interval(0.1)
	mt.tween_property(mark, "modulate:a", 1.0, 0.04)
	mt.tween_property(mark, "modulate:a", 0.0, 0.16)
	mt.tween_callback(mark.queue_free)

func _laser_sweep_step(fr: float, blade: Sprite3D, origin: Vector2, base_ang: float, rng: float, wh: float) -> void:   # 刃沿扇形扫过一步 + 留淡出拖尾
	if not is_instance_valid(blade): return
	var a: float = base_ang + deg_to_rad(lerpf(-60.0, 60.0, fr))
	var bd := Vector2(cos(a), sin(a))
	blade.position = _world_pos(origin + bd * (rng * 0.6), wh * 0.5)
	var tr := Sprite3D.new()
	...
	tr.pixel_size = blade.pixel_size; tr.position = blade.position
	tr.modulate = Color(1.0, 0.4, 0.42, 0.55)
	_world.add_child(tr)
	var tt := _reg_tween(); tt.tween_property(tr, "modulate:a", 0.0, 0.14); tt.tween_callback(tr.queue_free)
