...
const REVIEW_DEMO_CFG := {
	...
	"elite:1": [ {"dx": 520.0, "dy": 0.0, "fixed": true} ],   # 铁锁: 远假人>350码→链射顿+眩晕0.4s→拉体落身后1A魔(demo每~4.5s传回再看)
...
func _spawn_teams() -> void:
	...
	for i in range(left.size()):
		...
		var _lu := (_make_unit("__minion__", "left", pos, {"minion": true, "elite": true, "level": 6}) if str(left[i]) == "elite" else _make_unit(str(left[i]), "left", pos))   # REVIEW_TURTLE="elite"→精英小将demo(虐杀原形改造)
		...
		if _review_demo() and str(left[i]) == "gambler" and REVIEW_SKILL_IDX == 2:   # 赌注低血模式: 起手22%血(<40%)→触发低血分支(回8%maxHp+3秒多重·绿闪绿环)·随回血/消耗在阈值震荡(巨血防死)
			...
			_lu["_review_dummy"] = false
		if _review_demo() and str(left[i]) == "pirate" and REVIEW_SKILL_IDX == 2:   # 朗姆酒: 海盗站桩(不攻击不移动·免假人盾/巨伤干扰)+35%残血血锁不死→放朗姆酒看船扔酒瓶+HoT绿回血把血顶上来(血条明显动)+暖色护光
...
func _play_action(u: Dictionary, kind: String) -> void:
	...
	if u.get("anim_action", "") == "death":
		return
	if (u.get("anim_action", "") == "backstab" or u.get("_manual_anim", false)) and kind != "death":
		return   # 背刺committed动作: 播完前不被普攻/受击换掉(死亡除外·用户2026-07-11 动作播完前不打断)
	var id := str(u.get("id", ""))
	...
	for o in _units:
		...
		for ns in ninja_sides.keys():
			if str(o.get("side", "")) != ns:
				is_enemy = true; break
		var eligible: bool = is_enemy and o.get("alive", false) and not o.get("egg", false) and not o.get("_untargetable", false) and _t >= float(o.get("_ninja_dash_until", 0.0))
...
func _phoenix_nozzle_flash(u: Dictionary, tgt: Dictionary) -> void:   # 喷嘴白热星芒闪(照兰博Q参考: 嘴部一小簇白热放射光·快闪; 替换原贴脸dragon-flame帧=用户2026-07-15"脸上莫名图片")
	...
	glow.position = _world_pos(mouth, 0.72)
	_world.add_child(glow)
	var tw := _reg_tween()
	tw.set_parallel(true)
	...
	for i in range(2):                                               # 2道放射白色小光条(星芒感·锥内前向)
		var sa: float = cdir.angle() + _juice_rng.randf_range(-0.5, 0.5)
		var endp: Vector2 = mouth + Vector2(cos(sa), sin(sa)) * _juice_rng.randf_range(26.0, 60.0)
		_bolt_line(mouth, endp, Color(1.0, 0.97, 0.85, 0.9))
...
func _lightning_electric(u: Dictionary, target: Dictionary) -> void:   # 叠1层电击; 满8引爆(天降+清零)
	...
	if lv >= 8:
		...
		_shake(JUICE_SHAKE_LIGHT)   # 被动8层引爆=目标本地天降落雷+环, 不拉施法者→目标连线(用户2026-07-15)
...
func _shotgun_pellet(from2d: Vector2, to2d: Vector2, col: Color, dur: float = 0.42) -> void:
...
func _glow_bb(pos2d: Vector2, h: float, size_px: float, col: Color) -> MeshInstance3D:
	...
	mi.mesh = q
	var m := StandardMaterial3D.new()
	m.albedo_texture = _make_glow_texture()
	m.albedo_color = col
	m.shading_mode = BaseMaterial3D.SHADING_MODE_UNSHADED
	m.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
...
func _sk_ghost_soulstorm(u: Dictionary, tgt: Dictionary) -> void: # 幽灵龟·灵魂风暴 ✅
	...
	if cursed:
		...
		_add_dot(tgt, "curse", tgt["maxHp"] * 0.05, 10.0)   # 技2诅咒10秒(用户2026-07-11·被动登场诅咒仍5s)
...
func _diamond_roll_impact(u: Dictionary, cp: Vector2, sf: float) -> void:
	...
	_burst_vfx("res://assets/sprites/vfx/diamond-impact.png", cp, 120.0 + 70.0 * sf, 0.5)   # 水晶碎裂撞击(按速插值·越快越大)
...
func _dice_dash_tick(u: Dictionary, delta: float) -> void:
	if _t < float(u.get("dice_dash_pause_until", 0.0)):   # 到位后顿一下(0.2s)再冲下一个(用户2026-07-13)
		return
	...
	if to_land.length() <= 26.0 or to_land.dot(ddir) <= 0.0 or stuck:   # 到达落点/已穿过/卡住 → 挥剑斩结算
		...
		u["dice_dash_pause_until"] = _t + DICE_DASH_PAUSE       # 顿一下再冲
		return
	u["pos"] += ddir * DICE_DASH_SPD * delta                   # 固定速真位移(沿冲刺方向直线·穿过目标)
	u["pos"].x = clampf(u["pos"].x, ARENA.position.x, ARENA.end.x)
	u["pos"].y = clampf(u["pos"].y, ARENA.position.y, ARENA.end.y)
	u["face_right"] = ddir.x > 0.0
	if int(_t * 45.0) % 2 == 0:                      # 青蓝刀锋拖尾(刀妹穿刺残影)
...
func _dice_dash_hit(u: Dictionary, tgt: Dictionary, dir: Vector2) -> void:
	...
	_apply_damage_from(u, tgt, _atk_dmg(u, scale_i, tgt), Color("#ff4444"))
...
func _rainbow_prism_shield_vfx(u: Dictionary) -> void:
	...
	for i in range(_PRISM_RAINBOW.size()):
		var col: Color = _PRISM_RAINBOW[i]
		var tw := _reg_tween()
		...
		tw.tween_callback(func() -> void:
			...
			r.pixel_size = (66.0 * WS) / 96.0
			r.position = _world_pos(u["pos"], 0.9)
			_world.add_child(r)
...
func _sk_gambler_bet(u: Dictionary, tgt: Dictionary) -> void:    # 赌神龟·赌注(用户封板·100龟能): 需当前生命>40%; 消耗当前生命40%→7段物理砸目标(共≈0.4×当前生命); 施放3秒多重概率+20%
	...
	for i in range(7):                                         # 7张牌barrage(错峰0.11s·命中才跳伤害·每张触发多重打击B·扣'七段')
		_pending_shots.append({"delay": float(i) * 0.11, "fn": func() -> void: _gambler_throw_hit(u, tgt, per, true), "src": u})

# 当前多重打击概率(命运之轮0.60/否则0.40 + 赌注放技3秒内+0.20)
func _gambler_bet_ch(u: Dictionary) -> float:
	var ch := float(u.get("multi_base", 0.40))
	if _t < float(u.get("gambler_bet_until", 0.0)): ch += 0.20
	return ch
...
func _gambler_throw_hit(u: Dictionary, tgt: Dictionary, dmg: int, roll_multi: bool = false, is_true: bool = true) -> void:
	...
	ctw.tween_method(func(f: float) -> void:
		...
		if _cam != null:
			...
			ct.basis = _cam.global_transform.basis * Basis(Vector3(0, 0, 1), f * 11.0)
...
func _gambler_pop(pos2d: Vector2, h: float, col: Color) -> void:
	var g := _glow_bb(pos2d, h + 0.5, 155.0, col)
	var tw := _reg_tween(); tw.set_parallel(true)
...
func _gambler_wild_vfx(u: Dictionary, tgt: Dictionary) -> void:
	...
	if t != null:
		...
		var from3 := _world_pos(u["pos"], 1.0)
		var to3 := _world_pos(tgt["pos"], 1.0)
		card.position = from3
		_world.add_child(card)
		var tp: Vector2 = tgt["pos"]
		var th: float = float(tgt.get("height", 0.0))
		...
		ctw.tween_method(func(f: float) -> void:
			...
			if _cam != null:
				var ct: Transform3D = card.global_transform
				ct.basis = _cam.global_transform.basis * Basis(Vector3(0, 0, 1), f * 9.0)   # 面向相机+滚转
				card.global_transform = ct
			, 0.0, 1.0, 0.62)   # 飞行0.35→0.62s放慢看清(用户2026-07-14)
		ctw.tween_callback(func() -> void:
			if is_instance_valid(card): card.queue_free()
			if tgt.get("alive", false):
				_apply_damage_from(u, tgt, _atk_dmg(u, 1.0, tgt), Color("#ff4444"))   # ★命中才跳伤害
				_buff(tgt, "atk", -0.15, true)                                        # 减攻也命中才上
...
func _sk_hunter_shot(u: Dictionary, tgt) -> void:              # 猎人龟·精准射击(封板·90龟能·射箭+毒箭+猎杀印记三合一): 蓄力狙2.0A物理+中毒5s+治疗削减50%5s+猎杀印记5s(<24%处决)
	...
	_gambler_pop(u["pos"], float(u.get("height", 0.0)) + 0.5, Color(1.0, 0.45, 0.2, 0.7))   # 蓄力红光(复用通用glow pop)
...
func _sk_hunter_barrage(u: Dictionary, _tgt) -> void:          # 猎人龟·狩猎弹幕(封板·100龟能): 10箭·每0.2s一发·随机目标(注意龟蛋)·慢速抛物线追踪·箭头随角度·命中才跳0.36A真实(共3.6A)·每箭<14%即处决(用户2026-07-14重做)
	for i in range(10):
		...
		_pending_shots.append({"delay": float(i) * 0.2, "src": u, "fn": func() -> void:   # 每0.2s射一发
			...
			for o in _enemies_of(uu):
				if o.get("alive", false) and not o.get("_untargetable", false):
					cand.append(o)
...
func _update_hunter_passive(u: Dictionary) -> void:   # 被动猎杀(重做·用户2026-07-14): 每帧扫场→任一敌<斩杀线(蛋免疫)且无强化箭在飞→猎人自动射强化箭→命中处决
	...
	for o in _enemies_of(u):
		if not o.get("alive", false): continue
		if o.get("egg", false) or o.get("_eggImmune", false) or o.get("_untargetable", false) or o.get("eq_exec_immune", false): continue   # 免疫处决: 蛋(免控/斩)/不沉之锚(免斩杀)/不可选
...
func _fire_hunter_exec_arrow(u: Dictionary, tgt: Dictionary) -> void:   # 强化箭(金色·更大·更快·小抛物)→命中处决; 复用homing_arc弹道
	...
	_world.add_child(p)
	_gambler_pop(from2d, float(u.get("height", 0.0)) + 0.6, Color(1.0, 0.86, 0.3, 0.8))   # 金色蓄力枪口光
	_projectiles.append({
...
func _hunter_exec_arrow_hit(src, tgt) -> void:   # 强化箭命中: 目标仍<斩杀线且非免疫→处决+窃取(demo靶不真死复位)
	...
	if tgt.get("egg", false) or tgt.get("_eggImmune", false) or tgt.get("eq_exec_immune", false) or tgt.get("_untargetable", false):   # 免疫处决: 蛋/不沉之锚/不可选 → 命中但不斩
...
func _bubble_shield_burst(ally: Dictionary) -> void:
	...
	if src is Dictionary:
		src["energy_lock_until"] = _t          # 盾爆裂(到期/打破/对象死)→解锁施法者龟能(可能提前爆·用户2026-07-15)
		for o in _enemies_of(src):
			if o.get("alive", false):
...
func _bubble_gate(pos2d: Vector2) -> void:   # 泡沫门(bubble-burst立绘·涨开脉动淡出)
	...
	g.pixel_size = (185.0 * WS) / float(maxi(1, tex.get_height()))   # 门调大(用户2026-07-15两门要清楚)
	g.position = _world_pos(pos2d, 1.2)
...
func _bubble_wall_erupt(pos2d: Vector2, delay: float) -> void:   # 虚空泡沫墙的一柱: 从地面猛涌起(竖墙·马尔扎哈Q式·用户2026-07-15加大加高·清晰成墙)
	var b := _glow_bb(pos2d, 0.15, randf_range(56.0, 78.0), Color(0.6, 0.9, 1.0, 0.92))
	b.scale = Vector3(0.4, 0.4, 0.4)
	var tw := _reg_tween()
	tw.tween_interval(delay)
	tw.tween_property(b, "position", _world_pos(pos2d, 3.3), 0.22).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)   # 猛涌起(高竖墙)
...
func _bubble_rise(pos2d: Vector2) -> void:   # 泡沫上涌(青泡上飘·膨胀淡)
	var b := _glow_bb(pos2d, 0.3, randf_range(22.0, 40.0), Color(0.6, 0.9, 1.0, 0.8))
	var tw := _reg_tween(); tw.set_parallel(true)
	tw.tween_property(b, "position", _world_pos(pos2d, randf_range(1.0, 1.8)), 0.55)
...
func _sk_rainbow_storm(u: Dictionary) -> void:                  # 彩虹龟·全色风暴 (重做2026-07-13: AI棱镜漩涡+GPU彩虹粒子+碎甲可视化-20%护甲魔抗+亮边AOE·期间锁龟能)
	...
	rim.texture = _make_ring_texture(Color(1, 1, 1))
...
func _rainbow_storm_tick(u: Dictionary, center: Vector2, radius: float, ti: int) -> void:
	if not u.get("alive", false):
		return
	var col: Color = _PRISM_RAINBOW[ti % _PRISM_RAINBOW.size()]
	# 每跳: 中心冲击波脉冲(贴地扩散环·当跳色→节拍可感)
	var pulse := Sprite3D.new()
	pulse.texture = _make_ring_texture(Color(1, 1, 1))
	...
	pulse.position = _world_pos(center, 0.08)
	pulse.pixel_size = (60.0 * WS) / 96.0
	_world.add_child(pulse)
	var ptw := _reg_tween(); ptw.set_parallel(true)
...
func _rainbow_storm_end(u: Dictionary) -> void:
	for n in u.get("storm_nodes", []):
		...
		if n is GPUParticles3D:
			...
			var dt := _reg_tween(); dt.tween_interval(1.4); dt.tween_callback(n.queue_free)
		else:
			var ft := _reg_tween(); ft.tween_interval(0.25); ft.tween_callback(n.queue_free)
	u["storm_nodes"] = []
...
func _crystal_spike_line(u: Dictionary) -> void:   # 照小菊R第三击(2026-07-16逐帧): 砸地爆发→发光波头贴地冲出→波头过处水晶刺依次从地底弹出→碎晶地痕渐隐; 命中+2结晶+击飞0.8s+50%减速3s(纯控制无伤害)
	...
	burst.pixel_size = (30.0 * WS) / float(maxi(1, glow0.get_width()))
	burst.position = _world_pos(origin + dirv * 40.0, 0.5)
	_world.add_child(burst)
	var bw := _reg_tween(); bw.set_parallel(true)
	bw.tween_property(burst, "pixel_size", (90.0 * WS) / float(maxi(1, glow0.get_width())), 0.2)
	bw.tween_property(burst, "modulate:a", 0.0, 0.2)
	bw.chain().tween_callback(burst.queue_free)
	# ② 发光波头贴地冲出(小菊: 滚动推进的能量团·全程0.5s)
	var wend: Vector2 = origin + dirv * SPIKE_LINE_RANGE
	wend.x = clampf(wend.x, ARENA.position.x + 20.0, ARENA.end.x - 20.0)
	wend.y = clampf(wend.y, ARENA.position.y + 14.0, ARENA.end.y - 14.0)
	...
	head.pixel_size = (18.0 * WS) / float(maxi(1, glow0.get_width()))
	head.position = _world_pos(origin + dirv * 30.0, 0.12)
	_world.add_child(head)
	var hw := _reg_tween()
	hw.tween_property(head, "pixel_size", (52.0 * WS) / float(maxi(1, glow0.get_width())), 0.12)   # 从砸点由小鼓大(小菊z07→z09·非凭空满大)
	hw.parallel().tween_property(head, "position", _world_pos(origin + dirv * 30.0, 0.3), 0.12)
	hw.chain().tween_property(head, "position", _world_pos(wend, 0.3), SPIKE_WAVE_TIME)
	hw.parallel().tween_property(head, "modulate:a", 0.75, SPIKE_WAVE_TIME)
	...
	if htex != null:                                             # 3片碎晶伴飞(小菊: 波头是实心翻涌团非软光·用碎晶簇加密度)
		...
		for r in range(3):
			...
			rid.pixel_size = (randf_range(16.0, 24.0) * WS) / float(maxi(1, int(htex.get_height())))
			var roff: Vector2 = perp * randf_range(-20.0, 20.0) + dirv * randf_range(-16.0, 10.0)
			rid.position = _world_pos(origin + dirv * 30.0 + roff, 0.06)   # 出生贴地(从翻起的土里带出来·非凭空半空)
			rid.rotation.z = randf() * TAU
			rid.modulate = Color(0.85, 0.97, 1.0, 0.95)
			_world.add_child(rid)
			var rt := _reg_tween(); rt.set_parallel(true)
			rt.tween_property(rid, "position", _world_pos(wend + roff, randf_range(0.3, 0.5)), SPIKE_WAVE_TIME)   # 飞行中渐升(贴地→半空=被浪带起)
			rt.tween_property(rid, "rotation:z", rid.rotation.z + randf_range(-9.0, 9.0), SPIKE_WAVE_TIME)
			rt.chain().tween_property(rid, "modulate:a", 0.0, 0.1)
			rt.chain().tween_callback(rid.queue_free)
	for i in range(segs):
		var pref: Vector2 = origin + dirv * (SPIKE_LINE_RANGE * float(i + 1) / float(segs))
		_pending_shots.append({"delay": SPIKE_WAVE_TIME * float(i + 1) / float(segs), "fn": func() -> void:
			...
			if scar_tex != null:                                      # 碎晶地痕(小菊翻土痕→冰蓝碎晶·贴地1.1s渐隐)
				for _sc in range(1):   # 22段密度下每段1片痕足够
					...
					scar.pixel_size = (randf_range(10.0, 16.0) * WS) / float(maxi(1, int(scar_tex.get_height())))
					scar.position = _world_pos(pref + Vector2(randf_range(-24.0, 24.0), randf_range(-14.0, 14.0)), 0.08)
					scar.rotation.z = randf() * TAU
					scar.modulate = Color(0.75, 0.92, 1.0, 0.8)
					_world.add_child(scar)
					var sct := _reg_tween()
			...
			for o in _units:
				...
				_knock_up(o, pref, 8.8)                               # 击飞0.8s(滞空=2×8.8/22)
				o["spd_move_mult"] = 0.5
				o["spd_dbf_until"] = _t + 3.0                         # 50%减速3秒
		, "src": u})
...
func _crystal_stack(src: Dictionary, tgt: Dictionary, n: int) -> void:
	...
	if cv >= 5:
		...
		_apply_damage_from(src, tgt, _mitigate(src, tgt["maxHp"] * 0.19, tgt, true), Color("#9bdcff"), 0.0, false)   # 引爆19%最大生命魔法(吃魔抗·封板; 冰蓝2026-07-15)
...
func _crystal_orb_halo(orb: Dictionary) -> void:   # 水晶球常驻脉动光环(跟随·死亡随_follow_vfx自动清·循环tween必bind_node)
	...
	halo.pixel_size = (52.0 * WS) / float(maxi(1, glow.get_width()))
	halo.modulate = Color(0.6, 0.9, 1.0, 0.32)
	_world.add_child(halo)
	_follow_vfx.append({"spr": halo, "unit": orb, "h": 0.8})
...
func _crystal_ray_vfx(src: Dictionary, tgt: Dictionary, seg_dmg_fn: Callable) -> void:
	...
	ch.pixel_size = (10.0 * WS) / float(maxi(1, glow.get_width()))
	ch.modulate = Color(0.85, 0.97, 1.0, 0.9)
	ch.position = _world_pos(src["pos"], 1.0)
	_world.add_child(ch)
	var ct := _reg_tween()
	ct.tween_property(ch, "pixel_size", (34.0 * WS) / float(maxi(1, glow.get_width())), 0.25).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN)
...
func _elite_try_consume(u: Dictionary, tgt: Dictionary) -> bool:   # 被动1·吞噬(Consume·用户拍板: 1.5s演出不中断/可被攻击/回复=触发瞬间目标剩余HP/只偷主动技)
	...
	var uu := u
	var tref: Dictionary = tgt
	var heal_amt: float = maxf(0.0, float(tgt["hp"]))
	tgt["_consuming"] = true
	tgt["stun_until"] = maxf(float(tgt.get("stun_until", 0.0)), _t + 1.7)
	tgt["untargetable_until"] = maxf(float(tgt.get("untargetable_until", 0.0)), _t + 1.7)
	u["_slam"] = true                                             # 精英站桩播1.5s(可被攻击不免伤·不中断)
	var glow := _make_fire_glow_tex()
	var tends: Array = []
	...
	_pending_shots.append({"delay": 1.5, "fn": func() -> void:   # 吞噬完成: 黑丝缕流入→回血→击杀→偷放主动技
		...
		_heal(uu, heal_amt)                                        # 回复=吞噬瞬间目标剩余HP(用户拍板)
...
func _elite_steal_skill_type(tref: Dictionary) -> String:        # 吞噬偷技: 目标主动技(passiveSkill不放·未实装不放·无技能不放)
...
func _chest_basic(u: Dictionary, tgt: Dictionary) -> void:       # 普攻·宝箱砸击(封板): K'Sante一段Q式·朝目标前方短直线AOE·各1A物理(近战扫一小片非单体)
	...
	for o in _enemies_of(u):
		...
		var oref: Dictionary = o
		_pending_shots.append({"delay": wave_t * clampf(along / reach, 0.0, 1.0), "fn": func() -> void:   # 波头扫到才结算(近先远后)
			if not oref.get("alive", false) or not uu.get("alive", false): return
			_apply_damage_from(uu, oref, _atk_dmg(uu, 1.0, oref) + bonus, Color("#ffd93d"))
			_burst_vfx("res://assets/sprites/vfx/treasure-slam.png", oref["pos"], 130.0)                  # 命中放射爆闪(s21·波到才炸)
		, "src": u})
	# ① 砸点层(s16): 尘环+金爆+轻震
	_shake(0.05)
	_skill_ring(u["pos"] + dir * 42.0, Color(1.0, 0.85, 0.3, 0.7), 30.0)
	_burst_vfx("res://assets/sprites/vfx/treasure-slam.png", u["pos"] + dir * 46.0, 120.0)
	# ② 金波头推进(0.18s冲到尽头→残亮头0.3s熄灭·不凭空消失)
	var glowt := _make_fire_glow_tex()
	var wh := Sprite3D.new()
	wh.texture = glowt
	wh.billboard = BaseMaterial3D.BILLBOARD_ENABLED; wh.shaded = false; wh.transparent = true
	wh.pixel_size = (44.0 * WS) / float(maxi(1, glowt.get_width()))
	wh.modulate = Color(1.0, 0.85, 0.35, 0.95)
	wh.position = _world_pos(u["pos"] + dir * 46.0, 0.18)
	_world.add_child(wh)
	var wt := _reg_tween()
	wt.tween_property(wh, "position", _world_pos(u["pos"] + dir * reach, 0.18), wave_t)
	wt.tween_property(wh, "pixel_size", (20.0 * WS) / float(maxi(1, glowt.get_width())), 0.3)   # 尽头残亮头收小熄灭(s26远端)
	...
	_beam_vfx("res://assets/sprites/vfx/fx-energy-beam.png", u["pos"], u["pos"] + dir * reach, 96.0, Color(1.0, 0.85, 0.25, 0.7), 0.16, 0.06)
	_beam_vfx("res://assets/sprites/vfx/fx-energy-beam.png", u["pos"], u["pos"] + dir * reach, 58.0, Color(1.0, 0.6, 0.15, 0.5), 0.6, 0.06)
	# ④ 尘旋圈随波头依次留下(s26·非同帧全出)
	for r in range(3):
		var rq: float = 0.3 + 0.28 * float(r)
		var rp: Vector2 = u["pos"] + dir * (reach * rq)
		_pending_shots.append({"delay": wave_t * rq, "fn": func() -> void:
			_skill_ring(rp, Color(1.0, 0.8, 0.35, 0.4), 26.0)
		, "src": u})
...
func _sk_star_gravity_warp(u: Dictionary) -> void:             # 星际龟·扭曲空间「奇点」(封板数值不动: 普通=敌阵中心500码全体0.8A魔法; 强化星能满=+拽向中心min(d×0.6,200)+清空星能; 120龟能)
	...
	# 用户2026-07-16批准: 伤害挪到爆发帧结算+施法锁(技2先例); 同日整改: ①普通版施法可积攒星能·强化版才锁星能 ②强化拉拽=真抛物击飞+滞空拽穿奇点镜像落对侧(真"换位"对调, 非聚拢) ③特效范围=伤害范围500码对齐 ④节奏再减慢30%→吸入1.03s/爆发帧1.24s/拉拽0.57s
	var es: Array = []
	for o in _enemies_of(u):
	...
	var uu := u
	u["energy_lock_until"] = maxf(float(u.get("energy_lock_until", 0.0)), _t + 2.4)   # 施法锁龟能(爆发/拽完提前恢复·兜底防永锁)
	if charged:
	...
	_world.add_child(halo)
	var ct := _reg_tween(); ct.set_parallel(true)
	ct.tween_property(core, "position", _world_pos(center, 0.9), 0.21).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
	...
	var tr_tex := _make_thin_ring_tex()                          # 细线环(48px像素环放大到1000码=满地白格子, 自验截图抓过)
	var ring := Sprite3D.new()
	ring.texture = tr_tex
	...
	ring.position = _world_pos(center, 0.065)
	_world.add_child(ring)
	var rt := _reg_tween()
	rt.tween_method(func(q: float) -> void:
		if is_instance_valid(ring): ring.pixel_size = (maxf(20.0, 1000.0 * q) * WS) / 256.0
	...
	if charged:                                                  # 强化锚点①: 贴地紫黑吸积盘持续自转(远看即知强化)
		...
		for o in es:                                             # 强化锚点②前奏: 敌人脚下指向中心的紫色拉力短痕
			...
			_beam_vfx("res://assets/sprites/vfx/fx-trail.png", o["pos"], (o["pos"] as Vector2) + pd * 46.0, 14.0, Color(0.62, 0.42, 1.0, 0.5), 0.86, 0.06)
	...
	var do_burst := func() -> void:
		if not uu.get("alive", false):                           # 施法者阵亡→奇点静默消散(有去处), 不结算
			if is_instance_valid(core):
				...
				cf.tween_property(core, "position", _world_pos(center, 0.08), 0.43)
		...
		if not charged:                                          # 普通版收尾: 奇点缩小沉回地里(与来处对称)+淡紫残痕
			...
			cd.tween_property(core, "position", _world_pos(center, 0.08), 0.64).set_delay(0.36)
			...
			return
		# ── 强化锚点③(用户2026-07-16"要拖到空中换位"): 真抛物击飞~0.57s+滞空拽穿奇点·镜像落对侧=真换位对调 头顶紫螺旋跟飞+尾迹+沿途白火花
		...
		for o3 in victims:
			...
			var dest3 := Vector2(clampf(center.x * 2.0 - from3.x, ARENA.position.x, ARENA.end.x), clampf(center.y * 2.0 - from3.y, ARENA.position.y, ARENA.end.y))   # 换位=过奇点镜像落对侧(用户2026-07-16"你理解什么叫换位吗": 左右对调, 非聚拢; 镜像保距→落点互不重叠)
			o3["_slam"] = true                                   # 定身贯穿拉拽+落地定坐(分离pass/假人归位都吃_slam门→换位不被碰撞顶散·用户2026-07-16"碰撞体积不能完成换位")
			_knockback(uu, o3, 0.0, 1.05, 0.0)                   # 竖直击飞~0.57s(vy=6.3·无横推; 横向对穿由下方mvt驱动)
			var mvt := _reg_tween()                              # 滞空拽着穿过奇点到镜像位(airborne期分离不生效→对穿无碰撞)
			mvt.tween_method(func(q: float) -> void:
				if oref3.get("alive", false) and oref3.get("airborne", false):
					oref3["pos"] = from3.lerp(dest3, q)
			, 0.0, 1.0, 0.55).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN_OUT)
			var rel := _reg_tween()                              # 落地定坐0.55s后解定身(记得清_slam)
			rel.tween_interval(1.15)
			rel.tween_callback(func() -> void: oref3["_slam"] = false)
			_beam_vfx("res://assets/sprites/vfx/fx-trail.png", from3, dest3, 26.0, Color(0.72, 0.55, 1.0, 0.55), 0.64)
			...
			swirl.position = _world_pos(from3, 1.2)
			...
			pt3.tween_method(func(q: float) -> void:             # 跟飞: 螺旋悬在头顶(含击飞高度), 沿途白粉火花
				...
				if oref3.get("alive", false):
					swirl.position = _world_pos(oref3["pos"], float(oref3.get("height", 0.0)) + 1.2)
				...
				if spark_i[0] % 5 == 0 and oref3.get("alive", false):   # 沿途白粉火花星屑(发条R脚下迸溅)
					...
					sp.position = _world_pos((oref3["pos"] as Vector2) + Vector2(randf_range(-16.0, 16.0), randf_range(-16.0, 16.0)), maxf(0.05, float(oref3.get("height", 0.0)) * randf()))
					_world.add_child(sp)
					var spt := _reg_tween()
		...
		var fin := _reg_tween()                                  # 拽完落地: 恢复锁+奇点吃饱膨胀一拍再缩灭+紫环印记缓隐
		...
		fin.tween_callback(func() -> void:
			...
			if is_instance_valid(core):
				...
				eat.parallel().tween_property(core, "position", _world_pos(center, 0.08), 0.57)
	...
	if charged:                                                   # ── 强化: 巨彗星「天崩」(逐帧订正asol_hi a30/a38/a44)
		...
		var comet := func() -> void:                              # 1.0s后: 坠落0.25s(大火球头+束状粗光柱+嵌星+坠落微震)
			...
			ct2.tween_callback(func() -> void:
				if is_instance_valid(head): head.queue_free()
				for o2 in _enemies_of(uu2):                        # 伤害撞击帧结算(用户2026-07-16: 100%消耗星能·真实伤害·400码)
					if o2.get("alive", false) and o2["pos"].distance_to(center) <= 400.0:
						_apply_damage_from(uu2, o2, maxi(1, int(burst)), Color("#ffffff"), 0.0, true)
				...
				for si in range(12):                               # 外围星星闪点
					...
					ss.position = _world_pos(sp2, randf_range(0.3, 1.4))
					_world.add_child(ss)
					var st2 := _reg_tween()
...
func _sk_candy_bomb_feed(u: Dictionary) -> void:               # 糖果龟·技能三糖果炸弹(封板·喂续命): 炸弹活→上限+25%糖果龟maxHp+治疗10%(喂); 炸弹亡→召新HP=20%糖果龟maxHp (登场召唤+死亡爆炸在spawn/summon)
	...
	if bomb != null:
		...
		_gambler_pop(bomb["pos"], float(bomb.get("height", 0.0)) + 0.4, Color(1.0, 0.7, 0.88, 0.85))   # 喂养涨大糖光
...
func _candy_sweet_drain(u: Dictionary) -> void:   # 甜蜜掠夺·甜蜜吸取(用户2026-07-15"第8秒生效"): 对最大生命最高敌吸25%maxHp→全回复自己(不杀留1)+粉精华VFX
	...
	if steal > 0:
		_raw_lose(fat, steal); _heal(u, steal)
...
func _lava_volcano_erupt(u: Dictionary) -> void:                 # 火山·火山爆发(用户2026-07-15定稿): 目标那头的尽头生成岩浆浪潮→朝火山龟压过来(250码/s)→穿过龟到另一头尽头消散; 碾过每敌击飞+5段0.5A魔+灼烧+回血12%
	var dir: Vector2 = _densest_enemy_point(u, 400.0) - u["pos"]   # 瞄准轴: 朝敌最密方向
	dir = dir.normalized() if dir.length() > 0.1 else Vector2.RIGHT
	...
	tel.position = _world_pos(origin, 0.03)                       # 预警=以龟为中心±600码走廊(浪的整条路径)
	...
	wave.position = _world_pos(start, 0.12)                       # 起步: 自己脚前(娜美R式从施法者推出)
	...
	var wall_w: float = (half_w * 2.0 + 60.0) * WS                # 浪宽(横向·世界)
	...
	wall.position = _world_pos(start, 0.04)                      # 底边贴地·从自己脚前出发
	...
	var step := func(d: float) -> void:
		...
		if is_instance_valid(wall):
			wall.position = _world_pos(front, 0.04)               # 浪壳随波前推进(mesh底边即地面)
			(wall.material_override as ShaderMaterial).set_shader_parameter("intensity", 0.92 + 0.08 * sin(_t * 13.0))
			if _juice_rng.randf() < 0.55:                         # 浪顶洒火星熔滴(行进点缀)
				var sx: Vector2 = front + perp_axis * _juice_rng.randf_range(-half_w * 0.8, half_w * 0.8)
				var sp := Sprite3D.new()
				sp.texture = _make_fire_glow_tex()
				sp.billboard = BaseMaterial3D.BILLBOARD_ENABLED; sp.shaded = false; sp.transparent = true
				sp.pixel_size = 0.0042
				sp.modulate = Color(1.0, 0.8, 0.4, 0.95)
				sp.position = _world_pos(sx, wall_h * _juice_rng.randf_range(0.7, 1.0))
				_world.add_child(sp)
				var st := _reg_tween(); st.set_parallel(true)
				st.tween_property(sp, "position", _world_pos(sx + tdir * _juice_rng.randf_range(20.0, 60.0), 0.1), 0.4).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
				st.tween_property(sp, "modulate:a", 0.0, 0.4)
				st.chain().tween_callback(sp.queue_free)
			if _juice_rng.randf() < 0.6:                          # ★雾云尾迹(娜美R观察: 不规则软雾云包四周+尾部拖白雾)
				var mpos: Vector2 = front + perp_axis * _juice_rng.randf_range(-half_w * 1.1, half_w * 1.1) - tdir * _juice_rng.randf_range(0.0, 180.0)
				var mf := Sprite3D.new()
				mf.texture = _make_fire_glow_tex()
				...
				mf.position = _world_pos(mpos, _juice_rng.randf_range(0.2, wall_h * 0.9))
				_world.add_child(mf)
				var mt := _reg_tween(); mt.set_parallel(true)
				mt.tween_property(mf, "scale", Vector3(1.6, 1.6, 1.6), 0.7)
				mt.tween_property(mf, "position", _world_pos(mpos - tdir * 40.0 + perp_axis * _juice_rng.randf_range(-30.0, 30.0), wall_h * 1.1), 0.7)
		...
		for o in _enemies_of(u):
			...
			if not o.get("airborne", false):                      # 击飞+击退加强(用户2026-07-15: vy 4→6·推力×1.6)
				o["airborne"] = true
				o["vy"] = 6.0
				o["vx"] = tdir.x * KNOCK_PUSH * 1.6
				o["vz"] = tdir.y * KNOCK_PUSH * 1.6
			for i in range(5):
	...
	_gambler_pop(start, 0.6, Color(1.0, 0.85, 0.5, 0.95))         # ★出生爆闪(娜美R观察: 浪在白亮爆闪中诞生·非淡入)
	_skill_ring(start, Color(1.0, 0.6, 0.2, 0.8), 70.0)
	for bi in range(6):                                           # 放射光条
		var ba: float = TAU * float(bi) / 6.0 + _juice_rng.randf_range(-0.2, 0.2)
		_bolt_line(start, start + Vector2(cos(ba), sin(ba)) * _juice_rng.randf_range(40.0, 80.0), Color(1.0, 0.9, 0.6, 0.9))
	_shake(JUICE_SHAKE_BIG)
	...
	if is_instance_valid(tel):
		tel.queue_free()
	if is_instance_valid(wall):
		var wf := _reg_tween()                                    # ★消散摊平(娜美R观察: 到端摊平变薄成一层膜+渐隐·非突然消失)
...
var _thin_ring_cache: ImageTexture = null
func _make_thin_ring_tex() -> ImageTexture:   # 高分细线圆环(256px·细带2px软边; 大范围预告圈用——48px像素环放大到1000码会糊成格子; 白色modulate上色)
...
var _pixel_ring_cache: ImageTexture = null
func _make_pixel_ring_tex() -> ImageTexture:   # 硬边像素圆环(48px低分辨率·无AA·NEAREST放大=像素锯齿边; 白色modulate上色)
...
var _pixel_block_cache: ImageTexture = null
func _make_pixel_block_tex() -> ImageTexture:   # 6px实心像素块(尘土/碎屑用·白色modulate上色)
...
func _lava_slam_telegraph(pos2d: Vector2, radius: float, dur: float) -> void:   # 落点预警(像素化·用户2026-07-15): 外硬边像素环+内环倒计时收缩到圆心+中心裂纹微光
	var outer := _px_ground_sprite(_make_pixel_ring_tex(), pos2d, radius * 2.0, Color(1.0, 0.32, 0.1, 0.0), 0.045)
	var inner := _px_ground_sprite(_make_pixel_ring_tex(), pos2d, radius * 2.0, Color(1.0, 0.62, 0.2, 0.0), 0.05)
	var crack := _px_ground_sprite(load("res://assets/sprites/vfx/lava-crack.png"), pos2d, radius * 1.1, Color(1, 1, 1, 0.0), 0.035)   # 中心裂纹预示(淡)
	var td := _reg_tween()
...
func _lava_charge_vfx(u: Dictionary) -> void:   # 滞空蓄力: 火山龟身上聚火光 (越蓄越盛, 临砸最亮)
	...
	for i in range(8):                                            # 火星向心汇聚(吸能感·像素化设计2026-07-15)
		...
		var soff: Vector2 = Vector2(cos(sa), sin(sa)) * _juice_rng.randf_range(70.0, 110.0)
		var sp := Sprite3D.new()
		sp.texture = _make_pixel_block_tex()
		sp.texture_filter = BaseMaterial3D.TEXTURE_FILTER_NEAREST
		sp.billboard = BaseMaterial3D.BILLBOARD_ENABLED; sp.shaded = false; sp.transparent = true
		sp.pixel_size = 0.018
		sp.modulate = Color(1.0, 0.75, 0.3, 0.0)
		sp.position = _world_pos(u["pos"] + soff, LAVA_LEAP_H + _juice_rng.randf_range(-0.3, 0.5))
		_world.add_child(sp)
		var st := _reg_tween()
		st.tween_interval(float(i) * 0.06)
		st.tween_property(sp, "modulate:a", 0.95, 0.08)
		st.tween_property(sp, "position", _world_pos(u["pos"], LAVA_LEAP_H + 0.3), 0.3).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN)
...
func _lava_volcano_slam(u: Dictionary) -> void:   # 加里奥R式: 高跃升+飞落点 → 滞空蓄力(预警) → 砸地击飞+爆发
	u["_slam"] = true
	u["untargetable_until"] = _t + LAVA_LEAP_UP_T + LAVA_CHARGE_T + LAVA_SLAM_T + 0.05   # 滞空不可被主动索敌(能受伤·敌立刻换目标·用户2026-07-15)
	var start: Vector2 = u["pos"]
	...
	for i in range(9):                                            # 起跳点像素尘土块弹开(2026-07-15像素化)
		var da: float = TAU * float(i) / 9.0 + _juice_rng.randf_range(-0.3, 0.3)
		var dp := Sprite3D.new()
		dp.texture = _make_pixel_block_tex()
		dp.texture_filter = BaseMaterial3D.TEXTURE_FILTER_NEAREST
		dp.billboard = BaseMaterial3D.BILLBOARD_ENABLED; dp.shaded = false; dp.transparent = true
		dp.pixel_size = 0.02 * _juice_rng.randf_range(0.7, 1.2)
		dp.modulate = Color(0.75, 0.6, 0.45, 0.9)                # 土色尘块
		dp.position = _world_pos(start, 0.15)
		_world.add_child(dp)
		var de: Vector2 = start + Vector2(cos(da), sin(da)) * _juice_rng.randf_range(40.0, 85.0)
		var dt := _reg_tween(); dt.set_parallel(true)
		dt.tween_property(dp, "position", _world_pos(de, _juice_rng.randf_range(0.1, 0.5)), 0.38).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
	...
	u["untargetable_until"] = _t                                  # 落地恢复可被索敌
...
func _lava_slam_impact(u: Dictionary, center: Vector2) -> void:   # 落地: 击飞范围内全部敌 + 入场爆发AOE
	...
	for ri in range(2):                               # 像素冲击环×2错峰扩散(硬边·替换软_skill_ring·2026-07-15像素化)
		var pring := _px_ground_sprite(_make_pixel_ring_tex(), center, LAVA_SLAM_RADIUS * 2.0, Color(1.0, 0.5 + 0.25 * float(ri), 0.15, 0.0), 0.05 + 0.01 * float(ri))
	...
	var crk := _px_ground_sprite(load("res://assets/sprites/vfx/lava-crack.png"), center, LAVA_SLAM_RADIUS * 1.3, Color(1, 1, 1, 0.0), 0.04)   # 裂地贴片(停留后淡出)
	var ct := _reg_tween()
	...
	for i in range(10):
		var ra: float = TAU * float(i) / 10.0 + _juice_rng.randf_range(-0.25, 0.25)
		var rdist: float = _juice_rng.randf_range(70.0, 170.0)
		var rp := Sprite3D.new()
		rp.texture = rtex
		...
		rp.flip_h = _juice_rng.randf() < 0.5
		rp.pixel_size = (26.0 * WS) / float(maxi(1, rtex.get_height())) * _juice_rng.randf_range(0.7, 1.3)
		rp.position = _world_pos(center, 0.5)
		_world.add_child(rp)
		var endp: Vector2 = center + Vector2(cos(ra), sin(ra)) * rdist
		var hpk: float = _juice_rng.randf_range(0.8, 1.5)
		var life: float = _juice_rng.randf_range(0.32, 0.5)
		var rw := _reg_tween()
		rw.tween_method(func(p: float) -> void:
			if is_instance_valid(rp): rp.position = _world_pos(center.lerp(endp, p), 0.5 + hpk * 4.0 * p * (1.0 - p) - 0.4 * p)   # 抛物(峰在中段·落回地)
		, 0.0, 1.0, life)
	...
	if u["id"] == "bubble":
		...
		if u["_bbtimer"] >= 5.0:                       # 用户2026-07-15: 3→5秒
			...
			if bs >= 1.0:
				...
				for _bh in range(2): _bubble_rise(u["pos"])   # 泡沫被动proc: 自身回血泡泡(用户2026-07-14)
				...
				if bt != null:
					...
					_bubble_rise(bt["pos"]); _bubble_rise(bt["pos"])   # 命中泡沫破
	...
	for i in range(n):
		...
		if d["fire_t"] <= 0.0:
			...
			if tgt != null and tgt.get("alive", false):
				...
				mz.texture = _make_fire_glow_tex()
				...
				p.texture = _make_bolt_texture(Color(0.55, 0.95, 1.0))
...
func _tick_summon_special(u: Dictionary, delta: float) -> void:
	...
	match special:
		...
		"ray":                                            # 水晶球射线(2026-07-16重做): 蓄力聚能→两段厚冰蓝光束·伤害不变(2段×0.5A魔法+2层结晶)
			var t = _nearest_enemy(u)
			if t == null: return
			_crystal_ray_vfx(u, t, func(sr: Dictionary, tr: Dictionary, last: bool) -> void:
				_apply_damage_from(sr, tr, _atk_dmg(sr, sr.get("special_scale", 1.0), tr, true), Color("#9bdcff"), 0.0, true)
				if last: _crystal_stack(sr, tr, 2))   # 与本体共享满5引爆(引爆改吃魔抗)
		"random_hit":
...
func _dice_scythe_sweep(u: Dictionary, origin: Vector2, dir: Vector2, rng: float, half_deg: float) -> void:
	...
	blade.pixel_size = (rng * 0.85 * WS) / fh          # 镰刀高 ≈ 0.85×射程(盖住扇形纵深)
...
func _eq_laser_chop(u: Dictionary, tgt: Dictionary, si: int, base_range: float) -> void:   # 竖劈冲击波(同斩击伤害/宽80/移动2×射程/短暂击飞0.1s)
	...
	_shake(JUICE_SHAKE_BIG)
	_play_anim_vfx("res://assets/sprites/vfx/laser-cleave-anim.png", tgt["pos"], 155.0, 20.0, 1.5, false)   # 竖劈5帧砸向目标(用户素材·斜刃下落→劈地红爆)
	...
	while traveled < reach and is_instance_valid(wave) and is_instance_valid(self):
		...
		wave.position = _world_pos(origin + dir * traveled, _wwh * 0.5)   # 底边贴地不入地(用户2026-07-12)
