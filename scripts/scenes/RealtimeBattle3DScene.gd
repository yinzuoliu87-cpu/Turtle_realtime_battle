...
func _eq_coral_spike(u: Dictionary, far: Dictionary, si: int) -> void:   # 珊瑚刺弹体→最远敌, 到达: 物理+%maxHP魔法(错峰显示)
	...
	_apply_damage_from(u, far, _atk_dmg(u, [1.0, 1.2, 1.5][si], far), Color("#ff6b5b"), 0.0, false, true)   # 物理
	await get_tree().create_timer(0.12).timeout   # 错峰→魔法数字不叠物理上
	if far.get("alive", false):
		var mrm: float = 40.0 / (40.0 + maxf(0.0, float(far["mr"])))   # 魔抗减免K=40(用户: 魔法必过魔抗)
		_last_dmg_type = "magic"
		_apply_damage_from(u, far, maxi(1, int(far["maxHp"] * [0.08, 0.12, 0.18][si] * mrm)), Color("#bfe9ff"), 0.0, false, true)   # %maxHP魔法(蓝, 过魔抗)
