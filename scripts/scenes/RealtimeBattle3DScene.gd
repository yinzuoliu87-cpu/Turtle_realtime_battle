...
func _bear_shockwave(u: Dictionary, tgt: Dictionary, _si: int) -> void:   # 大熊冲击波(小菊式): 蓄力→直线移动波, 1.5ATK物理+击飞0.8s+拉回70码
	...
	if is_instance_valid(glow): glow.queue_free()
	if not u.get("alive", false): return
	_shake(JUICE_SHAKE_BIG)   # 砸地
	# 释放: 冲击波沿 dir 前进, 沿途暖金块一簇簇破土冒起(小菊式地面喷涌), 波前首经过即命中
...
func _summon_walking_bear(u: Dictionary, tgt: Dictionary, dmg: int) -> void:   # 玩偶小熊仔: 召出走路动画小熊→走向敌→踢击动画(伤+击飞)→消失
	...
	bear.texture = load("res://assets/sprites/vfx/bear-walk.png")   # 7帧走路
	bear.hframes = 7
	bear.frame = 0
	bear.pixel_size = 1.2 / 96.0   # ~1.2m 高小熊仔 (小于2m龟)
	bear.offset = Vector2(0.0, 48.0)   # 底部对齐地面 (96帧半高)
	...
	while is_instance_valid(bear) and tgt != null and tgt.get("alive", false):
		...
		bear.flip_h = float(tgt["pos"].x) < pos.x   # 面向目标
	...
	if tgt != null and tgt.get("alive", false) and is_instance_valid(bear):
		bear.texture = load("res://assets/sprites/vfx/bear-attack.png")
		bear.hframes = 5
		bear.frame = 0
		var kt := 0.0
		var hit := false
