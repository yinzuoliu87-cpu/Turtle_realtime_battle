...
func _sk_crystal_bulwark(u: Dictionary) -> void:                 # 水晶龟·水晶壁垒(封板数值; 2026-07-15补冰蓝VFX): 1.5A护盾+全体友军护甲魔抗+15%·4秒
	_grant_shield(u, u["atk"] * 1.5, 4.0)
	...
	if dtex != null:
		...
		dw.tween_callback(dome.queue_free)
	for o in _allies_of(u):
...
func _sk_cyber_smart(u: Dictionary) -> void:                   # 赛博龟·智能AI(封板·40龟能·充能型): +1充能→冲刺重定位(kite拉到理想射程·躲贴身); 常驻走位+登场+20%移速 [完整行动脑(躲大招/对齐激光)留F5]
	...
	if ne != null:
		var best: Vector2 = u["pos"]                            # ★真智能走位(用户2026-07-16"被逼到角落就不动"): 绕最近敌一圈采样16个候选点打分(离全体敌远+离墙远+移动代价), 被堵角自动从敌侧面绕出去
		var best_score := -INF
		var es := _enemies_of(u)
		for i in range(16):
			var aa: float = TAU * float(i) / 16.0
			var cand: Vector2 = ne["pos"] + Vector2(cos(aa), sin(aa)) * 380.0
			...
			var score: float = minf(d_enemy, 420.0) + minf(d_wall, 160.0) * 0.8 - cand.distance_to(u["pos"]) * 0.15
			if score > best_score:
				best_score = score
				best = cand
