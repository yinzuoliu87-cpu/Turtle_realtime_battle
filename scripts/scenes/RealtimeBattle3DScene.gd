...
const REVIEW_DEMO_CFG := {
	...
	"elite:-1": [ {"dx": 140.0, "dy": 0.0} ],   # 吞噬: 假人钉18%残血带技能→<15%吞噬1.5s(卷须缠绕染黑缩小→黑丝缕流入)+回血+偷放技(demo复活循环)
...
func _tick_elite_whip(u: Dictionary) -> void:                    # 被动2·铁锁(Whipfist Longshot·CD5s用户拍板): 索敌>350码→链射→顿+目标眩晕0.4s→拉体落身后+1A魔法
	...
	if d <= 350.0:
		if _review_demo() and _review_turtle() == "elite" and _review_skill_idx() == 1 and _t >= float(u.get("_whipdemo_next", 1.0e9)):
