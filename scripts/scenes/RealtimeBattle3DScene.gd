...
const GROUND_FAR := Color(0.03, 0.1, 0.13)   # 远/边缘地色 (深蓝青murk, 提亮点→远处是幽暗海水不是黑洞)
...
func _build_environment() -> void:
	...
	sky_mat.sky_top_color = Color(0.01, 0.028, 0.045)      # 顶部深蓝(略提亮, 别纯黑)
	sky_mat.sky_horizon_color = Color(0.022, 0.06, 0.085)  # 水平线深青murk (提亮→远景是海水不是黑幕)
	sky_mat.ground_bottom_color = Color(0.014, 0.04, 0.058)
	sky_mat.ground_horizon_color = Color(0.022, 0.06, 0.085)
...
void fragment() {
	...
	float sink = 1.0 - 0.6 * smoothstep(1.1, 3.6, d);   // 远处渐入幽暗(拉长过渡+不沉全黑)→ 深海往远处延伸感, 不硬切黑洞
