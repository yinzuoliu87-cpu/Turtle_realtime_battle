...
const REVIEW_SKILL_IDX := 0   # 评审受审龟放哪个技(skillPool索引): 0=普攻/1-3=候选技/-1=默认轮转 (财神: 0=普攻(看聚宝盆被动冒金币)/1=骰子/2=梭哈/3=招财进宝)
...
const REVIEW_DUMMY_KILLABLE := true   # true=假人会死(看换目标·财神被动:死亡时金币涌向财神); false=不死回满沙包(看完整动画)
