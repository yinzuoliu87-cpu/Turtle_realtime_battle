...
const REVIEW_SKILL_IDX := 2   # 评审受审龟放哪个技(skillPool索引): 0=普攻/1-3=候选技/-1=默认轮转 (猎人: 0射箭✅/1精准射击✅/2隐蔽/3狩猎弹幕)
...
const REVIEW_DEMO_CFG := {
	...
	"hunter:2": [ {"dx": -165.0, "dy": 20.0}, {"dx": 165.0, "dy": -20.0} ],   # 猎人隐蔽: 双侧melee假人夹击(都<150威胁·不固定→追击)→触发智能翻滚"远离最近近战"分支①·猎人在两假人间左右横跳(方向随最近威胁反转→留在中央开阔区)·看残影拖尾+起落尘+灵巧绿环·下发普攻绿色强化数字
}
...
func _sk_hunter_hide(u: Dictionary) -> void:                     # 猎人龟·隐蔽(封板·薇恩Q Tumble·80龟能): 智能翻滚~250码→下次普攻附带0.9A物理(吃吸血)+25%闪避+0.7A护盾
	...
	_hunter_roll_fx(u, start, dest)                            # 翻滚残影(立绘幻影拖尾)+起落尘+灵巧绿环
	u["pos"] = dest
...
func _hunter_roll_fx(u: Dictionary, from2d: Vector2, dest: Vector2) -> void:
	# 隐蔽翻滚观感(薇恩Q Tumble): 沿路径铺猎人立绘幻影(向起点递减alpha=运动拖尾) + 起跳/落地尘 + 灵巧绿环(闪避激活)
	var spr = u.get("sprite", null)
	var uh: float = float(u.get("height", 0.0))
	if is_instance_valid(spr) and spr.texture != null:
		var n := 7
		for i in range(n):
			var f := float(i) / float(n - 1)                   # 0=起点(最虚) → 1=终点(最实)
			var g := Sprite3D.new()
			...
			g.offset = spr.offset
			g.position = _world_pos(from2d.lerp(dest, f), uh)
			g.modulate = Color(0.35, 1.0, 0.6, 0.22 + 0.5 * f)  # 灵巧绿幻影(更鲜亮更实)·越近终点越实
			_world.add_child(g)
			var tw := _reg_tween()
			tw.tween_property(g, "modulate:a", 0.0, 0.22 + f * 0.08)
			tw.tween_callback(g.queue_free)
	_beam_vfx("res://assets/sprites/vfx/fx-trail.png", from2d, dest, 34.0, Color(0.5, 1.0, 0.72, 0.6), 0.32)   # 侧翻绿拖影束
	_burst_vfx("res://assets/sprites/vfx/dust-impact.png", from2d, 175.0, 0.12)   # 起跳轻尘
	_burst_vfx("res://assets/sprites/vfx/dust-impact.png", dest, 200.0, 0.12)     # 落地轻尘
	_skill_ring(dest, Color(0.45, 1.0, 0.68, 0.6), 66.0)                           # 灵巧翻滚绿环(25%闪避激活提示)
...
func _sk_hunter_shot(u: Dictionary, tgt) -> void:              # 猎人龟·精准射击(封板·90龟能·射箭+毒箭+猎杀印记三合一): 蓄力狙2.0A物理+中毒5s+治疗削减50%5s+猎杀印记5s(<24%处决)
	...
	_pending_shots.append({"delay": 0.4, "fn": func() -> void:
		...
		_pending_shots.append({"delay": 0.16, "fn": func() -> void:
			...
			_apply_damage_from(uu, tref, _atk_dmg(uu, 2.0, tref), Color("#a8ffb0"))
			...
			_gambler_pop(tref["pos"], float(tref.get("height", 0.0)) + 0.4, Color(0.55, 1.0, 0.4, 0.9))   # 命中毒绿爆
			_shake(JUICE_SHAKE_HEAVY)
			, "src": uu})
		, "src": u})
