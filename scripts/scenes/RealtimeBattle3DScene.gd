...
const REVIEW_DEMO_CFG := {
	...
	"bubble:3": [ {"dx": 140.0, "dy": -55.0}, {"dx": 300.0, "dy": 40.0, "fixed": true} ],   # 泡泡爆破: 近假人打泡泡龟攒泡泡值+远目标→看目标两侧开泡沫门+门间泡沫涌+消耗泡泡值魔法
...
func _sk_bubble_burst(u: Dictionary, tgt) -> void:              # 泡泡龟·泡泡爆破(马尔扎哈Q式·用户设计): 消耗当前泡泡值40%→目标两侧泡沫门·门间敌每个受=消耗泡泡值魔法+0.8A物理(无沉默)
	...
	_bubble_gate(c + perp)                                      # 目标两侧开泡沫门(马尔扎哈Q式·用户设计)
	_bubble_gate(c - perp)
	for o in _enemies_of(u):
		if o.get("alive", false) and o["pos"].distance_to(c) <= 200.0:   # 门间200码带
			_apply_damage_from(u, o, int(_mitigate(u, consumed, o, true)) + _atk_dmg(u, 0.8, o), Color("#cdebff"))   # 消耗泡泡值魔法(吃魔抗)+0.8A物理(封板L437)
	for k in range(10):                                         # 门间泡沫涌
		_bubble_rise(c + Vector2(cos(TAU * float(k) / 10.0), sin(TAU * float(k) / 10.0)) * randf_range(20.0, 185.0))
...
func _bubble_gate(pos2d: Vector2) -> void:   # 泡沫门(bubble-burst立绘·涨开脉动淡出)
	...
	g.pixel_size = (150.0 * WS) / float(maxi(1, tex.get_height()))
	g.position = _world_pos(pos2d, 1.1)
