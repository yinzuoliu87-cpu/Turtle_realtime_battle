...
func _sk_rainbow_reflect(u: Dictionary) -> void:               # 彩虹龟·反射弹射 (重做2026-07-13: 棱镜光束敌我间反射弹跳·治友绿/伤敌全色魔法)
	...
	for i in range(6):
		...
		var tw := _reg_tween()
		tw.tween_interval(float(i) * 0.09)      # 逐段错峰=看得见弹射依次跳
		tw.tween_callback(func() -> void:
			_beam_vfx("res://assets/sprites/vfx/fx-energy-beam.png", from_p, tp, 30.0, Color(_col.r, _col.g, _col.b, 0.9), 0.3, 0.9)   # 棱镜反射光束(当跳色)
			if _ally:
...
func _reflect_pop(pos2d: Vector2, h: float, col: Color) -> void:
	...
	var tw := _reg_tween(); tw.set_parallel(true)
	tw.tween_property(g, "scale", Vector3.ONE * 1.6, 0.28)
	tw.tween_property(g, "material_override:albedo_color", Color(col.r, col.g, col.b, 0.0), 0.28)
	tw.chain().tween_callback(g.queue_free)
