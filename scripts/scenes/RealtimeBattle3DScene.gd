...
func _step_homing_arrow(pr: Dictionary, node: Sprite3D, delta: float) -> bool:   # 追踪抛物箭逐帧: 追踪移动+抛物高度+箭头随角度; 命中/目标消失→自销返false, 否则返true续飞
	...
	if dist <= 30.0:   # 命中: 白色真伤(处决由中央路径判·egg免疫)
		node.queue_free()
		_apply_damage_from(pr["src"], tgt, int(pr["dmg"]), pr["col"], 0.0, true)
		_flash(tgt); _hit_spark(tgt)
		return false
	cur2d = cur2d.move_toward(tp, HUNTER_ARROW_SPD * delta)   # 慢速追踪(向当前目标位置移动=追踪)
	pr["pos2d"] = cur2d
