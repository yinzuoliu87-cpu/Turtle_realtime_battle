...
const PASSIVE_SKILL_TYPES := {"iceBurnImmune": true, "shellEnhanceAwaken": true}   # phoenixEnhancedRebirth/lavaEnhancedRage 已废弃(lava技三改lavaErupt主动)
...
func _sk_basic_strike(u: Dictionary, _tgt = null) -> void:      # 小龟·打击(封板·10波序列驱动·80龟能): 全程定身·10波每0.15s·每波随机挑1存活敌当方向·气波可被挡(命中路径第一敌/蛋)·每波0.4A(吃不屈)·[慢飞弹道视觉留F5]
	...
	for i in range(10):
		var fn := func():
			...
			if hit != null:
				_apply_damage_from(u, hit, _atk_dmg(u, 0.4, hit), Color("#ff4444"))
			_bolt_line(u["pos"], u["pos"] + dir * 420.0, Color(1.0, 0.4, 0.4))   # 气波占位(慢飞弹道美术F5)
...
func _sk_basic_chiwave(u: Dictionary, tgt) -> void:            # 小龟·龟派气波(封板·100龟能): 先自增buff(暴击25%/暴伤20%/吸血10%/护穿0.1A·3秒)→朝当前目标发穿透气波(带宽80·打沿途所有敌+蛋)每命中3.5A物理+击飞1.5s+击退200 [智能位移留F5]
	...
	dir = dir.normalized()
	for o in _enemies_of(u):                                    # 穿透气波: 打沿途所有敌+蛋(带宽80)
		if not o.get("alive", false): continue
		if not _on_line(u["pos"], dir, o["pos"], 80.0): continue
		_apply_damage_from(u, o, _atk_dmg(u, 3.5, o), Color("#7fd0ff"))
		_knockback(u, o, 200.0, 1.8, 2.0)                       # 击飞1.5s+击退200
	_bolt_line(u["pos"], u["pos"] + dir * 900.0, Color(0.5, 0.85, 1.0))   # 青白气功波占位(重做美术F5)

func _sk_basic_slam(u: Dictionary, tgt) -> void:              # 小龟·过肩摔(封板·120龟能·Sett R式): 抓主目标→挑空+摔到龟背后60码+眩晕0.5s·主0.7A+26%目标maxHp·周围250码0.2A+19%主maxHp物理; 蛋免控只吃原地伤
	if tgt == null: tgt = _nearest_enemy(u)
	if tgt == null: return
	var tmax: float = float(tgt["maxHp"])
	if not tgt.get("_eggImmune", false):                        # 蛋免疫控制(不被挑空/摔·只吃原地伤)
		var dir: Vector2 = tgt["pos"] - u["pos"]
		if dir.length() < 1.0: dir = Vector2.RIGHT
		dir = dir.normalized()
		var back: Vector2 = u["pos"] - dir * 60.0               # 落点=龟背后~60码
		back.x = clampf(back.x, ARENA.position.x, ARENA.end.x)
		back.y = clampf(back.y, ARENA.position.y, ARENA.end.y)
		tgt["pos"] = back
		tgt["stun_until"] = maxf(float(tgt.get("stun_until", 0.0)), _t + _cc_dur(tgt, 0.5))   # 砸地眩晕0.5s
		_knockback(u, tgt, 20.0, 1.4, 0.4)                      # 挑空(视觉)
	_apply_damage_from(u, tgt, _atk_dmg(u, 0.7, tgt) + int(tmax * 0.26), Color("#ff9d5c"))   # 主目标
	for o in _enemies_of(u):                                    # 周围250码其余敌
...
func _sk_rock_shockwave(u: Dictionary) -> void:                  # 石头龟·岩石之躯 主动(用户设计): 横排带敌(0.5DEF+0.5MR)×(1+4%岩层)物理 + 1%×层概率眩晕1.5s + 击飞
	...
	for o in _enemies_of(u):
		...
		_apply_damage_from(u, o, int((u["def"] * 0.5 + u["mr"] * 0.5) * (1.0 + 0.04 * layers)), Color("#c8a878"))
		if randf() < 0.01 * layers:
			o["stun_until"] = maxf(float(o.get("stun_until", 0.0)), _t + _cc_dur(o, 1.5))
		_knockback(u, o, 60.0)
	_skill_ring(u["pos"], Color(0.79, 0.64, 0.42, 0.5), 60.0)   # 岩脊推进VFX占位
...
func _sk_stone_taunt(u: Dictionary) -> void:                    # 石头龟·嘲讽(用户设计·120龟能): 500码敌4秒硬嘲讽 + 自身1A永久盾 + 0.5×护甲减伤4秒 + 将结束砸地(400码1A魔法+击飞2s)
	...
	u["stone_dr_until"] = _t + 4.0            # 0.5×护甲%减伤4秒(减伤钩TODO)
	_skill_ring(u["pos"], Color(0.79, 0.64, 0.42, 0.55), 90.0)   # 500码仇恨光环VFX占位
	var uu := u
...
func _sk_bamboo_heal(u: Dictionary) -> void:                     # 竹叶龟·自然恢复 ✅
	...
	if allies.is_empty():
		...
		for o in allies:
			_grant_shield(o, o["maxHp"] * 0.12, 4.0)   # 寒冰团队护盾(通用护盾4秒·封板)
...
func _ninja_dash(u: Dictionary, target: Dictionary) -> void:    # 被动·冲击(亚索E式自动冲刺斩·封板): 穿过目标落身后450码·路径敌1.2A物理+击飞0.8s·每敌10s冷却·无伤害递增
	u["_ninja_last_dash"] = _t
	var start: Vector2 = u["pos"]
	var dir: Vector2 = target["pos"] - start
	if dir.length() < 1.0: dir = Vector2.RIGHT
	dir = dir.normalized()
	_dash_to(u, target, -80.0)                                  # 落到目标身后~80码(负gap=穿过)
	for o in _enemies_of(u):
		if not o.get("alive", false): continue
		if not _on_line(start, dir, o["pos"], 62.0): continue
		if o["pos"].distance_to(start) > 470.0: continue        # 450码穿透范围
...
func _sk_ninja_backstab(u: Dictionary, tgt: Dictionary) -> void: # 技二·背刺(封板): +5穿甲5秒→闪现到最远敌(后排C)身后→连刺3段共2.0A物理→留原地追砍
	...
	_bolt_line(u["pos"], far["pos"], Color(0.8, 0.9, 1.0))       # 背后连刺刀光占位(烟遁×3刀美术F5)

func _sk_ninja_shuriken(u: Dictionary, tgt) -> void:           # 技三·手里剑(封板): 当前目标飞镖1.6A物理·暴击(40%+2%/级)时整发转真实伤害; 忍者足打包被动在登场gate
	if tgt == null: tgt = _nearest_enemy(u)
	if tgt == null: return
	var cc: float = 0.40 + 0.02 * float(u.get("level", 1))
	if randf() < cc:
		_apply_damage_from(u, tgt, int(u["atk"] * 1.6), Color("#ffe066"), 0.0, true)   # 暴击→真伤(穿减伤)
	else:
		_apply_damage_from(u, tgt, _atk_dmg(u, 1.6, tgt), Color("#cfd8e8"))
...
func _sk_ghost_phase(u: Dictionary, tgt: Dictionary) -> void:    # 幽灵龟·虚化 (用户2026-07-08): 虚化4秒受物理伤害-90% + 对目标2段共1.2A真伤
	...
	_skill_ring(u["pos"], Color(0.78, 0.49, 1.0, 0.55), 50.0)   # 虚化紫环(VFX占位, 美术阶段重做)
...
func _sk_diamond_powerball(u: Dictionary, tgt) -> void:          # 钻石龟·钻石滚球(封板·龙龟Q Powerball·100龟能): [简化=dash近似·完整"0→满速4s加速+按速插值伤害+免疫打断+免费滚被动"位移状态机待F5] 蜷球冲最近敌·撞击120码AOE(护甲/魔抗/2%maxHp满速插值)+击飞1s+眩晕
	if tgt == null: tgt = _nearest_enemy(u)
	if tgt == null: return
	var sf: float = 1.0    # 简化: 视为满速(完整版0速起步4秒线性到满速)
	var cp: Vector2 = tgt["pos"]
	_dash_to(u, tgt, 40.0)                                       # 蜷球滚向目标
	for o in _enemies_of(u):
		if not o.get("alive", false): continue
		if o["pos"].distance_to(cp) > 120.0: continue            # 撞击点120码小AOE
		var dmg: int = int(u["def"] * (0.1 + 0.9 * sf) + u["mr"] * (0.1 + 0.9 * sf) + o["maxHp"] * (0.02 + 0.18 * sf))
		_apply_damage_from(u, o, dmg, Color("#9bdcff"))
		_knockback(u, o, 50.0, 1.8, 1.0)                        # 击飞1秒(vy高)
		_freeze(o, 0.5 + 2.5 * sf)                              # 眩晕0.5~3s(随移速插值·满速3s)
	_skill_ring(cp, Color(0.6, 0.86, 1.0, 0.5), 120.0)
...
func _sk_hunter_hide(u: Dictionary) -> void:                     # 猎人龟·隐蔽(封板·薇恩Q Tumble·80龟能): 智能翻滚~250码→下次普攻附带0.9A物理(吃吸血)+25%闪避+0.7A护盾
	...
	_bolt_line(u["pos"], dest, Color(0.7, 1.0, 0.75))           # 灵巧侧翻残影占位(美术F5)
	u["pos"] = dest
...
func _sk_hunter_barrage(u: Dictionary, _tgt) -> void:          # 猎人龟·狩猎弹幕(封板·100龟能): 10箭优先锁残血·每箭0.3A真实(共3A)·每箭<14%即处决(可被挡弹道待小龟系统·当前直击近似)
	for i in range(10):
		var pick = null
		var lo := INF
		for o in _enemies_of(u):
			if o.get("alive", false) and float(o["hp"]) < lo: lo = float(o["hp"]); pick = o
		if pick == null: break
		_apply_damage_from(u, pick, int(u["atk"] * 0.3), Color("#a8ffb0"), 0.0, true)   # 0.3A真实(处决由中央路径判)
...
func _sk_bubble_shield(u: Dictionary, _tgt: Dictionary) -> void: # 泡泡龟·泡泡盾 ✅(简化:无延迟爆裂)
	var ally = _lowest_hp_ally(u)
	if ally == null: ally = u
	_grant_shield(ally, u["atk"] * 1.8)   # 泡泡盾: 暂保持(封板4秒但"到期爆裂对全体敌2.0A"需接timed-expiry-burst→留泡泡专项回补·防静默过期丢爆裂)
	for o in _enemies_of(u):
...
func _sk_phoenix_haste(u: Dictionary) -> void:                   # 凤凰龟·技三主动 (用户2026-07-07: 自身+50%攻速+50%移速4秒·配合喷火随攻速增伤; 强化涅槃被动在spawn施加)
	...
	_skill_ring(u["pos"], Color(1.0, 0.55, 0.15, 0.6), 56.0)   # 烈焰加速VFX占位(美术: 展火凤翼+加速残影拖火尾)

func _sk_angel_ascend(u: Dictionary) -> void:                   # 天使龟·飞升 (用户2026-07-06: +20%攻速+10%移速+25码射程·到战斗结束·可叠加无上限)
...
func _chest_basic(u: Dictionary, tgt: Dictionary) -> void:       # 普攻·宝箱砸击(封板): K'Sante一段Q式·朝目标前方短直线AOE·各1A物理(近战扫一小片非单体)
	...
	_bolt_line(u["pos"], u["pos"] + dir * reach, Color(1.0, 0.85, 0.25))            # 短直线冲击占位
	_skill_ring(u["pos"] + dir * (reach * 0.55), Color(1.0, 0.85, 0.2, 0.45), 46.0) # 砸点金光占位
...
func _sk_chest_cannon(u: Dictionary, tgt) -> void:              # 技三·财宝炮击(封板·120龟能): 蓄力→朝一条直线发长激光→线上所有敌各3A物理+击飞+击退 (贪婪打包被动在登场gate)
	...
	_bolt_line(u["pos"], u["pos"] + dir * 1300.0, Color(1.0, 0.9, 0.3))   # 长激光能量射线占位
...
func _sk_cyber_cannon(u: Dictionary, tgt) -> void:              # 赛博龟·能量大炮(封板): 跃到目标贴身→1A物理 + 0.1A×浮游炮数 真实(炮越多真伤越猛)
	if tgt == null: tgt = _nearest_enemy(u)
	if tgt == null: return
	_dash_to(u, tgt, 80.0)
	...
	if drones > 0:
		_apply_damage_from(u, tgt, int(u["atk"] * 0.1 * drones), Color("#d0ffff"), 0.0, true)   # 0.1A×炮数真伤
	_skill_ring(tgt["pos"], Color(0.6, 0.94, 1.0, 0.5), 46.0)
...
func _sk_hiding_shrink(u: Dictionary) -> void:                  # 缩头(封板·100龟能): 立即给随从+50%技能龟能 + 自身缩头3秒(80%减伤·不能攻击/移动) [龟能锁定nuance留F5]
...
func _sk_shell_shadow_dive(u: Dictionary, tgt) -> void:        # 龟壳·暗影俯冲(封板·130龟能·Corki库奇式): 俯冲600码→落地2.5A魔法+击退+路径敌→暗影燃烧区150码5s(每0.5s 0.1A灼烧层+减速20%)→进入隐身
	...
	for i in range(10):                                         # 暗影燃烧区150码·5秒·每0.5秒结算
		...
		_pending_shots.append({"delay": float(i) * 0.5, "fn": fn, "src": u})
	_bolt_line(start, dest, Color(0.6, 0.2, 0.7))              # 暗影猛扑拖影占位
...
func _sk_cyber_hijack(u: Dictionary) -> void:                   # 赛博龟·侵入(封板·120龟能·Botworld黑客): 黑1随机敌4秒倒戈(side改赛博方·标hijacked→打原队友+被原队友打·不算存活数·击杀归赛博·蛋免控·可黑多个)
	...
	_bolt_line(u["pos"], v["pos"], Color(0.25, 1.0, 0.82))      # 数据链占位(赛博蓝绿缠绕美术F5)
...
func _sk_cyber_smart(u: Dictionary) -> void:                   # 赛博龟·智能AI(封板·40龟能·充能型): +1充能→冲刺重定位(kite拉到理想射程·躲贴身); 常驻走位+登场+20%移速 [完整行动脑(躲大招/对齐激光)留F5]
	...
	if ne != null:
		...
		dest.x = clampf(dest.x, ARENA.position.x, ARENA.end.x)
		dest.y = clampf(dest.y, ARENA.position.y, ARENA.end.y)
		_bolt_line(u["pos"], dest, Color(0.3, 0.9, 1.0))        # 赛博残影瞬移占位
		u["pos"] = dest
...
func _sk_shell_copy(u: Dictionary, tgt) -> void:               # 龟壳·复制(封板·130龟能): 复制2敌方可用技(_COPYABLE白名单)·轮流依次释放(不同帧糊); 60%效果=待伤害输出乘数钩子(F5·当前全效)
	var pool: Array = []
	for o in _enemies_of(u):
...
func _tick_periodic_passive(u: Dictionary, delta: float) -> void:
	...
	# --- 忍者·冲击(亚索E式被动auto-dash): 400码内有"可冲"敌(不在其10s冷却)且距上次冲刺≥0.4s → 自动朝最近敌冲刺斩 ---
...
func _tick_summon_special(u: Dictionary, delta: float) -> void:
	...
	match special:
		...
		"ray":
			...
			if t["alive"]:
				var cv := _add_stack(t, "crystal", 1, 5)
				if cv >= 5:
