...
func _lava_volcano_erupt(u: Dictionary) -> void:                 # 火山·火山爆发(用户2026-07-15完整重设计·照娜美R官方): 起手聚力→红矩形预警→圆角熔岩浪丘【从自身推出】沿最密方向0.9s推进1200码(原从场外涌向自己=怪已反转)→碾过每敌击飞+5段0.5A魔+灼烧+回血12%→到端塌落
	var dir: Vector2 = _densest_enemy_point(u, 400.0) - u["pos"]
	dir = dir.normalized() if dir.length() > 0.1 else Vector2.RIGHT
	var full_len := 1200.0                                         # 行进全长(从龟前方出发向前推1200码)
	var half_w := 140.0                                            # 浪半宽(全宽280码·巨浪感·原160太窄)
	var origin: Vector2 = u["pos"]
	var start: Vector2 = origin + dir * 40.0                       # 浪从自己脚前出发(娜美R式)
	...
	tel.position = _world_pos(start + dir * (full_len * 0.5), 0.03)   # 预警=从龟前方向前1200码的走廊(浪的路径)
	...
	var step := func(d: float) -> void:
		var front: Vector2 = start + dir * d                      # 当前波前位置 (从自己脚前向前推进·娜美R式)
		...
		if is_instance_valid(wall):
			...
			if _juice_rng.randf() < 0.55:                         # 浪顶洒火星熔滴(行进点缀)
				...
				_world.add_child(sp)
				var st := _reg_tween(); st.set_parallel(true)
				st.tween_property(sp, "position", _world_pos(sx + dir * _juice_rng.randf_range(20.0, 60.0), 0.1), 0.4).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
			...
			if _juice_rng.randf() < 0.6:                          # ★雾云尾迹(娜美R观察: 不规则软雾云包四周+尾部拖白雾)
				var mpos: Vector2 = front + perp_axis * _juice_rng.randf_range(-half_w * 1.1, half_w * 1.1) - dir * _juice_rng.randf_range(0.0, 180.0)
				var mf := Sprite3D.new()
				...
				mt.tween_property(mf, "position", _world_pos(mpos - dir * 40.0 + perp_axis * _juice_rng.randf_range(-30.0, 30.0), wall_h * 1.1), 0.7)
	...
	_shake(JUICE_SHAKE_BIG)
	var travel := 2.5                                             # 1200码/2.5s=480码/s(照娜美R比例·每秒约7个身位; 原0.9s快4倍看不清)
	var wt := _reg_tween()
