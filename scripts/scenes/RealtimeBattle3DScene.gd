...
const SPIKE_WAVE_TIME := 0.5      # 波头跑完700码用时(照小菊R第三击: 短程~0.3s跑完→换算1400码/s·参考帧C:/tmp/ivern_wave)
func _crystal_spike_line(u: Dictionary) -> void:   # 照小菊R第三击(2026-07-16逐帧): 砸地爆发→发光波头贴地冲出→波头过处水晶刺依次从地底弹出→碎晶地痕渐隐; 命中+2结晶+击飞0.8s+50%减速3s(纯控制无伤害)
	...
	hw.tween_property(head, "modulate:a", 0.0, 0.12)
	hw.tween_callback(head.queue_free)
	for i in range(segs):
		...
		_pending_shots.append({"delay": SPIKE_WAVE_TIME * float(i + 1) / float(segs), "fn": func() -> void:
			...
			if scar_tex != null:                                      # 碎晶地痕(小菊翻土痕→冰蓝碎晶·贴地1.1s渐隐)
				for _sc in range(2):
					...
					var sct := _reg_tween()
					sct.tween_property(scar, "modulate:a", 0.0, 1.1)
