...
func _lava_volcano_erupt(u: Dictionary) -> void:                 # 火山·火山爆发(用户2026-07-15完整重设计·照娜美R官方): 起手聚力→红矩形预警→圆角熔岩浪丘【从自身推出】沿最密方向0.9s推进1200码(原从场外涌向自己=怪已反转)→碾过每敌击飞+5段0.5A魔+灼烧+回血12%→到端塌落
	...
	wave.emitting = true
	var wall := MeshInstance3D.new()                              # ★浪墙主体: 竖立shader墙(浪身上卷+浪尖白亮翻滚)随波前推进
	var wqm := QuadMesh.new()
	wqm.size = Vector2(1.0, 1.0)
	wall.mesh = wqm
	var wsm := ShaderMaterial.new()
	wsm.shader = load("res://assets/shaders/lava_wave.gdshader")
	wall.material_override = wsm
	wall.cast_shadow = GeometryInstance3D.SHADOW_CASTING_SETTING_OFF
	var wall_w: float = (half_w * 2.0 + 50.0) * WS                # 浪丘宽=浪宽(+50px溢出软边)
	var wall_h: float = wall_w * 0.4                              # 浪高≈宽×0.4(娜美R浪丘比例·圆拱)
	wall.scale = Vector3(wall_w, wall_h, 1.0)
	wall.position = _world_pos(start, wall_h * 0.5)               # 底边贴地·从自己脚前出发
	...
	var step := func(d: float) -> void:
		...
		if is_instance_valid(wall):
			wall.position = _world_pos(front, wall_h * 0.5)       # 浪丘随波前推进(底边贴地)
	...
	if is_instance_valid(tel):
		tel.queue_free()
	if is_instance_valid(wall):
		var wf := _reg_tween()                                    # 浪墙到端: 快速塌落淡出(浪撞岸散掉)
		wf.set_parallel(true)
		wf.tween_property(wall, "scale:y", 0.25, 0.22)
		wf.tween_property(wall, "position:y", 0.2, 0.22)
