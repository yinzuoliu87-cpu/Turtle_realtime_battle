...
func _build_environment() -> void:
	...
	if OS.has_environment("BLACKMAP"):   # 临时黑地图(验证VFX·用户2026-07-12, 弄完删env即可)
...
func _tick_unit(u: Dictionary, delta: float) -> void:
	...
	_update_hunt_mark(u)   # 猎杀印记: 被标记的敌头顶贴十字准星图标(持续随buff·用户2026-07-14)
	_update_stack_badge(u, "ink", "res://assets/sprites/passive/ink-mark-icon.png", Color(0.75, 0.72, 0.85), 0)   # 墨迹层数头顶徽章(用户2026-07-15)
	_update_stack_badge(u, "electric", "res://assets/sprites/passive/lightning-storm-icon.png", Color(1.0, 0.92, 0.4), 1)   # 电击层数头顶徽章
...
func _make_stack_badge(icon_path: String, tint: Color) -> Control:   # 头顶层数徽章(图标+数字·墨迹/电击层·用户2026-07-15"只头顶小图标+数字")
	var root := Control.new()
	root.mouse_filter = Control.MOUSE_FILTER_IGNORE
	var icon := TextureRect.new()
...
func _update_stack_badge(u: Dictionary, key: String, icon_path: String, tint: Color, slot: int) -> void:   # 每帧: 有层数则头顶显图标+数字(2D屏幕空间unproject·根治透视), 无则清
	var n := int((u.get("stacks", {}) as Dictionary).get(key, 0))
	var mk := "_badge_" + key
	var badge = u.get(mk, null)
	var valid: bool = badge != null and is_instance_valid(badge)
	if n > 0 and u.get("alive", false):
		if not valid:
			badge = _make_stack_badge(icon_path, tint)
			if _ui_layer != null: _ui_layer.add_child(badge)
			u[mk] = badge
		(badge.get_meta("lbl") as Label).text = str(n)
		if _cam != null:
			...
			if _cam.is_position_behind(head):
				...
				badge.visible = true
				badge.position = _cam.unproject_position(head) + Vector2(-16.0 + float(slot) * 36.0, -72.0)
	elif valid:
		u[mk] = null
		badge.queue_free()

# 猎杀印记: 目标身上贴十字准星图标(跟随·脉动·持续随hunt_mark_until buff·不用文字·用户2026-07-14)·仿_update_ninja_marks
func _update_hunt_mark(u: Dictionary) -> void:
...
func _sk_hunter_shot(u: Dictionary, tgt) -> void:              # 猎人龟·精准射击(封板·90龟能·射箭+毒箭+猎杀印记三合一): 蓄力狙2.0A物理+中毒5s+治疗削减50%5s+猎杀印记5s(<24%处决)
	...
	_pending_shots.append({"delay": 0.4, "fn": func() -> void:
		...
		_pending_shots.append({"delay": 0.16, "fn": func() -> void:
			...
			tref["hunt_mark_until"] = _t + 5.0                           # 猎杀印记5秒: <24%即处决(头顶贴十字准星图标·_update_hunt_mark·不用文字)
