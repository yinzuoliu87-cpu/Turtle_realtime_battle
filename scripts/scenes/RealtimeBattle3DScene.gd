...
func _crystal_spike_line(u: Dictionary) -> void:   # 照小菊R第三击(2026-07-16逐帧): 砸地爆发→发光波头贴地冲出→波头过处水晶刺依次从地底弹出→碎晶地痕渐隐; 命中+2结晶+击飞0.8s+50%减速3s(纯控制无伤害)
	...
	head.pixel_size = (52.0 * WS) / float(maxi(1, glow0.get_width()))
	head.position = _world_pos(origin + dirv * 30.0, 0.3)
	_world.add_child(head)
	var hw := _reg_tween()
	hw.tween_property(head, "position", _world_pos(wend, 0.3), SPIKE_WAVE_TIME)
	...
	if htex != null:                                             # 3片碎晶伴飞(小菊: 波头是实心翻涌团非软光·用碎晶簇加密度)
		...
		for r in range(3):
			...
			rid.position = _world_pos(origin + dirv * 30.0 + roff, randf_range(0.25, 0.5))
			rid.rotation.z = randf() * TAU
			rid.modulate = Color(0.85, 0.97, 1.0, 0.95)
			_world.add_child(rid)
			var rt := _reg_tween(); rt.set_parallel(true)
			rt.tween_property(rid, "position", _world_pos(wend + roff, randf_range(0.25, 0.5)), SPIKE_WAVE_TIME)
	...
	for i in range(segs):
		...
		_pending_shots.append({"delay": SPIKE_WAVE_TIME * float(i + 1) / float(segs), "fn": func() -> void:
			...
			if tex2 != null:                                          # 尖刺从地底刺上来(低位小→满高BACK弹出→定格→碎)
				...
				st.chain().tween_property(spk, "modulate:a", 0.0, 0.2)
