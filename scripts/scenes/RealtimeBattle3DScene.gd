...
func _chest_basic(u: Dictionary, tgt: Dictionary) -> void:       # 普攻·宝箱砸击(封板): K'Sante一段Q式·朝目标前方短直线AOE·各1A物理(近战扫一小片非单体)
	...
	for o in _enemies_of(u):
		...
		_burst_vfx("res://assets/sprites/vfx/treasure-slam.png", o["pos"], 130.0)   # 命中者金色放射爆闪(K'Sante s21·最抢眼层)
	# VFX照K'Sante一段Q逐帧(2026-07-16 C:/tmp/ksq_slam): 砸点爆闪+尘环→冲击带瞬铺→地面燃金痕+旋纹圈~0.5s渐隐
	_shake(0.05)
	_skill_ring(u["pos"] + dir * 42.0, Color(1.0, 0.85, 0.3, 0.7), 30.0)                                                                  # 砸点尘环(s16锤落点)
	_burst_vfx("res://assets/sprites/vfx/treasure-slam.png", u["pos"] + dir * 46.0, 120.0)                                                # 砸点金光爆
	_beam_vfx("res://assets/sprites/vfx/fx-energy-beam.png", u["pos"], u["pos"] + dir * reach, 96.0, Color(1.0, 0.85, 0.25, 0.8), 0.14)   # 冲击带瞬铺(一段Q=瞬时判定·短亮)
	_beam_vfx("res://assets/sprites/vfx/fx-energy-beam.png", u["pos"], u["pos"] + dir * reach, 58.0, Color(1.0, 0.6, 0.15, 0.5), 0.5)     # 地面燃金痕(窄·渐隐s26)
	for r in range(3):                                                                                                                     # 沿线旋纹圈(s26)
		_skill_ring(u["pos"] + dir * (reach * (0.3 + 0.28 * float(r))), Color(1.0, 0.8, 0.35, 0.4), 26.0)
