...
func _step_projectiles(delta: float) -> void:
	...
	for pr in _projectiles:
		...
		if pr.get("spin", false):                                  # 手里剑: 飞行中旋转
			node.rotation.y += 26.0 * delta
		if frac >= 1.0:
			node.queue_free()
			if tgt["alive"]:
...
func _fire_ice_shard(src: Dictionary, tgt: Dictionary, dmg: int) -> void:   # 冰锥弹道(水平朝目标, 命中魔伤+冻结1.5s)
	...
	_projectiles.append({
		...
		"src": src, "t": 0.0, "dur": dur, "basic_onhit": false, "freeze_on_hit": 1.5, "oriented": true,
	})
...
func _sk_ninja_shuriken(u: Dictionary, tgt) -> void:           # 技·手里剑(封板·远程): 朝当前目标掷旋转飞镖·命中1.6A物理·暴击(40%+2%/级)整发转真实伤害(用户2026-07-11: 补飞镖弹道·这是远程技)
	if tgt == null: tgt = _nearest_enemy(u)
	if tgt == null: return
	var cc: float = 0.40 + 0.02 * float(u.get("level", 1))
	var is_true: bool = randf() < cc
	var dmg: int = (int(u["atk"] * 1.6) if is_true else _atk_dmg(u, 1.6, tgt))   # 暴击=1.6A整发真伤(raw)·否则1.6A物理(预减)
	_fire_shuriken(u, tgt, dmg, is_true)

func _fire_shuriken(src: Dictionary, tgt: Dictionary, dmg: int, is_true: bool) -> void:   # 旋转飞镖弹道: 飞向目标·命中结算(is_true→raw真伤·金色)
	var start2d: Vector2 = src["pos"]
	var p := Sprite3D.new()
	p.texture = _make_star_texture()
	p.texture_filter = BaseMaterial3D.TEXTURE_FILTER_LINEAR
	p.billboard = BaseMaterial3D.BILLBOARD_DISABLED
	p.axis = Vector3.AXIS_Y                                     # 贴平面·绕Y自旋(飞镖旋转·见 _step_projectiles spin)
	p.shaded = false; p.transparent = true
	p.modulate = (Color(1.0, 0.88, 0.4, 0.95) if is_true else Color(0.86, 0.9, 0.96, 0.95))   # 暴击金 / 钢灰
	var _stw := float(maxi(1, int(p.texture.get_width())))
	p.pixel_size = (46.0 * WS) / _stw
	var world_from := _world_pos(start2d, 1.0)
	p.position = world_from
	_world.add_child(p)
	var dur := clampf(start2d.distance_to(tgt["pos"]) / 850.0, 0.12, 0.5)   # 快镖
	_projectiles.append({
		"node": p, "from": world_from, "tgt": tgt, "dmg": dmg,
		"col": (Color("#ffe066") if is_true else Color("#cfd8e8")),
		"src": src, "t": 0.0, "dur": dur, "basic_onhit": false, "raw": is_true, "spin": true,
	})
