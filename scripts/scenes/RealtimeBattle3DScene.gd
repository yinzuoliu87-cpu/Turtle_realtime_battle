...
func _sk_basic_strike(u: Dictionary, _tgt = null) -> void:      # 小龟·打击(封板·10波序列驱动·80龟能): 全程定身·10波每0.15s·每波随机挑1存活敌当方向·气波可被挡(命中路径第一敌/蛋)·每波0.4A(吃不屈)·[慢飞弹道视觉留F5]
	...
	for i in range(10):
		var fn := func():
			...
			_fly_vfx("res://assets/sprites/vfx/qibo-ball.png", u["pos"], u["pos"] + dir * 420.0, 52.0, 1.1, 1.0)   # 打击气波弹(慢飞·用户2026-07-05"速度要慢"·复用气波球)
...
func _sk_basic_slam(u: Dictionary, tgt) -> void:              # 小龟·过肩摔(封板·120龟能·Sett R式): 抓主目标→挑空+摔到龟背后60码+眩晕0.5s·主0.7A+26%目标maxHp·周围250码0.2A+19%主maxHp物理; 蛋免控只吃原地伤
	...
	_burst_vfx("res://assets/sprites/vfx/dust-impact.png", tgt["pos"], 190.0, 0.35)   # 过肩摔落地尘爆(用户07-05"二人落地")
	_burst_vfx("res://assets/sprites/vfx/fx-shock-ring.png", tgt["pos"], 250.0, 0.06)   # 落地冲击环
...
func _chest_basic(u: Dictionary, tgt: Dictionary) -> void:       # 普攻·宝箱砸击(封板): K'Sante一段Q式·朝目标前方短直线AOE·各1A物理(近战扫一小片非单体)
	...
	_beam_vfx("res://assets/sprites/vfx/fx-energy-beam.png", u["pos"], u["pos"] + dir * reach, 64.0, Color(1.0, 0.85, 0.25, 0.7), 0.26)   # 短直线冲击(用户#3"参考lol ksante一段Q")
...
func _sk_chest_cannon(u: Dictionary, tgt) -> void:              # 技三·财宝炮击(封板·120龟能): 蓄力→朝一条直线发长激光→线上所有敌各3A物理+击飞+击退 (贪婪打包被动在登场gate)
	...
	_beam_vfx("res://assets/sprites/vfx/fx-energy-beam.png", u["pos"], u["pos"] + dir * 1300.0, 88.0, Color(1.0, 0.9, 0.3, 0.85), 0.55)   # 财宝炮击长激光(用户07-07"对一条直线·类似手枪长激光")
...
func _sk_cyber_cannon(u: Dictionary, tgt) -> void:              # 赛博龟·能量大炮(用户#15: 对一条直线蓄力后发长激光能量射线·线上敌各1A物理+0.1A×浮游炮数真伤·炮越多越猛)
	...
	var fire := func() -> void:                                 # 蓄力后·直线长激光能量射线(用户#15·非贴身跃击)
		...
		_beam_vfx("res://assets/sprites/vfx/fx-energy-beam.png", uu["pos"], uu["pos"] + dir * 900.0, 84.0, Color(0.6, 0.94, 1.0, 0.9), 0.5)   # 能量大炮长激光(用户07-07"对一条直线蓄力后发出长的激光似的能量射线")
