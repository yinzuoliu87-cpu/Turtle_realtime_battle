...
const PANEL_COUNT := {"p2eq_034": "bear_layers"}   # 头像下装备格右下角层数徽章: id → 层数字段
...
func _blood_slash(from2d: Vector2, to2d: Vector2, delay: float) -> void:   # 饮血连斩: 血红斩弧落敌身(错峰delay=快速飞舞乱斩), 纯视觉
	var off := Vector2(randf_range(-14.0, 14.0), randf_range(-12.0, 12.0))
	var arc := Sprite3D.new()
	arc.texture = _make_slash_texture(Color("#ff2d55"))
	arc.billboard = BaseMaterial3D.BILLBOARD_ENABLED; arc.shaded = false; arc.transparent = true
	arc.pixel_size = 0.045
	arc.flip_h = (randf() < 0.5)
	arc.position = _world_pos(to2d + off, 1.0)
	arc.modulate = Color(1.0, 0.22, 0.34, 0.0)
	arc.scale = Vector3(0.4, 0.4, 0.4)
	_world.add_child(arc)
	var tw := create_tween()
	if delay > 0.0: tw.tween_interval(delay)
	tw.set_parallel(true)
	tw.tween_property(arc, "modulate:a", 0.95, 0.05)
	tw.tween_property(arc, "scale", Vector3(1.15, 1.15, 1.15), 0.13)
	tw.chain().tween_property(arc, "modulate:a", 0.0, 0.12)
	tw.chain().tween_callback(arc.queue_free)
...
func _bear_shockwave(u: Dictionary, tgt: Dictionary, _si: int) -> void:   # 大熊冲击波(小菊式): 蓄力→直线移动波, 1.5ATK物理+击飞0.8s+拉回70码
	...
	wave.billboard = BaseMaterial3D.BILLBOARD_DISABLED; wave.axis = Vector3.AXIS_Y   # 躺平贴地→随行进方向转(不再永远横着的billboard)
	wave.shaded = false; wave.transparent = true
	wave.pixel_size = 0.095
	var wave_ang: float = -atan2(dir.y, dir.x)   # 世界Y旋转(屏y→世界z反号); +90°使波纹长轴垂直行进=波前墙
	var wrot: float = deg_to_rad(float(OS.get_environment("EQDEMO_WAVEROT"))) if OS.has_environment("EQDEMO_WAVEROT") else 0.0   # 默认B: 波纹顺行进方向(用户选); EQDEMO_WAVEROT可调
	wave.rotation = Vector3(0.0, wave_ang + wrot, 0.0)
	...
	while traveled < reach and is_instance_valid(wave) and is_instance_valid(self):
		await get_tree().process_frame
		traveled += 500.0 * get_process_delta_time()   # 波速 500px/s (用户: 慢点)
		var wp: Vector2 = origin + dir * traveled
		wave.position = _world_pos(wp, 0.15)   # 贴地(躺平精灵)
		for o in _enemies_of(u):
			...
			if proj >= -40.0 and proj <= traveled + 30.0 and _on_line(origin, dir, o["pos"], 85.0):
				...
				_pull_airborne(o, origin, 70.0, 0.45)    # 拉回70码: 滞空(击飞态,禁移动/充能)期平滑滑向大熊(留24px不重叠)
