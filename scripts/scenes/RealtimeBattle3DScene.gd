...
func _sk_basic_chiwave(u: Dictionary, tgt) -> void:            # 小龟·龟派气波(封板·100龟能): 先自增buff(暴击25%/暴伤20%/吸血10%/护穿0.1A·3秒)→朝当前目标发穿透气波(带宽80·打沿途所有敌+蛋)每命中3.5A物理+击飞1.5s+击退200 [智能位移留F5]
	...
	var hit2: Array = []  # ★2026-07-10 闪退真因: 不能拿【单位字典】当 Dictionary 的 key —— Godot 会对 key 求哈希, 单位字典里有 summons/summon_owner 等互相引用的结构 → recursive_hash 无限递归 → 每次查表刷一条 ERROR: Max recursion reached。改用 Array(.has 走 == 不哈希)。
	var step2 := func(d: float) -> void:                        # 球缓慢飞·扫到敌才打(沿途穿透各1次)
		...
		for o in _enemies_of(uu2):
			...
			_knockback(uu2, o, 200.0, 2.752, 2.0)                 # 击飞【真1.5s】滞空(vy_mult=1.5/0.545=2.752·用户2026-07-05逐字"击飞1.5s，击退大概200码"; 原1.8只给0.98s)+击退200
	var btw := _reg_tween()
...
func _ninja_dash(u: Dictionary, target: Dictionary) -> void:    # 被动·冲击(亚索E式): 朝最近敌固定位移450码·主目标1.3A/路径其余0.8A物理+击飞0.8s·每敌10s冷却·无伤害递增
	...
	if dir.length() < 1.0: dir = Vector2.RIGHT
	dir = dir.normalized()
	u["pos"] = start + dir * 450.0                              # 固定位移450码(不是"落目标身后")·用户逐字
	u["pos"].x = clampf(u["pos"].x, ARENA.position.x, ARENA.end.x)
	u["pos"].y = clampf(u["pos"].y, ARENA.position.y, ARENA.end.y)
	for o in _enemies_of(u):
		if not o.get("alive", false): continue
		if not _on_line(start, dir, o["pos"], 62.0): continue
		if o["pos"].distance_to(start) > 450.0: continue        # 450码冲刺路径长度
		_apply_damage_from(u, o, _atk_dmg(u, 1.3 if o == target else 0.8, o), Color("#9fe8ff"))
		_knockback(u, o, 40.0, 1.468, 1.0)                      # 击飞【真0.8s】滞空(vy_mult=0.8/0.545=1.468·用户#18"击飞0.8秒"; 原1.6给0.87s)
...
func _sk_angel_equality(u: Dictionary, tgt) -> void:
	...
	for i in range(2):
		_pending_shots.append({"delay": 0.10 * float(i), "src": u, "fn": func():
			...
			_apply_damage_from(u, tgt, _mitigate(u, tgt["hp"] * 0.11, tgt, true), Color("#9be7ff"), 0.0, false)   # 审判(每段攻击命中都吃·独立结算不触发其他被动·用户2026-07-10"每次攻击都要吃")
		})
