...
const _IMPL_SKILLS := {
	...
	"lavaBolt": true, "lavaQuake": true, "lavaSplash": true, "crystalSpike": true, "crystalBurst": true,
...
func _do_skill(u: Dictionary, tgt: Dictionary, stype: String) -> void:
	match stype:
		...
		"lavaSplash":           _sk_lava_cast(u, tgt, "C")   # 熔岩喷射: 三段全体魔+灼烧 (变身→火山爆发)
...
func _lava_bolt(u: Dictionary, tgt: Dictionary) -> void:         # 熔岩弹: 0.9×ATK魔 + 8%目标最大HP + 0.67×ATK灼烧
	if tgt == null or not tgt.get("alive", false):
		return
	var dmg := _atk_dmg(u, 0.9, tgt, true) + int(tgt["maxHp"] * 0.08)
	_apply_damage_from(u, tgt, dmg, Color("#ff7a3c"))
	_apply_dot_stacks(tgt, "burn", _default_burn_stacks(u), u)   # 0.67×ATK 灼烧(层数DoT, 非通用rider0.5)
...
func _lava_spray(u: Dictionary) -> void:                         # 小·熔岩喷射: 全体三段 0.2×ATK×3魔 + 0.67×ATK灼烧
	for o in _enemies_of(u):
		if not o.get("alive", false):
			continue
		for i in range(3):                                        # 三段(伤害总量 0.2×ATK×3, 视觉分三跳)
			if not o["alive"]: break
			_apply_damage_from(u, o, _atk_dmg(u, 0.2, o, true), Color("#ff9d5c"))
		_apply_dot_stacks(o, "burn", _default_burn_stacks(u), u)
		_skill_ring(o["pos"], Color(1.0, 0.62, 0.36, 0.4), 46.0)
...
func _lava_transform(u: Dictionary) -> void:
	...
	# 变身瞬间: 对全体敌爆发 (1.2×ATK魔) + 灼烧, 每令一名敌灼烧回 8% 已损生命
	_anticipate(u); _shake(JUICE_SHAKE_HEAVY)
	_skill_ring(u["pos"], Color(1.0, 0.35, 0.08, 0.6), 150.0)
