...
const REVIEW_SKILL_IDX := 1   # 评审受审龟放哪个技(skillPool索引): 0=普攻/1-3=候选技/-1=默认轮转 (猎人: 0射箭✅/1精准射击✅/2隐蔽/3狩猎弹幕)
...
func _sk_hunter_barrage(u: Dictionary, _tgt) -> void:          # 猎人龟·狩猎弹幕(封板·100龟能): 10绿箭·智能优先锁残血为方向·每箭可被挡直线弹道(命中路径第一敌/蛋·障碍穿·我方不挡·复用小龟_basic_first_blocker)·0.3A真实(共3A)·每箭<14%即处决
	for i in range(10):
		...
		if hit != null:
			_apply_damage_from(u, hit, int(u["atk"] * 0.3), Color("#ffffff"), 0.0, true)   # 0.3A真实=白字(raw穿透·飘字色规范·处决由中央路径判)
