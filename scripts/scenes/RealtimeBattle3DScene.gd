				...
				if pr.get("fireball", false):   # 抛物线火球045: 落点火爆+魔法伤(蓝字)+灼烧
					...
					_spawn_bamboo_orb(tgt["pos"], _bsrc["pos"], func() -> void:    # 绿球飞回携带者, 落到身上才吸收
						...
						if _bgrow > 0.0:
							...
							_recalc_stats(_bsrc)
						_vfx._flash(_bsrc, Color(0.5, 1.7, 0.65)))                      # 吸收瞬间携带者绿闪
	...
	match u["id"]:
		...
		"ice":
			...
			for o in _enemies_of(u):
				...
				_vfx._flash(o, Color(0.6, 0.86, 1.0))   # 敌蓝闪
		"headless":
			u["lifesteal"] += 0.22
		...
		"crystal":
			if "crystalBall" in _chosen_skill_types(u["id"], u["side"] == "left"):   # 水晶球(技三·选中才召): 登场召唤实体水晶球(2026-07-16加大+特效)
				...
				if _orb != null:                              # 登场爆闪+常驻脉动光环(用户2026-07-16: 球太小没特效)
					...
					_vfx._flash(_orb, Color(1.5, 1.8, 2.0))
...
#  2.5D 适配: 逻辑/数值全照搬; VFX/坐标触点用 Phase3 的 3D 等价 (_vfx._float_text/_skill_ring/_bolt_line/_fire_bolt_from/_spawn_summon).
