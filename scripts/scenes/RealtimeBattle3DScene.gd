...
func _spawn_dual_lane() -> void:
	# ★战场不在此刻加载 —— 先走呈现(总览/预览), 预览结束才 _dl_sys._dl_build_lane_field() 真正 spawn.
	# 这样呈现时后面根本没有战场, 从源头杜绝上下路串场(用户2026-07-12「不能预览后再加载好战场吗，非要遮住是为啥」).
	if OS.has_environment("DL_AUTOFIGHT"):
		_dl_sys._dl_build_lane_field()
		_dl_state = "place"; _dl_sys._dl_enter_place()   # 测试: 跳呈现直接加载战场→放置→自动打
	elif not _dl_overview_shown:
		_dl_overview_shown = true
		_dl_sys._dl_enter_present("overview")   # 匹配后首次: 3路总览→预览→(加载战场)→放置
	else:
		_dl_sys._dl_enter_present("preview")    # 后续路: 对阵预览→(加载战场)→放置
...
## 也不会挤进战线影响分离/避障。
## ★幂等: 已经有了就不重复建 —— _spawn_teams 与 _dl_sys._dl_build_lane_field 都会调, 双路模式下
