...
const REVIEW_SKILL_IDX := 1   # 评审受审龟放哪个技(skillPool索引): 0=普攻/1-3=候选技/-1=默认轮转 (赌神: 0普攻卡牌✅/1万能牌/2赌注/3命运之轮)
...
func _gambler_pop(pos2d: Vector2, h: float, col: Color) -> void:
	var g := _glow_bb(pos2d, h + 0.5, 130.0, col)
	var tw := _reg_tween(); tw.set_parallel(true)
	tw.tween_property(g, "scale", Vector3.ONE * 1.5, 0.3)
	tw.tween_property(g, "material_override:albedo_color", Color(col.r, col.g, col.b, 0.0), 0.3)
	tw.chain().tween_callback(g.queue_free)
...
func _gambler_wild_vfx(u: Dictionary, tgt: Dictionary) -> void:
	...
	if t != null:
		...
		card.pixel_size = 1.2 / 56.0   # 比普攻牌大(特殊技)
		...
		ctw.tween_method(func(f: float) -> void:
			...
			if _cam != null:
				...
				ct.basis = _cam.global_transform.basis * Basis(Vector3(0, 0, 1), f * 12.0)   # 面向相机+滚转
...
func _sk_gambler_wild(u: Dictionary, tgt: Dictionary) -> void:   # 赌神龟·万能牌: 丢1张牌=1段1.0A物理(用户2026-07-09"只造成1段伤害")+自身0.25A护盾+回5%maxHp+目标攻-15%
	_gambler_wild_vfx(u, tgt)   # 万能牌特效(丢小丑牌+护盾+回血+减攻·用户2026-07-14)
