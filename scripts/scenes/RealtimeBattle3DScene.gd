...
func _fire_shuriken(src: Dictionary, tgt: Dictionary, dmg: int, is_true: bool) -> void:   # 旋转飞镖弹道(ninja-shuriken 4帧自旋): 飞向目标·命中结算(is_true→raw真伤·金染)
	var start2d: Vector2 = src["pos"]
	var p := Sprite3D.new()
	...
	p.shaded = false; p.transparent = true
	p.modulate = (Color(1.0, 0.86, 0.4, 1.0) if is_true else Color(1, 1, 1, 1))   # 暴击金染 / 普通原色
	...
	_projectiles.append({
		...
		"col": (Color("#ffe066") if is_true else Color("#ff4444")),   # 暴击金(真伤)/物理红(供_bkt统计正确)
		"src": src, "t": 0.0, "dur": dur, "basic_onhit": false, "raw": is_true, "shuriken_anim": true, "dtype": _last_dmg_type,
	})
