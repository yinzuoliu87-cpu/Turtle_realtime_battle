...
const REVIEW_DEMO_CFG := {
	...
	"hunter:3": [ {"dx": 320.0, "dy": -80.0, "fixed": true}, {"dx": 320.0, "dy": 80.0, "fixed": true}, {"dx": 540.0, "dy": 0.0, "fixed": true} ],   # 猎人狩猎弹幕: 前方2假人(±80·近)+1远假人(540后)→看10箭连珠速射弹幕锁残血为向·可被挡直线弹道(前假人挡后假人)·命中才跳白色真伤(0.3A/箭)
}
...
func _sk_hunter_barrage(u: Dictionary, _tgt) -> void:          # 猎人龟·狩猎弹幕(封板·100龟能): 10箭·智能优先锁残血为方向·每箭可被挡直线弹道(命中路径第一敌/蛋·障碍穿·我方不挡·复用小龟_basic_first_blocker)·0.3A真实(共3A)·每箭<14%即处决·★命中才跳伤害(用户2026-07-14)
	var es: Array = _enemies_of(u)
	for o in es:
		o["_brg_pred"] = float(o.get("hp", 0.0))             # 预测血(仅方向分配用: 残血打死→下一箭移向次残血=散射感·不改真实血)
	for i in range(10):
		var aim = null                                          # 智能优先锁残血: 选最低预测血存活敌当方向
		var lo := INF
		for o in es:
			if not o.get("alive", false): continue
			var ph: float = float(o.get("_brg_pred", 0.0))
			if ph > 0.0 and ph < lo: lo = ph; aim = o
		if aim == null: break
		var dir: Vector2 = aim["pos"] - u["pos"]
		if dir.length() < 1.0: dir = Vector2.RIGHT
		dir = dir.normalized()
		aim["_brg_pred"] = float(aim.get("_brg_pred", 0.0)) - float(u["atk"]) * 0.3   # 预测扣血→下一箭据此重选(散射感)
		var hit = _basic_first_blocker(u, dir)                  # 可被挡: 命中路径第一个敌/蛋(挡在残血目标前的敌先吃)
		var launch_delay: float = float(i) * 0.055              # 连珠速射错峰
		var flight_to: Vector2 = u["pos"] + dir * 520.0
		var flight_dur := 0.26
		if hit != null:                                         # 有挡→箭飞到挡位(伤害到点才跳=命中才跳伤害)
			flight_to = hit["pos"]
			flight_dur = clampf(u["pos"].distance_to(hit["pos"]) / 2200.0, 0.1, 0.34)
		_fly_vfx("res://assets/sprites/vfx/hunter-arrow.png", u["pos"], flight_to, 72.0, flight_dur, 1.0, launch_delay)   # 箭飞行(接现有hunter-arrow素材·0额度)
		if hit != null:
			var tgt_ref: Dictionary = hit
			var uu: Dictionary = u
			_pending_shots.append({"delay": launch_delay + flight_dur, "fn": func() -> void:   # ★命中才跳伤害: 箭到点(发射延迟+飞行时长)才结算
				if not uu.get("alive", false) or not tgt_ref.get("alive", false): return
				_apply_damage_from(uu, tgt_ref, int(uu["atk"] * 0.3), Color("#ffffff"), 0.0, true)   # 0.3A真实=白字(raw穿透·处决由中央路径判)
				, "src": u})
	for o in es:
		o.erase("_brg_pred")
