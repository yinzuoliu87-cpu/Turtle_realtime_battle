...
func _spawn_teams() -> void:
	...
	for i in range(left.size()):
		...
		if REVIEW_DEMO and str(left[i]) == "fortune":
			_lu["gold"] = 120.0   # demo: 财神起手金币(让梭哈打得动+看换目标)
		_units.append(_lu)
...
const PASSIVE_SKILL_TYPES := {"iceBurnImmune": true, "phoenixEnhancedRebirth": true}
...
func _sk_fortune_dice(u: Dictionary) -> void:                    # 财神龟·骰子 ✅
	...
	if u.get("allin_used", false):
		_grant_shield(u, u["maxHp"] * 0.10)   # 梭哈后: 额外10%最大生命永久护盾(数据 postAllInShieldPct)
...
func _tick_periodic_passive(u: Dictionary, delta: float) -> void:
	...
	# --- 财神聚宝盆: 每2.5秒 +3~8金币 (数据值) ---
	if u["id"] == "fortune":
		u["_goldtimer"] = u.get("_goldtimer", 0.0) + delta
		if u["_goldtimer"] >= 2.5:
			u["_goldtimer"] = 0.0; u["gold"] += _juice_rng.randi_range(3, 8)
