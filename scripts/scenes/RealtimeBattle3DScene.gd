...
const MAX_ENERGY := 100.0                   # (仅 UI 默认参照; 实际放招看各技 cost)
# 龟能充能 = Botworld 大招模型: 靠【输出伤害】攒(普攻打出的伤害转龟能), 非固定冷却时间.
#   高DPS龟攒得快/坦克慢; 各技 cost 不同(龟盾便宜常放, 气波/大招贵少放) → 见 SKILL_COST.
const ENERGY_PER_DMG := 0.5                 # 普攻每点伤害 → 龟能 (主来源, F5调)
const ENERGY_TRICKLE := 4.0                 # 每秒被动微回 (防输出受阻的龟永远攒不出, F5调)
const ENERGY_CAP := 200.0                   # 龟能累积上限 (>最贵技 cost, 防卡死)
const SKILL_COST_DEFAULT := 95.0
...
func _regen_energy(u: Dictionary, delta: float, stunned: bool, tgt) -> void:
	if stunned or _is_passive_pick(u):
		return
	# 龟能=Botworld大招模型: 主要靠普攻输出攒(在 _basic_attack 里加), 这里只补被动微回防卡死
	u["energy"] = minf(ENERGY_CAP, u["energy"] + ENERGY_TRICKLE * delta)
	if tgt != null:
		_cast_active(u, tgt)   # 攒够当前轮转技的 cost 就放 (cost 各技不同 → 龟盾≠气波)
...
func _basic_attack(u: Dictionary, tgt: Dictionary) -> void:
	...
	# Botworld式: 普攻打出的伤害 → 攒龟能 (高DPS龟大招更勤, 坦克更慢)
	if not _is_passive_pick(u):
		u["energy"] = minf(ENERGY_CAP, u["energy"] + float(per * hits) * ENERGY_PER_DMG)
...
func _cast_active(u: Dictionary, tgt: Dictionary) -> void:
	...
	if skills.is_empty():
		return                                        # 全普攻/被动 → 无主动可放
	var n := skills.size()
	# 1) 定位当前轮转技: 从 skill_idx 起找下一个"可放"的(实装 + 非已用完的梭哈), 跳过冷门未实装
	var idx := -1
	var stype := ""
	for k in range(n):
		var ii := (int(u["skill_idx"]) + k) % n
		var s := str(skills[ii])
		if _IMPL_SKILLS.has(s) and not (s == "fortuneAllIn" and u.get("allin_used", false)):
			idx = ii; stype = s; break
	if idx < 0:
		return                                        # 全不可放
	u["skill_idx"] = idx
	# 2) 攒够【当前技】的 cost 才放 (贵技→等更久攒; 不抢放便宜的 → 体现 龟盾≠气波 不同节奏)
	if u["energy"] < _skill_cost(stype):
		return
	if _cast_skill(u, tgt, stype):
		u["energy"] -= _skill_cost(stype)             # 花掉对应龟能
		u["skill_idx"] = (idx + 1) % n                # 轮转推进 → 下次攒下一个
		_eq_on_cast(u, tgt)                           # on-cast 装备钩子
	else:
		u["skill_idx"] = (idx + 1) % n                # 放失败(如梭哈已用)→推进避免卡
...
func _update_overlay() -> void:
	...
	for u in _units:
		...
		if enf != null and is_instance_valid(enf) and enf.visible:
			# 进度 = 当前龟能 / 下一个要放的技的 cost (各技 cost 不同 → 条满速度不同)
			var ecap: float = MAX_ENERGY
			var ask: Array = u.get("active_skills", [])
			if not ask.is_empty():
				ecap = _skill_cost(str(ask[int(u.get("skill_idx", 0)) % ask.size()]))
			enf.size.x = BAR_W * clampf(u["energy"] / maxf(1.0, ecap), 0.0, 1.0)
		# 头顶世界坐标 → 屏幕
