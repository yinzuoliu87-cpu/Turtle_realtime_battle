...
const REVIEW_DEMO_CFG := {
	...
	"space:1": [ {"dx": 320.0, "dy": 0.0, "minion_role": "front", "move": true, "skills": ["minionBodysurf"]} ],   # 虫洞(用户2026-08-30「只要两个单位」): 星际龟+近战小将各一·双方无敌·NO_TRAINER 关大师; 小将自带位移技人体浪板(审阅台会清空假人技能故显式给回)⇒ 看得到被吸住→挣脱整段
	...
	"shell:1": [ {"dx": 210.0, "dy": -70.0, "id": "lava", "skills": ["lavaQuake"], "fixed": true}, {"dx": 210.0, "dy": 70.0, "id": "ice", "skills": ["iceFreeze"], "fixed": true} ],   # ★复制(skillPool[1], 不是[2] —— 2026-08-30 修: 表里 shell:1/shell:2 的注释整个换反了)。假人必须【真带技能】: 审阅台默认清空 active_skills, 而复制是抄敌人的技 ⇒ 池子为空、一次都放不出来(实测空转)。给两只不同龟各一个好认的技, 才看得出抄到了谁的什么
	"shell:2": [ {"dx": 150.0, "dy": 0.0, "fixed": true} ],   # 吸收(skillPool[2]): 4红珠鱼贯流向龟壳+maxHp转移
...
func _sim_step(dt: float, frozen: bool, in_ts: bool) -> void:
	...
	if _fight_on:
		...
		_equip_tick_sys.tick_delayed(dt)  # 装备【延时结算】共享队列(024/025/026: 走 sim 钟不走 tween)
...
## ★法器【一件都不在这张表里】(2026-08-12): 法器的主动只能由法力条满触发
...
## 021 守护贝母的绑定绳 —— 实现搬进 `battle_vfx.barnacle_line()`。
## ★CLAUDE.md §5:「不在 `_sim_step` 调用链上的代码不进主文件」—— 这一段是**纯演出**
##   (不挂任何结算), 主文件只留这一行转发。同 v0.19.361 `_bolt_line` 的处置。
func _update_barnacle_line(u: Dictionary, target) -> void:
	_vfx.barnacle_line(u, target)
...
# shellCopy 能抄什么 ⇒ scripts/gamedata/copy_rules.gd (2026-08-29 白名单倒成黑名单)
#   规则原文「排除变身/召唤/自增益」被用户否掉了:「虚化就是给自己 buff 并造成伤害,
#   我不明白为什么不能复制」。黑名单只留【探针量到真会留下残留】的 5 个, 理由见那个文件。
...
func _on_basic_hit(u: Dictionary, tgt: Dictionary) -> void:
	...
	if u["id"] == "headless":
		...
		u["atk"] = u["base_atk"] * (1.0 + lost_pct * HeadlessSystem.UNDEAD_ATK_PER_LOST)
...
func _make_panel_equip_slot(u: Dictionary, eid: String) -> Control:   # 头像下装备格: 图标(稀有度描边)+充能条/层数徽章
	...
	if PANEL_CHARGE.has(eid):   # 充能进度条: 宽刃弯刀等
		## 值 = [字段名, 满值, 可选条色]。★这里【曾经】支持"一件多条"(023/026 各有
		## 一条自己的老充能条 + 一条法器法力条), 2026-08-12 把那两条老充能并进法力条后
		## 零使用者 ⇒ 撤掉, 免得留一条"写了没人读"的死泛化。构建在 _hud。
		_hud.add_equip_charge_bar(u, slot, eid, PANEL_CHARGE[eid])
	return slot
