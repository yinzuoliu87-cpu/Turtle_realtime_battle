...
func _sk_dice_flash_strike(u: Dictionary) -> void:   # 稳定骰子(刀妹Irelia Q·破空斩式·用户2026-07-13): 掷骰(4+点数)段, 每段真冲刺穿到随机敌+挥剑斩·递减10%
	...
	u["dice_dash_seg"] = 0
	u["dice_dash_target"] = t0

# 稳定骰子真冲刺连突(刀妹Irelia Q式): 固定高速逐帧穿刺到随机敌 → 到位挥剑斩 → 接下一段. 覆盖正常AI(锁住).
func _dice_dash_tick(u: Dictionary, delta: float) -> void:
	...
	if tgt == null or not tgt.get("alive", false):
		tgt = _dice_pick_strike_target(u)           # 目标死/无 → 换随机敌
		u["dice_dash_target"] = tgt
	if tgt == null:
		u["dice_dash_active"] = false; u["state"] = "move"
		return
	var to_t: Vector2 = tgt["pos"] - u["pos"]
	if to_t.length() <= 55.0:                        # 冲到/穿到目标 → 挥剑斩结算
		_dice_dash_hit(u, tgt)
		...
		if int(u["dice_dash_remaining"]) <= 0:
			u["dice_dash_active"] = false; u["state"] = "move"
			return
		u["dice_dash_target"] = _dice_pick_strike_target(u)   # 下一段随机敌
		u["dice_dash_pause_until"] = _t + DICE_DASH_PAUSE     # 顿一下再冲
		return
	u["pos"] += to_t.normalized() * DICE_DASH_SPD * delta     # 固定高速真位移(不是瞬移)
	u["pos"].x = clampf(u["pos"].x, ARENA.position.x, ARENA.end.x)
	u["pos"].y = clampf(u["pos"].y, ARENA.position.y, ARENA.end.y)
	u["face_right"] = to_t.x > 0.0
	if int(_t * 45.0) % 2 == 0:                      # 青蓝刀锋拖尾(刀妹穿刺残影)
		_dice_blade_trail(u["pos"], to_t)

func _dice_dash_hit(u: Dictionary, tgt: Dictionary) -> void:
	...
	_dice_blade_slash(tgt["pos"], tgt["pos"] - u["pos"])   # AI挥剑斩
	_melee_lunge(u, tgt)
