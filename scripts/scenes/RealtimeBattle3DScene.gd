...
func _spawn_teams() -> void:
	...
	for _pu in _units:        # 海盗: 开战即在后方显示持久海盗船(用户2026-07-14"战斗开始就在位置上")
		if str(_pu.get("id", "")) == "pirate" and not _pu.get("is_summon", false):
			_pirate_sys._pirate_get_ship(_pu)
	...
	match u["id"]:
		...
		"pirate":
			...
			if not es.is_empty():
				...
				_pirate_sys._pirate_ship_muzzle(ps2d, psh)
...
func _spawn_pirate_ship(u: Dictionary, tgt = null) -> void:   # 首次: 后方持久演出船俯冲冲锋→撞击点转变为实体海盗船参战(用户2026-07-14设计)
	...
	var deco := _pirate_sys._pirate_get_ship(u)                        # 后方持久演出船=这次冲锋的船
	...
	_pirate_sys._pirate_ship_muzzle(from2d, from_h); _pirate_sys._pirate_ship_muzzle(from2d + Vector2(30, 0), from_h)   # 蓄力: 炮口全闪
	var uu: Dictionary = u
	_pirate_sys._pirate_ship_charge(deco, from2d, from_h, impact2d, func() -> void:   # 俯冲到撞击点→结算+转变实体
		_pirate_sys._pirate_ship_splash(impact2d)                     # 大水花爆
