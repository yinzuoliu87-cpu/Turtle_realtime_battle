...
##   熔岩地面区               13     _lava_
##   VFX 工具                 10     _skill_ring / _burst_vfx / _beam_vfx / _hit_spark …
...
const HOOK_MISSILE_SPD := 950.0  # 钩子飞行速度(码/秒·中速, 不再瞬移): 600码约0.63秒到(原来0.14-0.34秒太快)
# 拖拽改成【一段段的拽】(锤石Q: 0.1s后拽一下, 隔0.6s再拽一下, 每下拽一小段) —— 非匀速
const HOOK_TUG_DELAY := 0.1      # 钩住后第一下拽的延迟
const HOOK_PULL_INTERVAL := 0.6  # 两下拽之间的间隔(每 0.6s 拽一下)
const HOOK_TUG_DUR := 0.15       # 每一下"拽"的快速位移时长(其余时间不动=停顿感)
const HOOK_TUG_DIST := 42.0      # 每一下拽把目标朝大师拽近的距离(码)
...
#   不走 _play_action —— 那个只认 attack/hurt/death 三种; 这些照忍者 dash/backstab 的做法,
#   在技能代码里直接 _elite_anim() 调。action 名会写进 u["anim_action"], 由 _play_action 顶部的
#   白名单挡住, 播完前不被普攻/受击换掉。
...
func _ready() -> void:
	...
	#   match1(第一把): "place"摆位引导延到 _dl_enter_place 才挂(那时才有摆位UI, 文案对得上屏幕);
...
func _tile_material(ti: int) -> Material:    # 每type材质: 水=滚动波纹shader发光; 其余=暗调+斜网格贴图
	...
	m.albedo_texture = VfxTex._make_tile_texture()
...
func _build_ground() -> void:
	...
	if OS.has_environment("BLACKMAP"):   # 黑地图: 地面纯黑
		...
		mi.material_override = _bm; _world.add_child(mi); return
...
func _spawn_teams() -> void:
	...
	for i in range(left.size()):
		...
		if _review_demo() and str(left[i]) == "fortune":
			_lu["gold"] = 30.0 if (_review_skill_idx() == 2 or _review_skill_idx() == 0) else 0.0   # demo: 审梭哈/被动时备30金币(梭哈看弹幕·被动看冒金币+普攻打得动看死亡涌币); 否则0看自然攒
		if _review_demo() and REVIEW_DUMMY_ATTACKS:
			_lu["_review_dummy"] = true   # 假人会还手时受审龟免死(看完整被动循环)
		if _review_demo() and str(left[i]) == "dice" and _review_skill_idx() == 0:   # 被动赌徒之血: 起手40%血→看血色气焰+暴击拉满(巨血防死·不回满以维持低血)
			_lu["maxHp"] = float(_lu["maxHp"]) * 20.0
			_lu["hp"] = _lu["maxHp"] * 0.4
			_lu["_review_dummy"] = false
		if _review_demo() and str(left[i]) == "gambler" and _review_skill_idx() == 2:   # 赌注低血模式: 起手22%血(<40%)→触发低血分支(回8%maxHp+3秒多重·绿闪绿环)·随回血/消耗在阈值震荡(巨血防死)
			_lu["maxHp"] = float(_lu["maxHp"]) * 20.0
			_lu["hp"] = _lu["maxHp"] * 0.7   # 起手70%>40%→先正常模式(消耗40%+红字多重), 掉到40%以下再低血模式(回血)·两模式都看到
			_lu["_review_dummy"] = false
		if _review_demo() and str(left[i]) == "pirate" and _review_skill_idx() == 2:   # 朗姆酒: 海盗站桩(不攻击不移动·免假人盾/巨伤干扰)+35%残血血锁不死→放朗姆酒看船扔酒瓶+HoT绿回血把血顶上来(血条明显动)+暖色护光
			_lu["hp"] = float(_lu["maxHp"]) * 0.35
			_lu["deathfloor_until"] = 999999.0
			_lu["_review_dummy"] = false
			_lu["no_basic"] = true; _lu["no_move"] = true; _lu["move_spd"] = 0.0
		if _review_demo() and str(left[i]) == "pirate" and _review_skill_idx() == -1:   # 被动掠夺: 海盗40%残血+_pdeath_demo(被假人打死→放死亡钩索但不真死复位循环)·开场也看登场轰击
			_lu["hp"] = float(_lu["maxHp"]) * 0.4
			_lu["_review_dummy"] = false
			_lu["_pdeath_demo"] = true
		if _review_demo() and str(left[i]) == "headless" and _review_skill_idx() >= 1:   # 无头技能窗评审: 充能加速(demo看得到反复放技·实战原速)
			_lu["echarge_perm"] = 4.0
		if _review_demo() and str(left[i]) == "headless" and _review_skill_idx() == -1:   # 亡灵被动demo: 中血非血锁→假人打到濒死触发免死(_undead_demo: 免死窗过后回满+清undead_used循环)+看残血紫焰渐浓
		...
		if _review_demo() and str(left[i]) == "lava":   # 熔岩评审便利: 出生90怒气→打几下就变身火山(验火山形态技变体/被动不用干等攒怒气)
			_lu["rage"] = 90.0
		if _review_demo() and str(left[i]) == "crystal" and _review_skill_idx() == 1:   # 水晶壁垒demo: 固定不动(防近战追击漂移到场角→水晶刺朝场外放看不见)
			_lu["no_move"] = true
			_lu["move_spd"] = 0.0
		if _review_demo() and str(left[i]) == "cyber" and _review_skill_idx() == -1:   # 赛博被动demo(用户2026-07-16: 固定不动+1假人打): 攒编队→打死→自由飞散→齐射→组装→循环
			_lu["_review_dummy"] = false
			_lu["_cydeath_demo"] = true
			_lu["no_move"] = true
			_lu["move_spd"] = 0.0
		if _review_demo() and str(left[i]) == "bubble" and _review_skill_idx() == 3:   # 泡泡爆破: 预置满泡泡值(不用挨打攒·每帧回满)+站桩→反复放爆破看两门+泡沫墙清晰展开
	...
	for i in range(right.size()):
		...
		if _review_demo():                          # 假人: 不放技/永不死训练靶; ATTACKS时会还手(动+普攻)
			...
			if _review_turtle() == "hunter" and _review_skill_idx() == -1:   # 猎人被动猎杀demo靶: 钉12%残血(<14%斩杀线)→猎人扫场自动射强化箭处决; 血锁不死(普通箭伤打不死·只被强化箭处决触发VFX)循环看处决+窃取
			...
			if _review_turtle() == "elite" and _review_skill_idx() == -1:   # 吞噬demo靶: 18%残血非沙包+吞噬后复活循环+留技能看偷放
				ru.erase("_review_dummy")
				ru["maxHp"] = 3000.0; ru["hp"] = ru["maxHp"] * 0.18
				ru["_consume_demo"] = true
				ru["active_skills"] = _resolve_active_skills(str(right[i]), false)
	...
	_equip_sys._eq_apply_all_stats()     # 开战: 全装备纯属性 / 永久 flag 加到携带者 (spawn 被动之后, 不被覆盖)
	...
	if OS.has_environment("EQDEMO_FORCE_HP50"):   # demo: 强制携带者<50%触发救命类(044/045), 便于验证特效
		for _fu in _units:
			if _fu["side"] == "left" and not _fu.get("equips", []).is_empty():
				...
				_equip_sys._eq_check_hp_threshold(_fu)
...
func _dl_enter_place() -> void:
	...
	if not _tut_place_shown:
		var _tdp = get_node_or_null("/root/TutorialDirector")
		if _tdp != null and _tdp.is_active() and str(_tdp.stage()) == "match1":
			_tut_place_shown = true
			_tutorial = _tdp.attach_guide(self, "battle")   # stage match1 → steps "place"
...
func _dl_build_lane_field() -> void:
	...
	_spawn_trainers()    # 双方场外监视者。★放在这里 = 每路 spawn 一次, 而 _dl_next_lane 会先
	...
	_equip_sys._eq_apply_all_stats()
...
func _dl_side_alive(side: String) -> int:   # 一侧存活非蛋·非惰性召唤 单位数(战斗型召唤算数·用户2026-07-11 #3)
	...
	if n == 0 and _t < float(_mech_incoming.get(side, 0.0)):
		n = 1
	return n
...
func _dl_drop_fence(side_lr: String) -> void:   # 该方蛋围栏消失(可被自由索敌); 定局路(终极/横扫)暴露蛋挂 ×5承伤+自损
	...
	for u in _units:
		if u.get("_isEgg", false) and str(u.get("egg_side_lr", "")) == side_lr:
			u["_egg_fence"] = false
			u["def"] = maxf(0.0, float(u.get("def", 60.0)) - EGG_FENCE_RES); u["mr"] = maxf(0.0, float(u.get("mr", 60.0)) - EGG_FENCE_RES)   # 屏障消失→回落到 60+15×等级
			u["base_def"] = maxf(0.0, float(u.get("base_def", 60.0)) - EGG_FENCE_RES); u["base_mr"] = maxf(0.0, float(u.get("base_mr", 60.0)) - EGG_FENCE_RES)
			if final_buff:
...
func _dl_flow_check() -> void:
	...
	if _dl_state == "fight":
		if la == 0 or ra == 0:
			...
			_dl_state = "eggwindow"
			if OS.has_environment("XDBG"): print("XDBG_DL wiped=", _dl_wiped_side, " decider=", decider, " t=", _t, " → eggwindow")
...
func _dl_clear_units() -> void:
	...
	for f in _follow_vfx:
		...
		if is_instance_valid(fs):
			fs.queue_free()
	_follow_vfx.clear()
	for z in _lava_sys._lava_zones:                     # 补清: 岩浆池(用户2026-07-12"回合间没清")
		var zd = z.get("disc", null)
		if is_instance_valid(zd): zd.queue_free()
	_lava_sys._lava_zones.clear()
	...
	_sim_tweens.clear()
	_float_dmg_window.clear(); _float_nd_window.clear(); _float_merge.clear()
...
func _make_unit(id: String, side: String, pos: Vector2, spec: Dictionary = {}) -> Dictionary:
	...
	if is_egg:
		...
		u["def"] = _egg_res + EGG_FENCE_RES; u["mr"] = _egg_res + EGG_FENCE_RES
		u["base_def"] = _egg_res + EGG_FENCE_RES; u["base_mr"] = _egg_res + EGG_FENCE_RES
...
func _draw_aim_indicator() -> void:
	if _world == null:
		return
	var tr = _my_trainer()
	if tr == null or _disc_aim_dir.length() < 0.01:
		return
	var u: Dictionary = tr
	var sid: String = str(u.get("_tr_active", "hook"))
	var rng: float = float(TRAINER_SKILLS.get(sid, {}).get("range", 600.0))
	...
	var to2d: Vector2 = from2d + _disc_aim_dir.normalized() * rng
...
## 也不会挤进战线影响分离/避障。
## ★幂等: 已经有了就不重复建 —— _spawn_teams 与 _dl_build_lane_field 都会调, 双路模式下
##   若两边都跑到会 spawn 两个(实测双路走的是后者, 但留这道闸防将来改动)。
func _spawn_trainers() -> void:
	if OS.has_environment("VFXPREVIEW") or DEBUG_EDIT:
		return
	for u in _units:
		if u.get("is_trainer", false):
			return
...
func _update_run_anim(u: Dictionary, delta: float) -> void:
	if not u.has("run_sd"):
		var e = ACTION_RUN.get(_anim_key(u), null)   # ★同 _play_action: 三种小将共用 id, 要走动画键
...
## 精英小将专属动作: 直接换帧表, 不经 _play_action (它只认 attack/hurt/death).
##   action 写进 u["anim_action"] → 被 _play_action 顶部白名单挡住, 播完前不被普攻/受击换掉;
...
func _elite_anim(u: Dictionary, action: String) -> void:
	...
	if u.get("anim_action", "") == "death":
		return
	var e = ACTION_ELITE.get(action, null)
	if e == null:
		return
...
#   加新单位时在这里加一行即可, 不要再复制一份 _elite_fix_norm。
...
func _elite_fix_norm(u: Dictionary, sd: Dictionary) -> void:
	...
	if not is_instance_valid(spr):
		return
	var nk = ANIM_NORM.get(_anim_key(u), null)
	if nk == null:
...
func _melee_anim(u: Dictionary, action: String) -> void:
	...
	if asd.is_empty():
		return
	_set_anim_sheet(u, asd, action, false)
	_elite_fix_norm(u, asd)   # 归一查 ANIM_NORM 表, 前排/精英各自一行
...
func _play_action(u: Dictionary, kind: String) -> void:
	...
	#   backstab=忍者背刺; ACTION_ELITE 五个=精英小将旋刃/铁锤/强化铁锤/铁链/吞噬
	var _cur_act := str(u.get("anim_action", ""))
	if (_cur_act == "backstab" or ACTION_ELITE.has(_cur_act) or ACTION_MELEE.has(_cur_act) or u.get("_manual_anim", false)) and kind != "death":
		return
	var id := _anim_key(u)   # ★不能直接用 u["id"]: 三种小将(前排/后排/精英)共用 "__minion__",
							 #   按 id 查表会让普通小将也命中精英的动作帧。见 _anim_key。
...
func _tick_unit(u: Dictionary, delta: float) -> void:
	...
	if _t < float(u.get("rum_until", 0.0)):   # 海盗朗姆酒: 每秒回4%maxHP(分秒HoT·rum_dps=每秒速率)
		...
		while u["_rum_vfx_t"] >= 0.55:   # 每0.55s: 暖酒气泡 + 实际回血绿字(真回了才显·符合数字规矩)
			u["_rum_vfx_t"] -= 0.55
			_pirate_rum_bubble(u)
			var _rhd: int = int(u.get("_rum_heal_acc", 0.0))
			u["_rum_heal_acc"] = 0.0
			if u.get("alive", false) and _rhd >= 1:
				_float_text(u["pos"] + Vector2(randf_range(-18.0, 18.0), -48.0), "+" + str(_rhd), Color("#7fe39a"))
	if not u["alive"]:
		return
			...
			if u["state_t"] <= 0.0:
				...
				if p == "B":
					...
					if u["id"] == "two_head" and stype != "twoHeadFusion":   # 双头双生(用户2026-07-11反序): 龟能满→①先切形态+位移(铺垫)→②顿→③再放该形态技(非收尾)
						...
						_two_head_after_cast(u, tgt)                          # ① 切形态+位移(滑扑进/滑退出·含切换攻击)
						var _thd2: float = 0.5 if u["melee"] else 0.45        # ② 顿(位移到位+停顿; 此时melee=切后新形态)
						u["_anim_lock_until"] = _t + _thd2
						_pending_shots.append({"delay": _thd2, "src": u, "fn": _two_head_deferred_cast.bind(u, tgt, stype)})   # ③ 放该形态技
...
func _ninja_mark_shatter(spr) -> void:
	...
	for k in range(4):
		...
		var st := create_tween().bind_node(sh); st.set_parallel(true)
...
func _fire_coral_spike(src: Dictionary, tgt: Dictionary, si: int) -> void:   # 珊瑚尖刺弹→最远敌(wisp_dir尖朝目标·方向等距不歪)·命中(arrival)物理+%maxHP魔法+珊瑚碎裂
	if tgt == null: return
	if _coralspike_tex == null: _coralspike_tex = VfxTex._make_coralspike_texture()
	var start2d: Vector2 = src["pos"]
	var p := Sprite3D.new()
	p.texture = _coralspike_tex
	p.texture_filter = BaseMaterial3D.TEXTURE_FILTER_LINEAR
	...
	_world.add_child(p)
	_projectiles.append({
		"node": p, "from": _world_pos(start2d, 1.0), "tgt": tgt, "dmg": 0, "col": Color(1.0, 0.5, 0.36),
...
func _sk_phoenix_scald(u: Dictionary, tgt) -> void:
	...
	tw.tween_callback(_phoenix_sys._phoenix_scald_hit.bind(u, tgt, fb))
...
func _fire_bolt_from(src, tgt: Dictionary, dmg: int, col: Color, from = null, basic_onhit: bool = false) -> void:
	...
	if src is Dictionary and str(src.get("id", "")) == "gambler":   # 赌神: 甩旋转扑克牌(黑桃A)
		...
		p.pixel_size = (30.0 * WS) / float(maxi(1, p.texture.get_height()))
		p.texture_filter = BaseMaterial3D.TEXTURE_FILTER_NEAREST
		oriented = true
		...
		if frac >= 1.0:
			...
			if tgt["alive"]:
				...
				if pr.get("fireball", false):   # 抛物线火球045: 落点火爆+魔法伤(蓝字)+灼烧
					...
					_bamboo_sys._bamboo_hit_splash(tgt)                                        # 大淡绿命中爆(≈上半身大小)
					...
					_spawn_bamboo_orb(tgt["pos"], _bsrc["pos"], func() -> void:    # 绿球飞回携带者, 落到身上才吸收
						...
						if _bgrow > 0.0:
							...
							_recalc_stats(_bsrc)
						_flash(_bsrc, Color(0.5, 1.7, 0.65)))                      # 吸收瞬间携带者绿闪
...
func _shotgun_pellet(from2d: Vector2, to2d: Vector2, col: Color, dur: float = 0.42, on_land: Callable = Callable()) -> void:
	if _pellet_tex == null: _pellet_tex = VfxTex._make_pellet_texture()
	var sp := Sprite3D.new()
	...
	tw.tween_property(sp, "position", _world_pos(to2d, 1.0), dur).set_ease(Tween.EASE_OUT)
	if on_land.is_valid():
...
func _apply_damage_from(src: Dictionary, u: Dictionary, dmg: int, col: Color, extra_ls: float = 0.0, raw: bool = false, from_equip: bool = false, pre_crit: bool = false, no_dodge: bool = false) -> void:
	...
	if _ink_true >= 0.5:   # ★墨迹(线条被动)真伤单独跳白字+计真伤桶(用户2026-07-18"墨迹真伤没生效"): 原折进d扣血但不跳字不计统计→看着像没生效; 现显式可见=真的在打
		...
		_float_text(u["pos"] + Vector2(0.0, -34.0), str(_iv), _VC.color_of(_VC.cls_for("damage", "true", false)), false, "damage", "true", -_jdir)
...
func _redirect_damage(carrier: Dictionary, amt: float, dtype: String) -> void:
	...
	_float_text(carrier["pos"] + Vector2(0, -40), str(shown), _VC.color_of(_VC.cls_for("damage", dtype, false)), false, "damage", dtype)
	if amt <= 0.0:
		return
...
func _fire_venom_fang(src: Dictionary, tgt: Dictionary, base: float) -> void:   # 毒牙弹: 双生獠牙飞向目标(wisp_dir尖朝目标)·命中(arrival)魔法伤+毒液飞溅+回100%
	if tgt == null: return
	if _venomfang_tex == null: _venomfang_tex = VfxTex._make_venomfang_texture()
	var start2d: Vector2 = src["pos"]
	var p := Sprite3D.new()
	p.texture = _venomfang_tex
	p.texture_filter = BaseMaterial3D.TEXTURE_FILTER_LINEAR
	p.billboard = BaseMaterial3D.BILLBOARD_DISABLED   # wisp_dir手动basis(尖朝目标屏幕方向)
	p.shaded = false; p.transparent = true
	p.pixel_size = 0.052
	...
	_world.add_child(p)
	_projectiles.append({
		"node": p, "from": _world_pos(start2d, 1.0), "tgt": tgt, "dmg": 0, "col": Color("#c96bff"),
...
# 蓝色能量火星 (一次性爆·自销) — 仿 _impact_particles 但蓝+加性
...
func _do_skill(u: Dictionary, tgt: Dictionary, stype: String) -> void:
	...
	match stype:
		...
		"minionBodysurf":       _sk_minion_bodysurf(u, tgt)
		"minionRocket":         _sk_minion_rocket(u, tgt)
...
func _sk_bamboo_smack(u: Dictionary, tgt) -> void:              # 竹叶龟·竹击(用户封板·120龟能): 钩全场最远敌·1.0A物理·眩晕0.5s·拉贴身·冰寒4秒(-20%攻/-20%移速); 蛋免控只吃伤
	...
	if not far.get("_eggImmune", false):                        # 蛋/免控只吃伤
		_stun(far, 0.5, "_sk_bamboo_smack")
...
func _ninja_glide(u: Dictionary, start: Vector2, endp: Vector2, dir: Vector2, target: Dictionary, hits: Array) -> void:
	...
	if _ndash:
		...
		_set_anim_sheet(u, _ndsd, "dash", false)
		...
		for h in hits:
			if not h["done"] and traveled >= float(h["proj"]):
				...
				if o.get("alive", false):
					_apply_damage_from(u, o, _atk_dmg(u, 1.3 if is_same(o, target) else 0.8, o), Color("#9fe8ff"))
					_knockback(u, o, 40.0, 1.468, 1.0)          # 击飞 0.8s 滞空
					if is_same(o, target): o["_ninja_dash_until"] = _t + 10.0          # 只有【目标】进10s冷却·顺路割到的不占(用户2026-07-11)
...
func _sk_ninja_backstab(u: Dictionary, tgt: Dictionary) -> void: # 技三·背刺(1:1回合制 ninjaBackstab): +5穿甲5秒→闪现到【全场最远敌】身后→背刺【3段·每段300ms(hitStaggerMs)】各0.6667A物理(共2.0A)→留该处追砍
	...
	var uu: Dictionary = u
	_pending_shots.append({"delay": 5.0, "fn": func(): uu["armor_pen"] = float(uu.get("armor_pen", 0.0)) - 15.0, "src": u})
	_dash_to(u, far, -70.0)                                     # 闪现到最远敌身后
	u["face_right"] = far["pos"].x > u["pos"].x                # 面向最远敌(背刺sheet已镜像统一朝向)
	_beam_vfx("res://assets/sprites/vfx/fx-trail.png", from2d, u["pos"], 34.0, Color(0.8, 0.9, 1.0, 0.55), 0.28)   # 闪现刀光拖影(起点→落点)
	var asd := _resolve_action("pets/animations/ninja/backstab-rt.png", 16.0)   # 忍者背刺18帧全身动作(盖身上·播完回idle)
	if not asd.is_empty():
		_set_anim_sheet(u, asd, "backstab", false)                            # action="backstab"→_play_action 里挡普攻/受击换anim
		u["_anim_lock_until"] = _t + float(asd.get("frames", 18)) / 16.0 + 0.05   # 动作播完前锁出手/移动(用户2026-07-11「动作播完前不打断」)
	# 背刺三段: 每段 300ms 一刀(hitStaggerMs 300) 各 0.6667A 物理 + 斩弧, 目标死则跳过后续段
	var far2: Dictionary = far
	for i in range(3):
		_pending_shots.append({"delay": 0.30 * float(i), "src": u, "fn": func():
			if not u.get("alive", false) or not far2.get("alive", false):
				return
			_apply_damage_from(u, far2, _atk_dmg(u, 0.6667, far2), Color("#cfd8e8"))
			_burst_vfx("res://assets/sprites/vfx/ninja-slash.png", far2["pos"], 88.0, 1.0)   # 每刀斩弧
			_hit_spark(far2)
		})
...
func _sk_ghost_phantom(u: Dictionary, tgt) -> void:
	...
	_shake(0.08)
	_pending_shots.append({"delay": 0.11, "src": u, "fn": _ghost_sys._ghost_phantom_hit.bind(u, tgt)})   # 命中延到动画第2帧算(~0.11s·用户2026-07-11)
...
func _sk_ghost_soulstorm(u: Dictionary, tgt: Dictionary) -> void: # 幽灵龟·灵魂风暴 ✅
	...
	if cursed:
		...
		_add_dot(tgt, "curse", tgt["maxHp"] * 0.05, 10.0, u)   # 技2诅咒10秒(用户2026-07-11·被动登场诅咒仍5s)
...
func _sk_ghost_phase(u: Dictionary, tgt: Dictionary) -> void:    # 幽灵龟·虚化 (用户2026-07-08): 虚化4秒受物理伤害-90% + 对目标2段共1.2A真伤
	...
	u["energy_lock_until"] = _t + 4.0   # 虚化时锁龟能条(用户2026-07-11·复用energy_lock_until→_tick_skill_cd锁)
	for i in range(2):
	...
	_skill_ring(u["pos"], Color(0.7, 0.45, 1.0, 0.78), 92.0)   # 虚化入场幽紫脉冲(用户2026-07-11)
	var _pr2 := _reg_tween()
	_pr2.tween_interval(0.08)
	_pr2.tween_callback(_skill_ring.bind(u["pos"], Color(0.6, 0.4, 0.95, 0.5), 145.0))
	_shake(0.06)
...
func _sk_diamond_unbreak(u: Dictionary) -> void:                 # 钻石龟·坚不可摧(封板): 20%最大生命护盾(4秒·限时盾原语)+护甲/魔抗各+20%攻击力(flat·5秒)
	...
	u["diamond_fortify_until"] = _t + 4.0   # 持盾期锁龟能+青水晶护罩(与shield同4s·盾破/到期即恢复·用户2026-07-12)
	_buff(u, "def", u["atk"] * 0.2, false, 5.0)
	_buff(u, "mr", u["atk"] * 0.2, false, 5.0)
	_burst_vfx("res://assets/sprites/vfx/diamond-fortify.png", u["pos"], 100.0, 0.5)   # 水晶护甲成型(蓝晶迸发·此前零特效)
	_skill_ring(u["pos"], Color(0.55, 0.82, 1.0, 0.55), 58.0)
...
func _sk_diamond_smash(u: Dictionary, tgt) -> void:             # 钻石龟·钻石冲撞(用户2026-07-12改): 短暂蓄力→撞击(保留1.0甲+1.0抗+0.1A物理)+击退300码顺冲撞方向+一点点击飞+流血(层数=0.5A+0.1甲+0.1抗)+3s50%减速
	...
	_pending_shots.append({"delay": DIAMOND_SMASH_CHARGE, "src": u, "fn": _diamond_sys._diamond_smash_impact.bind(u, tgt)})
...
func _sk_dice_allin(u: Dictionary) -> void:                      # 骰子龟·孤注一掷(用户设计: 前方120°/300码镰刀扇形斩·1.2A物理+30%吸血)
	...
	_dice_sys._dice_scythe_sweep(u, u["pos"], dir, 300.0, 60.0)   # 红镰刀贴地弧扫过120°扇形(用户2026-07-13)
	_skill_ring(u["pos"], Color(1.0, 0.3, 0.3, 0.35), 52.0)
...
func _sk_dice_flash_strike(u: Dictionary) -> void:   # 稳定骰子(刀妹Irelia Q·破空斩式·用户2026-07-13): 掷骰(4+点数)段, 每段真冲刺穿到随机敌+挥剑斩·递减10%
	...
	var t0 = _dice_sys._dice_pick_strike_target(u)
	if t0 == null: return
	u["dice_dash_active"] = true                    # 进入真冲刺连突态(逐帧穿刺, 见 _dice_sys._dice_dash_tick)
	u["dice_dash_remaining"] = count
	u["dice_dash_seg"] = 0
	_dice_sys._dice_dash_set_target(u, t0)                    # 设目标+算穿过落点
...
func _sk_rainbow_shield(u: Dictionary) -> void:                  # 彩虹龟·棱镜护盾 ✅
	_rainbow_sys._rainbow_prism_shield_vfx(u)   # 七彩棱镜爆发+每友军护盾罩(用户2026-07-13补施法特效)
	for o in _allies_of(u):
...
func _sk_gambler_bet(u: Dictionary, tgt: Dictionary) -> void:    # 赌神龟·赌注(用户封板·100龟能): 需当前生命>40%; 消耗当前生命40%→7段物理砸目标(共≈0.4×当前生命); 施放3秒多重概率+20%
	...
	if u["hp"] < u["maxHp"] * 0.40:                              # ★低血模式(用户2026-07-14): 血不够押本→转而回8%maxHp + 3秒多重+20%(不甩牌·不消耗血)
		...
		u["gambler_bet_until"] = _t + 3.0
		_flash(u, Color(0.4, 1.7, 0.6))                         # 绿闪(求稳回血)
		_skill_ring(u["pos"], Color(0.3, 1.0, 0.5, 0.55), 54.0)   # 绿环
		_gambler_sys._gambler_pop(u["pos"], float(u.get("height", 0.0)), Color(0.3, 1.0, 0.5, 0.85))   # 回血绿
		return
	var cost: float = u["hp"] * 0.40
	u["hp"] = maxf(1.0, u["hp"] - cost)
	_flash(u, Color(1.7, 0.4, 0.4))                            # HP牺牲红闪(押上血本)
	...
	for i in range(7):                                         # 7张牌barrage(错峰0.11s·命中才跳伤害·每张触发多重打击B·扣'七段')
		_pending_shots.append({"delay": float(i) * 0.11, "fn": func() -> void: _gambler_sys._gambler_throw_hit(u, tgt, per, true), "src": u})
...
func _sk_gambler_fate_wheel(u: Dictionary) -> void:             # 赌神龟·命运之轮(用户封板·80龟能): 抽1花色永久加属性(♠攻+5&血+30/♥护甲魔抗+2/♦暴击+8%&护穿+2/♣吸血+4%)·跨场累积=存GameState.gambler_wheel_stacks(本大轮累积·切轮重置·方案B·用户2026-07-09)
	...
	_gambler_sys._gambler_wheel_vfx(u, _suit)                # 花色老虎机转盘spin(SPIN秒·EASE_OUT减速落定)
	var uu: Dictionary = u
	var suit: int = _suit
	_pending_shots.append({"delay": SPIN, "fn": func() -> void:   # 花色落定那一刻才结算
		if not uu.get("alive", false): return
		_gambler_sys._gambler_apply_wheel_suit(uu, suit)
		, "src": u})
...
func _sk_gambler_wild(u: Dictionary, tgt: Dictionary) -> void:   # 赌神龟·万能牌: 丢1张牌=1段1.0A物理(用户2026-07-09"只造成1段伤害")+自身0.25A护盾+回5%maxHp+目标攻-15%
	_grant_shield(u, u["atk"] * 0.25)   # 自身护盾/回血即时(施法者)
	_heal(u, u["maxHp"] * 0.05)
	_gambler_sys._gambler_wild_vfx(u, tgt)   # 丢小丑牌; ★目标伤害+减攻在牌命中回调里结算(命中才跳伤害·用户2026-07-14)
...
func _sk_hunter_hide(u: Dictionary) -> void:                     # 猎人龟·隐蔽(封板·薇恩Q Tumble·80龟能): 智能翻滚~250码→下次普攻附带0.9A物理(吃吸血)+25%闪避+0.7A护盾
	...
	_skill_ring(u["pos"], Color(0.45, 1.0, 0.68, 0.6), 60.0)                       # 起手灵巧绿环(闪避激活)
	_burst_vfx("res://assets/sprites/vfx/dust-impact.png", u["pos"], 175.0, 0.12)  # 起跳轻尘
...
func _hunter_roll_best_dest(from2d: Vector2, pref_dir: Vector2) -> Vector2:
	...
	var xlo := ARENA.position.x + pad; var xhi := ARENA.end.x - pad
	var ylo := ARENA.position.y + pad; var yhi := ARENA.end.y - pad
	var best := from2d
...
func _hunter_roll_tick(u: Dictionary, delta: float) -> void:
	...
	var step: float = HUNTER_ROLL_SPD * delta
	...
	if to_d.length() <= step + 2.0 or to_d.dot(rdir) <= 0.0:     # 到达/穿过 → 落地结算
		...
		u["state"] = "move"
		_burst_vfx("res://assets/sprites/vfx/dust-impact.png", u["pos"], 200.0, 0.12)   # 落地轻尘
		return
...
func _hunter_roll_ghost(u: Dictionary) -> void:                 # 单道灵巧绿立绘幻影(在当前位铺·淡出=翻滚拖尾)
	...
	g.offset = spr.offset
	g.position = _world_pos(u["pos"], float(u.get("height", 0.0)))
...
func _sk_hunter_shot(u: Dictionary, tgt) -> void:              # 猎人龟·精准射击(封板·90龟能·射箭+毒箭+猎杀印记三合一): 蓄力狙2.0A物理+中毒5s+治疗削减50%5s+猎杀印记5s(<24%处决)
	...
	_beam_vfx("res://assets/sprites/vfx/fx-energy-beam.png", u["pos"], tref["pos"], 6.0, Color(1.0, 0.35, 0.2, 0.4), 0.4, 1.0)
	_gambler_sys._gambler_pop(u["pos"], float(u.get("height", 0.0)) + 0.5, Color(1.0, 0.45, 0.2, 0.7))   # 蓄力红光(复用通用glow pop)
	# ② 0.4s后开火: 金色狙击曳光 → 到位(0.16s)才结算伤害+毒+印记+爆(命中才跳伤害)
	_pending_shots.append({"delay": 0.4, "fn": func() -> void:
		if not uu.get("alive", false) or not tref.get("alive", false): return
		var tp: Vector2 = tref["pos"]
		_beam_vfx("res://assets/sprites/vfx/fx-energy-beam.png", uu["pos"], tp, 18.0, Color(1.0, 0.9, 0.45, 0.9), 0.18, 1.0)   # 金色狙击曳光(粗亮快)
		_shake(0.06)
		_pending_shots.append({"delay": 0.16, "fn": func() -> void:
			if not tref.get("alive", false): return
			_apply_damage_from(uu, tref, _atk_dmg(uu, 2.0, tref), Color("#ff4444"))   # 物理红(2.0A物理狙击·飘字色规范)
			_apply_dot_stacks(tref, "poison", maxi(1, int(round(uu["atk"] * 0.5))), uu)   # 中毒5s
			tref["heal_reduce_until"] = _t + 5.0                         # 治疗削减50%·5秒
			tref["heal_reduce_pct"] = maxf(float(tref.get("heal_reduce_pct", 0.0)), 0.5)
			tref["hunt_mark_until"] = _t + 5.0                           # 猎杀印记5秒: <24%即处决(头顶状态图标行贴十字准星·_layout_head_badges·不用文字)
			_skill_ring(tref["pos"], Color(0.5, 0.9, 0.2, 0.6), 40.0)   # 命中毒绿环(小·非大绿球)
			for _pb in range(4): _spawn_poison_bubble(tref)             # 命中即冒一簇毒泡
			_shake(JUICE_SHAKE_HEAVY)
			, "src": uu})
		, "src": u})
...
func _sk_hunter_barrage(u: Dictionary, _tgt) -> void:          # 猎人龟·狩猎弹幕(封板·100龟能): 10箭·每0.2s一发·随机目标(注意龟蛋)·慢速抛物线追踪·箭头随角度·命中才跳0.36A真实(共3.6A)·每箭<14%即处决(用户2026-07-14重做)
	for i in range(10):
		var uu: Dictionary = u
		_pending_shots.append({"delay": float(i) * 0.2, "src": u, "fn": func() -> void:   # 每0.2s射一发
			if not uu.get("alive", false): return
			var cand: Array = []                                # 随机目标(非最残血): _enemies_of已跳围栏未破的蛋; 破栏后的蛋是合法目标(注意龟蛋)
			for o in _pick_enemies_of(uu):
...
func _fire_hunter_arrow(u: Dictionary, tgt: Dictionary, dmg: int) -> void:   # 慢速抛物线追踪箭(箭头随行进角度转·命中才跳白色真伤·处决由中央路径判egg免疫)
	...
	p.shaded = false; p.transparent = true
	p.pixel_size = (66.0 * WS) / float(fh)
	var from2d: Vector2 = u["pos"]
	p.position = _world_pos(from2d, 1.2)
	_world.add_child(p)
	_projectiles.append({
...
func _step_homing_arrow(pr: Dictionary, node: Sprite3D, delta: float) -> bool:   # 追踪抛物箭逐帧: 追踪移动+抛物高度+箭头随角度; 命中/目标消失→自销返false, 否则返true续飞
	...
	node.position = _world_pos(cur2d, h)
	if _cam != null:   # 箭头随角度: 屏幕空间速度方向(含上下弧)→in-plane roll(箭尖上扬/下俯)
		var sv: Vector2 = _cam.unproject_position(node.global_position) - _cam.unproject_position(prev_world)
		if sv.length() > 0.4:
...
func _hunter_execute_fx(u: Dictionary) -> void:   # 被动猎杀·处决瞬间: 金色斩杀爆(环+金光pop+轻震)·配"处决!"金字(用户2026-07-14)
	...
	_gambler_sys._gambler_pop(u["pos"], float(u.get("height", 0.0)) + 0.5, Color(1.0, 0.86, 0.3, 0.85))
	_shake(0.05)
...
## 正卡在 14% 处决线下, 猎人每 0.1s 扫场 → 射处决箭 → `_hunter_exec_arrow_hit` 直接 hp=0 + _kill,
## 完全绕过 `_apply_damage_from` 里的 `_assembling` 免疫闸。黑洞中/滞空中的单位同样能被处决。
...
func _fire_hunter_exec_arrow(u: Dictionary, tgt: Dictionary) -> void:   # 强化箭(金色·更大·更快·小抛物)→命中处决; 复用homing_arc弹道
	...
	p.pixel_size = (88.0 * WS) / float(fh)   # 更大(强化)
	var from2d: Vector2 = u["pos"]
	p.position = _world_pos(from2d, 1.3)
	_world.add_child(p)
	_gambler_sys._gambler_pop(from2d, float(u.get("height", 0.0)) + 0.6, Color(1.0, 0.86, 0.3, 0.8))   # 金色蓄力枪口光
	_projectiles.append({
...
func _hunter_exec_arrow_hit(src, tgt) -> void:   # 强化箭命中: 目标仍<斩杀线且非免疫→处决+窃取(demo靶不真死复位)
	...
	if tgt.get("egg", false) or tgt.get("_eggImmune", false) or tgt.get("eq_exec_immune", false) or _is_untargetable(tgt):   # 免疫处决: 蛋/不沉之锚/不可选(含机甲组装期) → 命中但不斩
		_hit_spark(tgt); return
	var thr: float = 0.24 if _t < float(tgt.get("hunt_mark_until", 0.0)) else 0.14
	...
	if tgt.get("_hunt_demo_victim", false):   # 被动demo靶: 处决→窃取+复位残血(循环看·在deathfloor判前, demo也带deathfloor)
		...
		_float_text(tgt["pos"] + Vector2(0, -40), "处决!", Color("#ffd700"))
		_hunter_apply_steal(src, tgt)
		tgt["hp"] = float(tgt["maxHp"]) * 0.12
		return
	if float(tgt.get("deathfloor_until", 0.0)) > _t:   # 临时免死(亡灵等)→免疫处决(命中但不斩)
		_hit_spark(tgt); return
	_hunter_execute_fx(tgt)
	_float_text(tgt["pos"] + Vector2(0, -40), "处决!", Color("#ffd700"))
	tgt["hp"] = 0.0
...
func _sk_pirate_rum(u: Dictionary) -> void:                     # 海盗龟·朗姆酒(120龟能): 海盗船扔酒瓶→每秒回4%maxHP×6秒(HoT绿回血) + 护甲魔抗各+0.15A×6秒(暖色酒气护光)
	u["rum_until"] = _t + 6.0; u["rum_dps"] = u["maxHp"] * 0.04   # 每秒回4%maxHP×6秒(分秒HoT·per-frame _heal结算)
	...
	_pirate_rum_bottle(ship2d, ship_h, u["pos"], func() -> void:   # 酒瓶到手: 饮酒琥珀爆+环
		if not uu.get("alive", false): return
		_gambler_sys._gambler_pop(uu["pos"], float(uu.get("height", 0.0)) + 0.5, Color(0.95, 0.62, 0.25, 0.85))
...
func _pirate_rum_bubble(u: Dictionary) -> void:   # 酒气暖泡(HoT期上飘·暖琥珀·区别中毒绿)
	...
	var spr := _glow_bb(pos2d, 0.4, 34.0, Color(0.95, 0.68, 0.3, 0.85))
	var tw := _reg_tween(); tw.set_parallel(true)
	tw.tween_property(spr, "position", _world_pos(pos2d, 1.1), 0.6)
...
func _pirate_get_ship(u: Dictionary) -> Sprite3D:   # 该海盗的持久演出船(一只·首次建后驻场·火炮/朗姆/登场轰击共用·不重复生成不淡出·用户2026-07-14"船是一只在的")
	...
	var acx: float = (ARENA.position.x + ARENA.end.x) * 0.5
	var sx: float = acx + (-230.0 if side == "left" else 230.0)
	var ship2d := Vector2(clampf(sx, ARENA.position.x + 150.0, ARENA.end.x - 150.0), ARENA.position.y + 55.0)   # 驻场后方
...
func _sk_bubble_shield(u: Dictionary, _tgt: Dictionary) -> void: # 泡泡龟·泡泡盾(封板L435·80龟能): 给最脆友军1.8A泡泡盾(4秒)·到期/被打破/挂盾对象死→爆裂对施法者全体敌2.0A魔法
	...
	if ally == null: ally = u
	_bubble_sys._bubble_shield_burst(ally)                       # 若该对象已挂着上一发泡泡盾未爆→先结算(防覆盖丢爆裂)
	...
	u["energy_lock_until"] = _t + 4.0                # 泡泡盾期间锁龟能(用户2026-07-15·盾在=不充能不放其它·爆裂时解锁·复用energy_lock_until)
	...
	if dtex != null:
		...
		dome.pixel_size = (108.0 * WS) / float(maxi(1, dtex.get_height()))
		dome.modulate = Color(0.75, 0.92, 1.0, 0.0)
		dome.position = _world_pos(ally["pos"], 0.9)
		_world.add_child(dome)
		_follow_vfx.append({"spr": dome, "unit": ally, "h": 0.9})
		var tw := _reg_tween()
...
func _sk_bubble_burst(u: Dictionary, tgt) -> void:              # 泡泡龟·泡泡爆破(马尔扎哈Q式·用户设计): 消耗当前泡泡值40%→目标两侧泡沫门·门间敌每个受=消耗泡泡值魔法+0.8A物理(无沉默)
	...
	_bubble_sys._bubble_gate(g1)                                            # ① 先现两门(portals)
	_bubble_sys._bubble_gate(g2)
	...
	wt.tween_callback(func() -> void:                          # ③ 门间竖起泡沫墙(从中心向两端展开·每点从地面猛涌起) + 墙起时命中结算
		for k in range(20):
			...
			_bubble_sys._bubble_wall_erupt(gg1.lerp(gg2, t), absf(t - 0.5) * 0.12)
		_skill_ring(cc, Color(0.5, 0.9, 1.0, 0.6), 200.0)
		if not uu.get("alive", false): return
		for o in _enemies_of(uu):
			if o.get("alive", false) and o["pos"].distance_to(cc) <= 200.0:   # 门间200码带内敌
				_apply_damage_from(uu, o, int(_mitigate(uu, cons, o, true)) + _atk_dmg(uu, 0.8, o), Color("#cdebff")))   # 消耗泡泡值魔法(吃魔抗)+0.8A物理(封板L437)
...
func _sk_rainbow_storm(u: Dictionary) -> void:                  # 彩虹龟·全色风暴 (重做2026-07-13: AI棱镜漩涡+GPU彩虹粒子+碎甲可视化-20%护甲魔抗+亮边AOE·期间锁龟能)
	...
	if vtex != null:
		...
		pivot.position = _world_pos(center, 0.06)
		_world.add_child(pivot)
		var mi := MeshInstance3D.new()
		var q := QuadMesh.new(); q.size = Vector2(radius * 2.3 * WS, radius * 2.3 * WS)
		mi.mesh = q
		var m := StandardMaterial3D.new()
		...
		pivot.add_child(mi)
		var fin := _reg_tween(); fin.tween_property(mi, "material_override:albedo_color", Color(1, 1, 1, 0.92), 0.3)
		var spin := _reg_tween(); spin.tween_property(pivot, "rotation:y", -TAU * 3.0, 4.0)   # 4s转3圈
		nodes.append(pivot)
	# ② 亮边圈(AOE边界·贴地)
	...
	rim.position = _world_pos(center, 0.05)
	rim.pixel_size = (radius * 2.0 * WS) / 96.0
	_world.add_child(rim)
	nodes.append(rim)
	# ③ GPU彩虹粒子(圆盘内上升·全光谱ramp·240颗=暴风能量/体积)
	var ps := _storm_particles(center, radius)
	if ps != null: nodes.append(ps)
	u["storm_nodes"] = nodes
	var tw := _reg_tween()
	for i in range(8):   # 4秒 / 每0.5秒 = 8跳
		tw.tween_callback(_rainbow_sys._rainbow_storm_tick.bind(u, center, radius, i))
		tw.tween_interval(0.5)
	tw.tween_callback(_rainbow_sys._rainbow_storm_end.bind(u))
...
func _sk_fortune_buyequip(u: Dictionary) -> void:              # 财神龟·招财进宝(封板·60龟能起): 首抽1件1/2/3费临时装备(1★·战后消失不占槽)→消耗变160/240/460; 后续释放升1星(精确数值delta) → 3★后消耗回60且每次释放回复1×ATK生命
	...
	if star == 0:                                               # 首抽临时装备
		var iid: String = _chest_sys._chest_pick_equip([1, 2, 3])
		if iid == "": return
		if not u.has("equips"): u["equips"] = []
		u["equips"].append({"id": iid, "star": 1})
		_equip_sys._eq_apply_one_stats(u, iid, 1)
		...
		_equip_sys._eq_star_delta_stats(u, iid2, star, star + 1)          # 精确升星: 加(新星-旧星)属性差量(flag类缩放留F5)
...
func _sk_lightning_shield(u: Dictionary) -> void:              # 闪电龟·雷盾 (用户2026-07-07: 3ATK护盾5秒, 盾在时反击0.1A魔法叠电击=见_apply_damage_from; 2026-07-15补以雷电包裹自身VFX)
	...
	_aura_vfx("res://assets/sprites/skills/lightning-3.png", u, 58.0, Color(0.45, 0.85, 1.0, 0.5), 5.0)   # 脚下电爆光环随身5秒
	...
	if etex != null:
		...
		for i in range(n):
			...
			s.pixel_size = (42.0 * WS) / float(maxi(1, etex.get_height()))
			s.position = _world_pos(u["pos"], float(u.get("height", 0.0)) + 0.7)
			_world.add_child(s)
			_follow_vfx.append({"spr": s, "unit": u, "h": 0.7, "orbit_r": 0.30, "orbit_a": float(i) * TAU / float(n), "orbit_spd": 6.0})
			var tw := _reg_tween()
...
func _sk_rainbow_reflect(u: Dictionary) -> void:               # 彩虹龟·反射弹射 (重做2026-07-13: 棱镜光束敌我间反射弹跳·治友绿/伤敌全色魔法)
	...
	for i in range(6):
		...
		var _col: Color = _PRISM_RAINBOW[i]
		var tw := _reg_tween()
		tw.tween_interval(float(i) * 0.22)      # 逐段错峰=看得见弹射依次跳(0.09→0.22减慢·用户2026-07-14)
		tw.tween_callback(func() -> void:
			_beam_vfx("res://assets/sprites/vfx/fx-energy-beam.png", from_p, tp, 30.0, Color(_col.r, _col.g, _col.b, 0.9), 0.55, 0.9)   # 棱镜反射光束(当跳色·时长0.3→0.55更慢更看得清)
			if _ally:
				if tref.get("alive", false): _heal(tref, u["atk"] * 0.5)
				_reflect_pop(tp, float(tref.get("height", 0.0)), Color(0.3, 1.0, 0.5, 0.9))    # 治友=绿pop
			else:
				if tref.get("alive", false): _apply_damage_from(u, tref, _atk_dmg(u, 0.5, tref, true), Color("#ff8ad8"))
				_reflect_pop(tp, float(tref.get("height", 0.0)), Color(_col.r, _col.g, _col.b, 0.95)))   # 伤敌=当跳色pop
...
func _sk_lightning_surge(u: Dictionary, tgt: Dictionary) -> void: # 闪电龟·涌动 ✅
	...
	_aura_vfx("res://assets/sprites/skills/lightning-3.png", u, 54.0, Color(0.3, 0.67, 0.97, 0.5), 5.0)   # 涌动增伤电流光环5秒(标示buff激活期)
	_hit_spark(u)                                                                                          # 自身电流上涌
...
func _sk_headless_fear(u: Dictionary, _tgt = null) -> void:      # 无头·恐吓(封板·110龟能): 半径200码内所有敌 定身+缴械+锁技3秒(蛋免控·无伤害); 2026-07-17演出: 咆哮紫黑气爆+三道恐惧波纹+紫雾盘+颤抖恐惧标记(删技能名飘字=UI规矩)
	...
	burst.pixel_size = (120.0 * WS) / 128.0
	burst.modulate = Color(0.55, 0.15, 0.7, 0.9)
	burst.position = _world_pos(cx, 0.8)
	...
	for wi in range(3):                                          # 三道恐惧波纹先后扩到200码(半径)
		...
		wv.pixel_size = (20.0 * WS) / 256.0
		wv.position = _world_pos(cx, 0.065)
		_world.add_child(wv)
		var wt := _reg_tween()
		wt.tween_interval(0.09 * float(wi))
		wt.tween_method(func(q: float) -> void:
			if is_instance_valid(wv):
				wv.pixel_size = (maxf(20.0, 400.0 * q) * WS) / 256.0
	...
	core.texture = mtex
	core.billboard = BaseMaterial3D.BILLBOARD_ENABLED; core.shaded = false; core.transparent = true
	core.pixel_size = (140.0 * WS) / 128.0
	...
	# 贴地黑紫雾盘(范围可视200码)缓散
	var pool := _px_ground_sprite(mtex, cx, 400.0, Color(0.28, 0.07, 0.42, 0.6), 0.06)   # 贴地雾盘更浓
	var pt := _reg_tween()
	...
	for o in _enemies_of(u):
		...
		_stun(o, 3.0, "_sk_headless_fear")
		_headless_sys._headless_fear_mark(o)
...
func _sk_headless_tendrils(u: Dictionary, _tgt = null) -> void:  # 无头·万千触须(封板·160龟能·虐杀原形毁灭者Q1): 全场无差别触须·伸0.3s→停→收3.0s·自身硬控·+22%吸血; 2026-07-17演出: 起手紫黑气爆+裂纹环→触须8×5网格布满全场(Q9"特效得布满")波前由近及远爆出→痉挛定格→3.0s撕扯缩回+吸血红珠回流
	_stun(u, 4.0, "_sk_headless_tendrils", true)   # 自身硬控全程(施法动作·亡灵拉全场自己也搭进去)
	...
	virus.pixel_size = (120.0 * WS) / float(maxi(1, virus.texture.get_height()))
	virus.modulate = Color(0.12, 0.05, 0.16, 0.0)               # 黑紫病毒球
	virus.position = _world_pos(center, 0.75)
	virus.scale = Vector3.ONE * 0.2
	_world.add_child(virus)
	_follow_vfx.append({"spr": virus, "unit": u, "h": 0.75})    # 跟随龟身(死亡自动清)
	var vgt := _reg_tween()
	...
	var vpt := _reg_tween()                                     # 脉动(变大变小·病毒感)3s
	...
	burst.texture = VfxTex._make_fire_glow_tex()
	burst.billboard = BaseMaterial3D.BILLBOARD_ENABLED; burst.shaded = false; burst.transparent = true
	burst.pixel_size = (200.0 * WS) / 128.0
	burst.modulate = Color(0.6, 0.08, 0.3, 0.98)   # 起手体内生物质暗红爆(2026-07-17)
	burst.position = _world_pos(center, 0.8)
	...
	_shake(0.1)
	_add_hitstop(JUICE_HITSTOP_KNOCK)
	var crack := _px_ground_sprite(VfxTex._make_pixel_ring_tex(), center, 60.0, Color(0.6, 0.2, 0.75, 0.8), 0.06)
	var ct := _reg_tween()
	ct.tween_method(func(q: float) -> void:
		if is_instance_valid(crack): crack.pixel_size = (maxf(60.0, 500.0 * q) * WS) / 48.0
	...
	for ti in range(100):                                       # 100根: 方位角0-360×仰角(偏低角·上半球)
		...
		_headless_sys._headless_tendril_shoot(center, od, elev, L, randf() < 0.4, dly, randf_range(2.0, 2.4))
	var pass_fn := func():                                      # 伸/穿过(≈0.3s铺满): 无差别 敌1A+眩晕 / 友0.5A+眩晕(刷新不叠)
		for o in _units:
			...
			_headless_sys._headless_tendril_shoot(center, ((o["pos"] as Vector2) - center).normalized() if ((o["pos"] as Vector2) - center).length() > 1.0 else Vector2.RIGHT, deg_to_rad(18.0), maxf(90.0, (o["pos"] as Vector2).distance_to(center)), true, 0.0, 2.4)   # 命中者: 从龟身甩一根粗触须缠住
			if _is_hostile(uu, o): _headless_sys._headless_drain_dot(o["pos"], uu)   # 吸血红珠回流
	...
	var detach_fn := func():                                    # 收/脱离(≈3.0s): 无差别 敌1.5A / 友0.5A + 回复行动(解眩晕)
		...
		for o in _units:
			...
			if _is_hostile(uu, o): _headless_sys._headless_drain_dot(o["pos"], uu)   # 脱离撕扯再吸一口
...
func _sk_headless_soul_charge(u: Dictionary) -> void:           # 无头·灵魂打击(机制大改·用户2026-07-17拍板·80龟能): 触发→下3次攻击强化(射程+60·各额外0.5A+10%当前HP魔法·牙齿闭合)→第3下落地蓄力→镰刀横扫(100°300码击退300+幽灵诅咒5s·Camille W); 全程锁龟能, 扫完解锁清零重充
	...
	u["energy_lock_until"] = _t + 999.0                            # 锁龟能条到镰刀扫完(_headless_scythe置回_t)
	...
	fl.pixel_size = (34.0 * WS) / 128.0
	fl.modulate = Color(0.75, 0.3, 1.0, 0.9)
	fl.position = _world_pos(u["pos"], 1.5)
	_world.add_child(fl)
	u["_soul_spr"] = fl
	var uref: Dictionary = u
	var lt := create_tween().set_loops()
	lt.bind_node(fl)
	lt.tween_method(func(q: float) -> void:
		if not is_instance_valid(fl): return
		if uref.get("alive", false):
			var aa: float = q * TAU
			fl.position = _world_pos((uref["pos"] as Vector2) + Vector2(cos(aa), sin(aa)) * 26.0, 1.5 + sin(aa * 2.0) * 0.12)
	, 0.0, 1.0, 0.9)
	_skill_ring(u["pos"], Color(0.6, 0.23, 0.7, 0.5), 44.0)
	_pending_shots.append({"delay": 7.0, "fn": func():            # 兜底: 7秒内没打完3下(无敌/被控)→强制镰刀收尾防永锁龟能
		if int(uref.get("headless_soul_stacks", 0)) > 0: _headless_sys._headless_scythe(uref), "src": u})
...
func _sk_crystal_bulwark(u: Dictionary) -> void:                 # 水晶龟·水晶壁垒(用户2026-07-16改制): 1A+5%maxHp护盾4秒·持盾锁龟能·盾结束/被打破→700码直线水晶刺·全体友军甲抗+15%4秒
	_grant_shield(u, u["atk"] * 1.0 + u["maxHp"] * 0.05, 4.0)
	u["bulwark_until"] = _t + 4.0
	...
	if dtex != null:
		dome.texture = dtex
		dome.billboard = BaseMaterial3D.BILLBOARD_ENABLED; dome.shaded = false; dome.transparent = true
		dome.pixel_size = (120.0 * WS) / float(maxi(1, dtex.get_height()))
		dome.modulate = Color(0.62, 0.88, 1.0, 0.0)
		dome.position = _world_pos(u["pos"], 0.9)
		_world.add_child(dome)
		_follow_vfx.append({"spr": dome, "unit": u, "h": 0.9})
	...
	for o in _allies_of(u):
		...
		_skill_ring(o["pos"], Color(0.62, 0.88, 1.0, 0.55), 40.0)   # 友军冰蓝强化环
...
func _sk_crystal_burst(u: Dictionary, tgt) -> void:   # 碎晶爆破: 目标周围350码内每敌3段错峰碎晶坠落(每段0.233A魔+0.033A真+叠1层结晶·共3层满5引爆)
	...
	u["energy_lock_until"] = maxf(float(u.get("energy_lock_until", 0.0)), _t + 0.65)   # 释放途中锁龟能·放完自动解锁(用户2026-07-16; 三段0.38s+坠落0.16s≈0.55s)
	_skill_ring(center, Color(0.6, 0.86, 1.0, 0.5), CRYSTAL_BURST_RADIUS)   # 350码范围环
	var uu := u
	for o in _enemies_of(u):
		if not o.get("alive", false): continue
		if (o["pos"] as Vector2).distance_to(center) > CRYSTAL_BURST_RADIUS: continue
		var oref: Dictionary = o
		for k in range(3):                                       # 3段碎晶从天坠落(错峰0.14s)
			_pending_shots.append({"delay": 0.1 + 0.14 * float(k), "fn": func() -> void:
				if not oref.get("alive", false) or not uu.get("alive", false): return
				var sh := Sprite3D.new()
				sh.texture = load("res://assets/sprites/vfx/crystal-shard.png")
				sh.texture_filter = BaseMaterial3D.TEXTURE_FILTER_NEAREST
				sh.billboard = BaseMaterial3D.BILLBOARD_ENABLED; sh.shaded = false; sh.transparent = true
				sh.pixel_size = (30.0 * WS) / 48.0
				sh.flip_h = randf() < 0.5
				var op2: Vector2 = oref["pos"] + Vector2(randf_range(-14.0, 14.0), randf_range(-8.0, 8.0))
				sh.position = _world_pos(op2, 2.4)
				_world.add_child(sh)
				var dt := _reg_tween()
				dt.tween_property(sh, "position", _world_pos(op2, 0.55), 0.16).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN)   # 坠落
				dt.tween_callback(func() -> void:
					if is_instance_valid(sh): sh.queue_free()
					if not oref.get("alive", false): return
					_crystal_sys._crystal_spark(oref["pos"], 0.7)
					_apply_damage_from(uu, oref, _atk_dmg(uu, 0.233, oref, true), Color("#9bdcff"))                 # 每段0.233A魔法
					_apply_damage_from(uu, oref, int(maxf(1.0, uu["atk"] * 0.033)), Color("#ffffff"), 0.0, true)   # +0.033A真实
					_crystal_sys._crystal_stack(uu, oref, 1))
			, "src": u})

func _sk_crystal_orb(u: Dictionary, tgt) -> void:
	if tgt == null or not tgt.get("alive", false):
		return
	_skill_ring(u["pos"], Color(0.72, 0.92, 1.0, 0.6), 34.0)     # 本体施法环
	_crystal_sys._crystal_ray_vfx(u, tgt, func(sr: Dictionary, tr: Dictionary, last: bool) -> void:
		_apply_damage_from(sr, tr, _atk_dmg(sr, 0.5, tr, true), Color("#9bdcff"), 0.0, true)   # 每段0.5A魔法(raw避免二次减免·封板)
		if last: _crystal_sys._crystal_stack(sr, tr, 2))                      # 末段叠2层结晶(封板)
...
func _elite_try_consume(u: Dictionary, tgt: Dictionary) -> bool:   # 被动1·吞噬(Consume·用户拍板: 1.5s演出不中断/可被攻击/回复=触发瞬间目标剩余HP/只偷主动技)
	...
	_stun(tgt, 1.7, "_elite_try_consume", true)
	tgt["untargetable_until"] = maxf(float(tgt.get("untargetable_until", 0.0)), _t + 1.7)
	...
	_pending_shots.append({"delay": 1.5, "src": u, "fn": func() -> void:
	...
	for k in range(3):                                            # 黑红卷须×3绕目标螺旋收紧(40→8码)
		...
		td.pixel_size = (randf_range(20.0, 30.0) * WS) / float(maxi(1, glow.get_width()))
		td.modulate = Color(0.15, 0.02, 0.03, 0.85) if k % 2 == 0 else Color(0.5, 0.07, 0.09, 0.7)
		td.position = _world_pos(tgt["pos"], 0.5)
		_world.add_child(td)
		tends.append(td)
	...
	drv.tween_method(func(q: float) -> void:
		...
		for i2 in range(tends.size()):
			...
			td2.position = _world_pos((tref["pos"] as Vector2) + Vector2(cos(aa), sin(aa)) * rr, 0.35 + 0.3 * (1.0 + sin(aa * 0.7)) * 0.5)
	, 0.0, 1.0, 1.45)
	_pending_shots.append({"delay": 1.5, "fn": func() -> void:   # 吞噬完成: 黑丝缕流入→回血→击杀→偷放主动技
		for td3 in tends:
		...
		for k3 in range(9):                                       # 黑丝缕×9流入精英体内(原作: 整个人化作黑丝缕被吸收)
			...
			strand.pixel_size = (randf_range(10.0, 18.0) * WS) / float(maxi(1, glow.get_width()))
			strand.modulate = Color(0.4, 0.05, 0.07, 0.9)
			var sp0: Vector2 = tp + Vector2(randf_range(-26.0, 26.0), randf_range(-16.0, 16.0))
			strand.position = _world_pos(sp0, randf_range(0.3, 0.9))
			_world.add_child(strand)
			var stw := _reg_tween(); stw.set_parallel(true)
			stw.tween_property(strand, "position", _world_pos(uu["pos"] as Vector2, 0.6), randf_range(0.22, 0.36)).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN)
		...
		if tref.get("_consume_demo", false) and tref.get("alive", false):   # 评审demo: 复活18%循环看
			...
			_kill(tref, uu)                                        # 正常击杀管线(统计/死亡钩子照走)
		_heal(uu, heal_amt)                                        # 回复=吞噬瞬间目标剩余HP×2(用户2026-07-19)
		uu["haste_until"] = _t + 5.0; uu["haste_mult"] = 1.5       # 吞噬完成: 5秒+50%攻速(用户2026-07-19)
		_float_text(uu["pos"] + Vector2(0, -62), "吞噬! +50%攻速", Color("#ff6a6a"))
		_flash(uu, Color(1.5, 0.95, 0.95))
		_skill_ring(uu["pos"] as Vector2, Color(0.55, 0.08, 0.1, 0.7), 46.0)
		if stype != "":                                            # 以自己属性立即放一次目标主动技(用户拍板: 只放主动)
			_pending_shots.append({"delay": 0.35, "fn": func() -> void:
				if not uu.get("alive", false): return
				var et = _acquire_target(uu)
				if et == null: return
				_elite_mist(uu["pos"] as Vector2, 0.8, 3)
				_do_skill(uu, et, stype)
			, "src": uu})
	, "src": u})
	return true
...
func _elite_steal_skill_type(tref: Dictionary) -> String:        # 吞噬偷技: 目标主动技(passiveSkill不放·未实装不放·无技能不放) — 吞噬小将→放小将技=设计正确(用户2026-07-18确认非bug·撤回上次误加的排除)
	for a in tref.get("active_skills", []):
		var stype := str(a)
		if stype == "": continue
		if bool((_skill_meta.get(stype, {}) as Dictionary).get("passiveSkill", false)): continue
...
func _tick_elite_whip(u: Dictionary) -> void:                    # 被动2·铁锁(Whipfist Longshot·CD5s用户拍板): 索敌 150~350码之间→链射→顿+目标眩晕0.4s→拉体落身后+1A魔法(用户2026-07-18: 触发范围>350改为150~350码之间)
	...
	if d >= 150.0 and d <= 350.0 and _elite_try_consume(u, tgt):
	...
	u["_slam"] = true
	_elite_anim(u, "whip")   # 甩臂动作 (挂在真链射这句, 不能挂函数头 —— 那里每帧都跑)
	var uu := u
	var tref: Dictionary = tgt
...
func _sk_minion_rocket(u: Dictionary, tgt) -> void:   # 远程小将·追踪火箭筒: 蓄力1.5s(枪口聚能)→枪口喷火发射→定向慢速追踪导弹(尾焰)→命中核爆(400码4A物理+4s50%治疗削减)
	...
	_rocket_sys._rocket_charge_vfx(uu, aim)                         # 蓄力表现(用户2026-07-18): 枪口聚能球渐大转白热+火星汇入+脚下能量环脉冲
	_pending_shots.append({"delay": 1.5, "fn": func() -> void:   # 蓄力1.5s→发射
		uu["_slam"] = false
		if not uu.get("alive", false): return
		...
		_rocket_sys._rocket_muzzle_flash(uu, muzzle, aim2)          # 枪口特效动画(用户2026-07-18): 白热闪+锥形喷火+火星前喷+后坐
...
func _minion_rocket_fly(u: Dictionary, tref: Dictionary, from: Vector2) -> void:   # 定向慢速追踪导弹(真导弹立绘·机头领飞+尾焰·−30%速)→命中/超时核爆
	...
	miss.pixel_size = (74.0 * WS) / float(maxi(1, (miss.texture as Texture2D).get_width()))   # 按弹长~74码(横向立绘按宽定)
	miss.modulate = Color(1.0, 1.0, 1.0, 1.0)
	var pos: Vector2 = from
	var mh: float = 0.95
	miss.position = _world_pos(pos, mh)
	_world.add_child(miss)
	var halo: MeshInstance3D = _glow_bb(pos, mh, 42.0, Color(1.0, 0.58, 0.26, 0.7))   # 弹头发光晕(加性·跟随脉动)
	var flew: float = 0.0
	...
	while flew < 2400.0 and is_instance_valid(miss) and is_inside_tree():
		...
		pos += dir * step
		miss.position = _world_pos(pos, mh)
		var travel_world: Vector3 = _world_pos(pos + dir, mh) - _world_pos(pos, mh)
		_orient_billboard_dir(miss, travel_world)       # 尖头领着飞(朝屏幕行进方向)
		...
		if is_instance_valid(halo):
			halo.position = _world_pos(pos, mh)
			halo.scale = Vector3.ONE * (0.9 + 0.18 * sin(puls * 32.0))
		trailc += dt
		if trailc >= 0.022:                             # 尾焰(推进尾迹)
			trailc = 0.0
			_rocket_sys._rocket_exhaust_puff(pos - dir * 14.0, mh, dir)
...
func _minion_rocket_nuke(u: Dictionary, center: Vector2) -> void:   # 核爆(用户2026-07-18"命中不够惊艳·再设计"): 全屏白闪+重震顿帧+白热核+膨胀火球+3冲击环+放射火焰+火花碎石+烟柱焦痕; 400码4A物理+4s50%治疗削减
	...
	# ① 撞击拍: 全屏白闪 + 重震 + 顿帧
	_screen_flash_light()
	_shake(0.4); _add_hitstop(0.09)
	# ② 白热核心闪(瞬爆特大→收·引爆过曝)
	var core := Sprite3D.new()
	core.texture = VfxTex._make_fire_glow_tex()
	core.billboard = BaseMaterial3D.BILLBOARD_ENABLED; core.shaded = false; core.transparent = true
	core.pixel_size = (128.0 * WS) / 128.0
	core.modulate = Color(1.0, 1.0, 0.98, 0.0)
	core.position = _world_pos(center, 1.0); core.scale = Vector3.ONE * 0.5
	_world.add_child(core)
	...
	ball.pixel_size = (128.0 * WS) / 128.0
	ball.modulate = Color(1.0, 0.9, 0.55, 0.0)
	ball.position = _world_pos(center, 0.9); ball.scale = Vector3.ONE * 0.4
	_world.add_child(ball)
	var bt := _reg_tween()
	...
	# ④ 冲击波环×3(白快·橙中·焦痕慢)
	_rocket_sys._rocket_shock_ring(center, Color(1.0, 0.97, 0.9, 0.9), 430.0, 0.3)
	_rocket_sys._rocket_shock_ring(center, Color(1.0, 0.7, 0.32, 0.7), 400.0, 0.42)
	_pending_shots.append({"delay": 0.05, "src": u, "fn": func() -> void: _rocket_sys._rocket_shock_ring(center, Color(0.9, 0.55, 0.25, 0.45), 300.0, 0.5)})
	# ⑤ 放射火焰streak×12(向外快闪)
	for k in range(12):
		var ang: float = TAU * float(k) / 12.0 + 0.26
		var d2: Vector2 = Vector2(cos(ang), sin(ang))
		_bolt_line(center + d2 * 30.0, center + d2 * randf_range(150.0, 240.0), Color(1.0, 0.75, 0.4, 0.8))
	# ⑥ 火花四溅 + 碎石 + 烟柱 + 焦痕
	_impact_particles(center, 0.4)
	_impact_particles(center, 1.6)
	_slam_debris(center)
	_rocket_sys._rocket_smoke_plume(center)
	_rocket_sys._rocket_scorch(center)
	# ⑦ 结算伤害(数值/机制封板不动)
	_skill_ring(center, Color(1.0, 0.7, 0.3, 0.75), 400.0)
	for o in _enemies_of(u):
		if not o.get("alive", false): continue
		if (o["pos"] as Vector2).distance_to(center) > 400.0: continue
		_hit_spark(o)
		_apply_damage_from(u, o, _atk_dmg(u, 4.0, o, false), Color("#ff4444"))   # 4A物理
		o["heal_reduce_until"] = maxf(float(o.get("heal_reduce_until", 0.0)), _t + 4.0)   # 4秒50%治疗削减
		o["heal_reduce_pct"] = maxf(float(o.get("heal_reduce_pct", 0.0)), 0.5)
...
func _sk_minion_bodysurf(u: Dictionary, tgt) -> void:   # 近战小将·人体浪板: 高跳回2A血→(太近后跳)→射铁链晕→拉己向目标→接触10%目标maxHP物理→踩滑(对被踩逐渐2A物理·沿途1.5A+击退)→跳下
	...
	var tref: Dictionary = tgt
	uu["_slam"] = true
	_melee_anim(uu, "leap")                              # 0.00-0.64 蓄力+蹬地起跳
	_heal(uu, float(uu.get("atk", 30.0)) * 2.0)          # 高跳: 回复2ATK生命
	_skill_ring(uu["pos"], Color(0.7, 0.85, 1.0, 0.5), 40.0)
	...
	apex.x = clampf(apex.x, ARENA.position.x + 30.0, ARENA.end.x - 30.0)
	apex.y = clampf(apex.y, ARENA.position.y + 20.0, ARENA.end.y - 20.0)
	var jt := _reg_tween()
	...
	, 0.0, 1.0, 0.34)
	_pending_shots.append({"delay": 0.68, "fn": func() -> void:   # 到顶后→从小将射铁链向目标+晕住顿一下(蓄力变久后挪·2026-07-18)
		if not (uu.get("alive", false) and tref.get("alive", false)):
			uu["_slam"] = false; return
		_melee_anim(uu, "throw")                          # 0.68 滞空甩索(与射链同帧)
		_surf_chain_shoot(uu["pos"], float(uu.get("height", 0.0)) + 0.6, tref["pos"], Color(0.6, 0.15, 0.15, 0.95))   # 从小将(空中·当前height)射铁链向目标
		_stun(tref, 1.1, "_sk_minion_bodysurf", true)   # 晕住覆盖整个空中顿+俯冲
		_hit_spark(tref)
	, "src": u})
	_pending_shots.append({"delay": 1.28, "fn": func() -> void:   # 空中顿~0.6s(0.68射链→1.28拉)→拉己俯冲向目标→接触→踩滑(coroutine·用户2026-07-18)
		if not (uu.get("alive", false) and tref.get("alive", false)):
			uu["_slam"] = false; return
		_melee_anim(uu, "dive")                           # 1.28-1.58 被绳拉着俯冲, 双脚前伸
...
func _minion_bodysurf_ride(u: Dictionary, tref: Dictionary) -> void:   # 拉己向目标→接触10%maxHP→踩着目标滑行(coroutine)
	...
	_apply_damage_from(u, tref, int(float(tref["maxHp"]) * 0.10), Color("#ff4444"))   # 接触: 10%目标最大生命(物理)
	_melee_anim(u, "surf")                                # 1.58-2.41 踩在目标身上滑行
	_hit_spark(tref); _shake(0.14)
	...
	slide_to.x = clampf(slide_to.x, ARENA.position.x + 30.0, ARENA.end.x - 30.0)
	slide_to.y = clampf(slide_to.y, ARENA.position.y + 20.0, ARENA.end.y - 20.0)
	...
	while sd < slide_dur and u.get("alive", false) and tref.get("alive", false):
		...
		if dot_t >= 0.1 and tref.get("alive", false):      # 对被踩目标逐渐2A物理(分摊·敌死则循环退出=自动结束·参考"血见底自动结束")
			dot_t -= 0.1
			_apply_damage_from(u, tref, _atk_dmg(u, 2.0 * 0.1 / slide_dur, tref, false), Color("#ff4444"))
		for o in _enemies_of(u):                           # 沿途撞到的其他敌: 1.5A物理+撞飞(参考"撞飞其他敌")
			if is_same(o, tref) or not o.get("alive", false) or _arr_has_unit(hit_others, o): continue
			if (o["pos"] as Vector2).distance_to(mypos) <= 70.0:
				hit_others.append(o)
				_apply_damage_from(u, o, _atk_dmg(u, 1.5, o, false), Color("#ff4444"))
				_knockback(u, o, 150.0, 1.5, 1.1)          # 撞飞(vy×1.5腾空)
	_melee_anim(u, "land")                                # 2.41 侧跳→单膝落地
	...
	land_to.x = clampf(land_to.x, ARENA.position.x + 30.0, ARENA.end.x - 30.0)
	land_to.y = clampf(land_to.y, ARENA.position.y + 20.0, ARENA.end.y - 20.0)
	var ho := _reg_tween()
		...
		for step in range(8):
			...
			_pending_shots.append({"delay": 0.43 + 0.052 * float(step + 1), "fn": func() -> void:
				...
				for o in _units:
					...
					o["_espk_tok"] = tok
					_apply_damage_from(uu, o, _atk_dmg(uu, 4.0, o, true), Color("#9bdcff"))   # 普通施法 1.5→4.0 ATK魔法(用户2026-07-19)
			...
			for step2 in range(10):
				...
				_pending_shots.append({"delay": 0.05 * float(step2 + 1), "fn": func() -> void:
					...
					for o2 in _units:
						...
						o2["_espk_tok"] = tok2
						_apply_damage_from(uu, o2, _atk_dmg(uu, 6.0, o2, true), Color("#9bdcff"))   # 第三段 3.0→6.0 ATK魔法(用户2026-07-19)
		...
		var fn := func():
			...
			for o in _enemies_of(uu):
				if o.get("alive", false) and o["pos"].distance_to(center) <= 400.0:
					...
					_chest_sys._chest_coin_spray(o["pos"], 2)                                              # 敌身金币被抽飞
...
func _sk_candy_barrage(u: Dictionary, tgt) -> void:            # 糖果龟·技能二糖衣炮弹(封板·120龟能): 敌最密集区降糖衣炮弹雨8跳·可见糖弹从天落下·落点局部命中(用户2026-07-14全套标准)·友2%maxHp盾/敌0.2A+2%maxHp魔法+减速20%
	...
	for i in range(8):                                          # 8跳·每跳8颗糖弹密集散落该区·落点局部结算(用户2026-07-15两次加密→3→6→8颗+炮弹/糖爆调大)
		_pending_shots.append({"delay": float(i) * 0.42, "src": u, "fn": func() -> void:
			for b in range(8):
				var ang := randf() * TAU
				var land: Vector2 = ctr + Vector2(cos(ang), sin(ang)) * randf_range(15.0, 430.0)
				_candy_shell_drop(land, func() -> void:          # 糖弹落地: 大糖爆+落点120码局部结算
					_burst_vfx("res://assets/sprites/vfx/candy-burst.png", land, 185.0, 0.3)
					for o in _units:
						if not o.get("alive", false): continue
...
func _candy_shell_drop(land2d: Vector2, on_land: Callable) -> void:   # 糖衣炮弹: 从高空翻滚落下到落点→自销调on_land
	...
	s.pixel_size = (58.0 * WS) / float(maxi(1, tex.get_height()))   # 炮弹调大(用户2026-07-15)
	var from_h := 8.5
	s.position = _world_pos(land2d, from_h)
	_world.add_child(s)
	var tw := _reg_tween()
	tw.tween_method(func(p: float) -> void:
		if not is_instance_valid(s): return
		s.position = _world_pos(land2d, maxf(0.2, from_h * (1.0 - p * p)))   # 重力加速下落
		if _cam != null:
			var tf := s.global_transform
			tf.basis = _cam.global_transform.basis * Basis(Vector3(0, 0, 1), p * TAU * 2.0)   # 翻滚
...
func _sk_candy_bomb_feed(u: Dictionary) -> void:               # 糖果龟·技能三糖果炸弹(封板·喂续命): 炸弹活→上限+25%糖果龟maxHp+治疗10%(喂); 炸弹亡→召新HP=20%糖果龟maxHp (登场召唤+死亡爆炸在spawn/summon)
	...
	if bomb != null:
		...
		_gambler_sys._gambler_pop(bomb["pos"], float(bomb.get("height", 0.0)) + 0.4, Color(1.0, 0.7, 0.88, 0.85))   # 喂养涨大糖光
		_skill_ring(bomb["pos"], Color(1.0, 0.62, 0.84, 0.5), 40.0)
...
func _candy_sweet_drain(u: Dictionary) -> void:   # 甜蜜掠夺·甜蜜吸取(用户2026-07-15"第8秒生效"): 对最大生命最高敌吸25%maxHp→全回复自己(不杀留1)+粉精华VFX
	...
	if steal > 0:
		_apply_damage_from(u, fat, maxi(1, int(round(steal))), Color("#ff9ecb"), 0.0, true, false, true, true)   # 真伤·必中·不暴击
		_heal(u, steal)
...
func _candy_drain_fx(candy: Dictionary, fat: Dictionary, amt: int) -> void:   # 甜蜜掠夺: 粉色精华从最肥敌流向糖果龟+吸取字
	...
	for k in range(4):
		var off := Vector2(randf_range(-18.0, 18.0), randf_range(-14.0, 14.0))
		var mote := _glow_bb(from2d + off, 0.9, 24.0, Color(1.0, 0.55, 0.82, 0.95))
		var tw := _reg_tween()
		tw.tween_interval(float(k) * 0.06)
		var mv := tw.tween_property(mote, "position", _world_pos(kp, 1.1), 0.5)
		mv.set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN)
		tw.parallel().tween_property(mote, "scale", Vector3(0.4, 0.4, 0.4), 0.5)
		tw.chain().tween_callback(mote.queue_free)
	_float_text(from2d + Vector2(0, -50), "甜蜜掠夺 -" + str(amt), Color("#ff6bb0"))
	_skill_ring(kp, Color(1.0, 0.55, 0.82, 0.55), 46.0)
...
func _candy_bomb_bubble(u: Dictionary) -> void:   # 糖果炸弹糖泡(粉糖色·上飘·区别中毒绿/酒琥珀)
	...
	var spr := _glow_bb(pos2d, 0.35, 26.0, Color(1.0, 0.65, 0.86, 0.85))
	var tw := _reg_tween(); tw.set_parallel(true)
	tw.tween_property(spr, "position", _world_pos(pos2d, 1.0), 0.55)
...
func _two_head_disrupt_vfx(u: Dictionary, tgt: Dictionary, broke_shield: bool) -> void:
	...
	_burst_vfx("res://assets/sprites/vfx/psychic-blast.png", head, 155.0, 1.5)
	_skill_ring(head, Color(0.72, 0.36, 1.0, 0.78), 66.0)
	var r2 := _reg_tween()
	r2.tween_interval(0.09)
	r2.tween_callback(_skill_ring.bind(head, Color(0.6, 0.3, 0.95, 0.5), 110.0))
	_shake(0.06)
	if broke_shield:
		_burst_vfx("res://assets/sprites/vfx/shield-shatter.png", tgt["pos"], 135.0, 1.0)
	_two_head_heal_reduce_mark(tgt)

# 治疗削减·裂心标记(用户2026-07-11): 头顶挂裂心(AI heart-crack), 跟随目标5秒后褪(复用 _follow_vfx 自动贴位+剔除)
func _two_head_heal_reduce_mark(tgt: Dictionary) -> void:
	...
	m.billboard = BaseMaterial3D.BILLBOARD_ENABLED
	m.shaded = false; m.transparent = true
	m.pixel_size = (34.0 * WS) / 64.0
	m.modulate = Color(1, 1, 1, 0.95)
	_world.add_child(m)
	tgt["_hr_mark"] = m
	_follow_vfx.append({"spr": m, "unit": tgt, "h": 2.0})         # 头顶跟随
	var mtw := _reg_tween()
...
func _two_head_absorb_vfx(u: Dictionary, tgt: Dictionary) -> void:
	_bolt_line(tgt["pos"], u["pos"], Color(0.92, 0.26, 0.52))
	...
	for k in range(4):
		...
		mote.pixel_size = (26.0 * WS) / float(maxi(1, gt.get_height()))
		mote.position = _world_pos(from2d, 1.0)
		_world.add_child(mote)
		var mtw := _reg_tween()
		mtw.tween_interval(float(k) * 0.05)
		mtw.tween_method(_absorb_mote_step.bind(mote, from2d, to2d), 0.0, 1.0, 0.3)
		mtw.tween_callback(mote.queue_free)
	_skill_ring(u["pos"], Color(0.32, 0.92, 0.5, 0.62), 66.0)
...
func _sk_lava_cast(u: Dictionary, tgt: Dictionary, set_id: String = "A") -> void:   # 熔岩龟·按选中技分派(A地裂/B岩浆涌动/C喷射)×形态变体
	...
	match set_id:
		"A":
			if volcano: _lava_sys._lava_volcano_erupt(u)
			else:       _lava_sys._lava_quake(u)
		"B":
			if volcano: _lava_sys._lava_flame_strike(u, tgt)
			else:       _lava_sys._lava_magma_surge(u, tgt)
		_:
			_lava_sys._lava_quake(u)

# 通用·击飞: 把 o 从 center 上抛+外推 (1:1 _lava_sys._lava_slam_impact 的击飞段, 抽公共)
...
func _sk_lava_erupt(u: Dictionary, tgt) -> void:                # 熔岩龟·技三(用户设计): 火山形态=暴走(+20%maxHp5s+30%攻速+30%移速) / 普通形态=智能冲刺(保命撤/追击贴)+下发普攻穿透
	if u.get("volcano", false):
		...
		tw.tween_callback(_lava_sys._lava_ult_revert.bind(u))
...
func _sk_cyber_cannon(u: Dictionary, tgt) -> void:              # 赛博龟·能量大炮(用户#15: 对一条直线蓄力后发长激光能量射线·线上敌各1A物理+0.1A×浮游炮数真伤·炮越多越猛)
	...
	var fire := func() -> void:                                 # 发射: 贯屏巨柱(三层: 白热核心+主青束+外晕)瞬现
		...
		_gambler_sys._gambler_pop(uu["pos"] + dir * 30.0, 0.9, Color(0.85, 1.0, 1.0, 0.95))                          # 发射爆闪
...
func _hiding_dome(u: Dictionary, col: Color, dur: float) -> void:   # 缩头·壳护罩: hex-bubble罩身呼吸dur秒→碎裂放大淡出(跟随单位·2026-07-17 Q4草案)
	...
	dm.pixel_size = (128.0 * WS) / float(maxi(1, dm.texture.get_height()))   # 110→128+alpha0.75→0.85(2026-07-17自查: 挤团时罩偏淡)
...
func _hiding_summon_vfx(pos: Vector2) -> void:                 # 喊龟·召唤法阵(2026-07-17 Q4草案): 细线环扩开+内环倒收+土金光柱+星粒上腾+落地尘
	...
	var inner := _px_ground_sprite(VfxTex._make_pixel_ring_tex(), pos, 260.0, Color(0.85, 0.7, 0.4, 0.7), 0.06)   # 内环倒收(聚拢感)
	var it := _reg_tween()
...
func _sk_hiding_defend(u: Dictionary) -> void:                   # 缩头乌龟·防御(封板·100龟能): 缩壳20%maxHp盾(4秒)+护甲+20%(5秒)·到期剩余盾20%转生命; 2026-07-17用户: 持盾锁龟能+盾段壳青绿特殊色
	...
	u["hiding_shield_until"] = _t + 4.0                          # 持盾锁龟能(石头rock_shield四连判据同款·盾破/到期恢复)
	...
	_pending_shots.append({"delay": 4.0, "fn": func(): uu["_hidingShellVal"] = 0.0, "src": u})   # 到期清特殊段标记
...
func _hiding_apply_buff(o: Dictionary, dur: float) -> void:     # 强化随从增益(dur秒·dur<=0=永久·供随从死亡继承给主人): 攻/甲/抗+10%·吸血+10%·暴击+20%
	...
	if dur > 0.0:
		...
		gl.pixel_size = (72.0 * WS) / 128.0
		gl.modulate = Color(1.0, 0.32, 0.25, 0.0)
		gl.position = _world_pos(o["pos"], 0.55)
		_world.add_child(gl)
		_follow_vfx.append({"spr": gl, "unit": o, "h": 0.55})
		var glt := _reg_tween()
...
func _sk_hiding_shrink(u: Dictionary) -> void:                  # 缩头(封板·100龟能): 立即给随从+50%技能龟能 + 自身缩头3秒(80%减伤·不能攻击/移动·★龟能条锁定=设stun_until→_tick_skill_cd遇stun直接return不充能·对用户#1"且锁龟能条")
	...
	_stun(u, 3.0, "_sk_hiding_shrink", true)   # 缩头3秒: 不能攻击/移动(定身)
...
func _sk_bubble_bind(u: Dictionary, tgt) -> void:
	if tgt == null:
		return
	_stun(tgt, 3.0, "_sk_bubble_bind")   # 泡泡束缚定身3秒(用户设计·原CTRL_SEC=1.5)
	...
	if btex != null:
		...
		cage.pixel_size = (118.0 * WS) / float(maxi(1, btex.get_height()))
		cage.position = _world_pos(tgt["pos"], 0.9)
		cage.modulate = Color(1, 1, 1, 0)
		_world.add_child(cage)
		_follow_vfx.append({"spr": cage, "unit": tgt, "h": 0.9})
		var tw := _reg_tween()
	...
	_skill_ring(tgt["pos"], Color(0.5, 0.9, 1.0, 0.5), 54.0)
	...
	match u["id"]:
		...
		"pirate":
			...
			if not es.is_empty():
				...
				_pirate_ship_muzzle(ps2d, psh)
				var uu2: Dictionary = u; var vv2: Dictionary = v
				_pirate_cannonball(ps2d, psh, v["pos"], func() -> void:
					...
					_shake(0.05))
		"candy":
			# 甜蜜掠夺·甜蜜吸取(用户2026-07-15改"第8秒生效"): 登场不触发→改由 _tick_unit 战斗第8秒调 _candy_sweet_drain 一次
		...
		"chest":
			if (not _review_demo()) and str(u.get("side", "")) == "left" and GameState != null:
				for _tw0 in GameState.chest_treasures_won:     # 大轮已开战利品→开局回装(常驻整轮·用户2026-07-16)
					_chest_sys._chest_apply_treasure(u, str(_tw0))
			if "chestCannon" in _chosen_skill_types(u["id"], u["side"] == "left"):   # 贪婪(技三打包被动)选中才有
				...
				_chest_sys._chest_greed_apply(u, (u.get("equips", []) as Array).size())   # 登场先按已带装备数结算
		"hiding":
		...
		"crystal":
			if "crystalBall" in _chosen_skill_types(u["id"], u["side"] == "left"):   # 水晶球(技三·选中才召): 登场召唤实体水晶球(2026-07-16加大+特效)
				...
				if _orb != null:                              # 登场爆闪+常驻脉动光环(用户2026-07-16: 球太小没特效)
					_skill_ring(_orb["pos"], Color(0.72, 0.92, 1.0, 0.8), 46.0)
					_flash(_orb, Color(1.5, 1.8, 2.0))
					_crystal_sys._crystal_orb_halo(_orb)
		"two_head":
...
func _on_unit_death(u: Dictionary, killer) -> void:
	...
	if u.get("death_aoe", 0.0) > 0.0:
		...
		if u.get("summon_kind", "") == "candybomb":   # 糖果炸弹死亡: 大糖爆+震屏+糖泡四溅(用户2026-07-14参照海盗船完整呈现)
			...
			_shake(JUICE_SHAKE_HEAVY)
			for _cb in range(10): _candy_bomb_bubble(u)
...
func _pirate_shotgun(u: Dictionary, tgt) -> void:
	...
	for i in range(8):
		...
		_shotgun_pellet(u["pos"], u["pos"] + d * 400.0, Color(1.0, 0.86, 0.5, 0.95), 0.72)   # 弹丸VFX(射程400·慢速用户2026-07-14)
...
func _pirate_ship_splash(pos2d: Vector2) -> void:   # 撞击大水花爆(青白泡沫团炸开+飞溅水珠·明显·用户2026-07-14"没看到水花")
	...
	for k in range(3):   # 3团青白泡沫(不透明·扩张淡出=能盖住亮水面)
		...
		f.pixel_size = ((150.0 + float(k) * 50.0) * WS) / dth
		f.position = _world_pos(pos2d + Vector2(randf_range(-20.0, 20.0), randf_range(-10.0, 10.0)), 0.45 + float(k) * 0.25)
		_world.add_child(f)
		var tw := _reg_tween(); tw.set_parallel(true)
	...
	for i in range(12):   # 飞溅水珠(向外+抛物上飞)
		...
		var drop := _glow_bb(pos2d, 0.4, 16.0, Color(0.9, 0.97, 1.0, 0.95))
		...
		dtw.tween_method(func(p: float) -> void:
			...
			drop.position = _world_pos(pos2d.lerp(dest2d, p), 0.4 + peak * sin(PI * p))
...
func _cyber_assemble_mech(u: Dictionary) -> void:   # 阵亡演出(用户2026-07-16理想效果): 本体消散→浮游炮游动集结→各自对最近敌蓄力→贯穿激光0.4A魔法→飞向集结点→机甲5秒组装(血/攻/甲/抗从低涨到设定值·期间不能动)→就位近战
	...
	if n <= 0:
		return
	_mech_incoming[str(u.get("side", "left"))] = _t + 3.0   # 死→机甲spawn(2.8s)空档桥接: 此侧仍算存活防提前团灭/破蛋(用户2026-07-18)
	var arr: Array = u.get("_drones", [])
	u["_drones"] = []
	u["drone_n"] = 0
	...
	_pending_shots.append({"delay": 2.8, "fn": func() -> void:
		_gambler_sys._gambler_pop(gather, 0.8, Color(0.7, 0.98, 1.0, 0.95))
		...
		var grow := _reg_tween()                                 # 5秒属性增长(血条0%→100%填充·攻/甲/抗0→满·组装期免伤无扣血)
...
func _hit_spark(tgt, at_pos = null) -> void:
	...
	if _hitring_tex == null:
		_hitring_tex = VfxTex._make_ring_texture(Color(1, 1, 1, 1))
	var r := Sprite3D.new()
...
func _check_end() -> void:
	...
	for u in _units:
		if u.get("is_trainer", false):
			continue   # 训龟大师不计胜负(同 _dl_side_alive; 漏这条会导致"打不死它→永远不结束")
...
func _edit_clear() -> void:
	...
	_units.clear()
	_projectiles.clear()
	for z in _lava_sys._lava_zones:                       # 清岩浆池(避免编辑重开残留)
...
#   注: 编辑态摆放时已逐个 _make_unit, 但还没跑过 _inject_equipment/_apply_spawn_passives/_equip_sys._eq_apply_all_stats,
...
func _edit_start_battle() -> void:
	...
	_apply_spawn_passives()
	_equip_sys._eq_apply_all_stats()
	if _edit_full_energy:   # 装备/被动结算后再补满龟能→技能即就绪(不被_eq_apply_all_stats重置回充率覆盖)
		for u in _units:
...
func _edit_unit_from_setup(s: Dictionary) -> Dictionary:
	...
	if bool(s.get("minion", false)) or str(s.get("id", "")) == "__minion__":
		...
		u = _make_unit(str(s.get("id", "basic")), side, s.get("pos", Vector2()))
...
func _edit_back_to_edit() -> void:
	...
	for s in _edit_paused_setup:
		_units.append(_edit_unit_from_setup(s))
	_edit_mode = true
	_over = false
...
func _edit_toggle_full_energy() -> void:   # 满龟能开关(用户2026-07-18: 不用死磕攒龟能就能看放技): 对已摆的友军立即生效+新摆的也带
	_edit_full_energy = not _edit_full_energy
	for u in _units:
		if u.get("side", "") == "left":
			_edit_apply_full_energy(u)
	_edit_refresh_labels()
	_edit_set_status("满龟能 %s · 友军技能开局即就绪+快速回充" % ("开(可反复看放技)" if _edit_full_energy else "关(正常攒龟能)"))
...
func _edit_refresh_labels() -> void:
	if _edit_btn_pick != null:
		if _edit_pick_id == "__minion__":
			_edit_btn_pick.text = "当前笔刷: %s" % ("近战小将·浪板" if _edit_minion_role == "front" else "远程小将·火箭")
		else:
	...
	if _edit_btn_energy != null:
		_edit_btn_energy.text = "满龟能:开" if _edit_full_energy else "满龟能:关"
		_edit_btn_energy.modulate = Color(1, 0.9, 0.4, 1) if _edit_full_energy else Color(0.55, 0.6, 0.66, 1)
...
func _edit_load_setup() -> void:
	...
	for s in arr:
		...
		_units.append(_edit_unit_from_setup({
...
func _vfx_smolder(origin: Vector2, dir: Vector2, si: int = 1) -> void:
	...
	_smolder_sys._smolder_mother(origin, dir)
	var t := _reg_tween()
	t.tween_interval(0.5)
	t.tween_callback(_smolder_sys._smolder_erupt.bind(origin, dir, reach, si))
...
func _vfx_preview_loop() -> void:
	...
	while is_instance_valid(self):
		...
		match eff:
			...
			"laser_chop": _equip_sys._eq_laser_chop(fu, {"pos": origin + dir * 300.0, "alive": true}, si, 180.0)
			"moon": _equip_sys._eq_wide_blade(fu, {"pos": origin + dir * 650.0, "alive": true}, si)
...
func _update_team_panels() -> void:
	for u in _units:
		...
		if sb != null and sb is StyleBoxFlat:
			var selected: bool = (_selected_unit != null and is_same(u, _selected_unit))
...
func _refresh_info_panel() -> void:
	...
	if _info_status_box != null and is_instance_valid(_info_status_box):
		var sig := _status_signature(ud)
		if sig != _info_status_sig:
			_info_status_sig = sig
...
func _info_status_chips(vb: VBoxContainer, u: Dictionary) -> void:
	...
	if _t < float(u.get("heal_reduce_until", 0.0)): chips.append(["💔 治疗削减", "#ff6b6b"])
	if _t < float(u.get("energy_lock_until", 0.0)): chips.append(["🔒 龟能锁", "#ffcf5a"])
...
func _show_unit_info_panel(u: Dictionary) -> void:
	...
	_info_status_chips(_info_status_box, u)
