...
func _lava_volcano_erupt(u: Dictionary) -> void:                 # 火山·火山爆发: 朝敌最密方向 超长矩形横扫 (5段×0.5ATK魔+灼烧+回血12%, 击退命中敌)
	...
	await tt.finished
	# 2) 岩浆流从远端沿 -dir 扫回向熔岩龟 (~0.4s) — AI生成熔岩流贴图 (单帧 Sprite3D, tween 移动)
	var flow_tex: Texture2D = _sheet("res://assets/sprites/vfx/fx_lava_flow.png")
	var flow := Sprite3D.new()
	flow.texture = flow_tex
	flow.billboard = BaseMaterial3D.BILLBOARD_ENABLED
	flow.shaded = false; flow.transparent = true
	flow.modulate = Color(1.0, 1.0, 1.0, 0.0)
	# 约 300px 覆盖矩形 (按贴图实际高度归一)
	var flow_h: float = float(flow_tex.get_height()) if flow_tex != null else 768.0
	flow.pixel_size = (300.0 * WS) / flow_h
	var far: Vector2 = origin + dir * length
	flow.position = _world_pos(far, 0.6)
	_world.add_child(flow)
	var ft := create_tween()
	ft.tween_property(flow, "modulate:a", 1.0, 0.1)               # 渐入
	ft.parallel().tween_property(flow, "position", _world_pos(origin, 0.6), 0.4).set_trans(Tween.TRANS_SINE)   # 沿 -dir 扫回
	ft.parallel().tween_property(tel, "modulate:a", 0.0, 0.4)
	ft.chain().tween_property(flow, "modulate:a", 0.0, 0.12)      # 渐出
