...
func _lava_volcano_erupt(u: Dictionary) -> void:                 # 火山·火山爆发(用户2026-07-15完整重设计·照娜美R官方): 起手聚力→红矩形预警→圆角熔岩浪丘【从自身推出】沿最密方向0.9s推进1200码(原从场外涌向自己=怪已反转)→碾过每敌击飞+5段0.5A魔+灼烧+回血12%→到端塌落
	...
	var wall_w: float = (half_w * 2.0 + 90.0) * WS                # 浪宽(世界·+90px视觉溢出)
	var wall_h: float = wall_w * 0.22                             # 浪高≈1个龟身(太高会成竖条·参考=宽矮浪包)
	var wall_d: float = wall_h * 1.5                              # 浪厚(沿行进)≈1.5×高(有纵深的鼓包·非薄片)
	# ★实心浪包(用户2026-07-15"薄壳侧对镜头=纸片"): 剖面闭环包过顶(前脚→前脸→浪尖→圆顶→背面→背脚)·两端捏合(hf→0)→任何方向看都有体量
	var prof: Array = [Vector2(0.2, 0.0), Vector2(0.17, 0.5), Vector2(0.24, 0.94), Vector2(0.0, 1.0), Vector2(-0.2, 0.9), Vector2(-0.26, 0.45), Vector2(-0.2, 0.0)]   # (x前后, y高)×浪高·加厚滚圆(前后厚~0.5×浪高)
	var M := 16                                                   # 横向分段
	...
	for j in range(M):
		...
		for k in range(prof.size() - 1):
			...
			var va0 := Vector3(pa.x * wall_d * hf0, pa.y * wall_h * hf0, z0)
			var va1 := Vector3(pa.x * wall_d * hf1, pa.y * wall_h * hf1, z1)
			var vb0 := Vector3(pb.x * wall_d * hf0, pb.y * wall_h * hf0, z0)
			var vb1 := Vector3(pb.x * wall_d * hf1, pb.y * wall_h * hf1, z1)
	...
	_shake(JUICE_SHAKE_BIG)
	var travel := 0.9                                             # 1200码/0.9s≈1333码/s(可读的巨浪推进)
	var wt := _reg_tween()
	...
	if is_instance_valid(tel):
		tel.queue_free()
	if is_instance_valid(wall):
		var wf := _reg_tween()                                    # 浪壳到端: 塌落淡出(浪撞岸散掉·mesh底在地面直接压扁)
		wf.tween_property(wall, "scale:y", 0.12, 0.22)
