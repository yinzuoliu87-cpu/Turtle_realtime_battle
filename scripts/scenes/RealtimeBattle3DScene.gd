...
func _sk_gambler_bet(u: Dictionary, tgt: Dictionary) -> void:    # 赌神龟·赌注(用户封板·100龟能): 需当前生命>40%; 消耗当前生命40%→7段物理砸目标(共≈0.4×当前生命); 施放3秒多重概率+20%
	...
	for i in range(7):                                         # 7张牌barrage(错峰0.08s·命中才跳伤害·扣'七段')
		_pending_shots.append({"delay": float(i) * 0.08, "fn": func() -> void: _gambler_throw_hit(u, tgt, per), "src": u})

# 赌神: 甩1张牌→命中结算dmg+金光pop (赌注barrage用·命中才跳伤害)
func _gambler_throw_hit(u: Dictionary, tgt: Dictionary, dmg: int) -> void:
	if tgt == null or not tgt.get("alive", false): return
	...
	if t == null:
		_apply_damage_from(u, tgt, dmg, Color("#ffd93d"), 0.0, true); return
