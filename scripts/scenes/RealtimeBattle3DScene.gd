...
func _sk_star_gravity_warp(u: Dictionary) -> void:             # 星际龟·扭曲空间「奇点」(封板数值不动: 普通=敌阵中心500码全体0.8A魔法; 强化星能满=+拽向中心min(d×0.6,200)+清空星能; 120龟能)
	...
	var do_burst := func() -> void:
		...
		for o3 in victims:
			...
			var dest3 := Vector2(clampf(center.x * 2.0 - from3.x, ARENA.position.x, ARENA.end.x), clampf(center.y * 2.0 - from3.y, ARENA.position.y, ARENA.end.y))   # 换位=过奇点镜像落对侧(用户2026-07-16"延中心点到对端": 左右对调, 非聚拢; 镜像保距→落点互不重叠)
			if o3.has("_home_pos"): o3["_home_pos"] = dest3      # 评审假人: 落点即新驻点(原归位逻辑会把人走回出生位→用户看到"拖到对端又回去"; 真实对局无_home_pos零影响)
			_knockback(uu, o3, 0.0, 1.05, 0.0)                   # 起跳vy=6.3(滞空0.57s)+震屏顿帧; 落地juice物理tick自带(压扁+尘+轻震)
			var mvt := _reg_tween()                              # 真抛物线弧: 水平匀速直线(linear无缓动)+竖直重力=被甩飞过中心点(用户2026-07-16"很真实的被重力拖拽·飞过中心点·落到对端后回复行动"→无定身, 落地即自由; airborne期分离/AI本就不生效)
