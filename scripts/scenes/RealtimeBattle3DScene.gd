...
func _sk_basic_chiwave(u: Dictionary, tgt) -> void:            # 小龟·龟派气波(封板·100龟能): 先自增buff(暴击25%/暴伤20%/吸血10%/护穿0.1A·3秒)→朝当前目标发穿透气波(带宽80·打沿途所有敌+蛋)每命中3.5A物理+击飞1.5s+击退200 [智能位移留F5]
	...
	var sp := Sprite3D.new()                                    # 气波·生成帧(掌心聚气·用户2026-07-05"生成动画")
	sp.texture = load("res://assets/sprites/vfx/qibo-spawn.png")
	sp.texture_filter = BaseMaterial3D.TEXTURE_FILTER_NEAREST
	sp.billboard = BaseMaterial3D.BILLBOARD_ENABLED; sp.shaded = false; sp.transparent = true
	sp.pixel_size = (72.0 * WS) / 64.0
	sp.position = _world_pos(start2, 1.0)
	_world.add_child(sp)
	var stw := _reg_tween()
	stw.tween_property(sp, "scale", Vector3.ONE * 1.3, 0.22).from(Vector3.ONE * 0.4).set_trans(Tween.TRANS_BACK)
	stw.parallel().tween_property(sp, "modulate:a", 0.0, 0.28)
	stw.tween_callback(sp.queue_free)
	var ball := Sprite3D.new()                                  # 气波·飞行循环球(拳皇罗伯特式旋转能量弹)
	ball.texture = load("res://assets/sprites/vfx/qibo-ball.png")
	...
	ball.pixel_size = (92.0 * WS) / 64.0
	ball.position = _world_pos(start2, 1.0)
	_world.add_child(ball)
	var hit2: Dictionary = {}
	var step2 := func(d: float) -> void:                        # 球缓慢飞·扫到敌才打(沿途穿透各1次)
		var c: Vector2 = start2 + dir * d
		if is_instance_valid(ball): ball.position = _world_pos(c, 1.0)
		for o in _enemies_of(uu2):
			if not o.get("alive", false) or hit2.has(o): continue
			if not _on_line(uu2["pos"], dir, o["pos"], 80.0): continue
			if o["pos"].distance_to(c) > 95.0: continue
			hit2[o] = true
			_apply_damage_from(uu2, o, _atk_dmg(uu2, 3.5, o), Color("#7fd0ff"))
			_knockback(uu2, o, 200.0, 1.8, 2.0)                   # 击飞1.5s+击退200
	var btw := _reg_tween()
	btw.tween_method(step2, 0.0, 900.0, 3.0).set_trans(Tween.TRANS_LINEAR)   # 缓慢飞到尽头(3.0s跨900码=300码/秒·用户2026-07-10"太快了"从562改300·嫌快慢报数再调)
	btw.tween_callback(ball.queue_free)                         # 到尽头即销毁·无消失动画(用户2026-07-05"不应有消失动画")
...
func _sk_ice_team_shield(u: Dictionary) -> void:               # 寒冰龟·团队护盾(封板L167·80龟能·通用护盾): 全体友军各0.5A护盾·4秒(通用护盾4秒·封板L74)
	for o in _allies_of(u):                                    # _allies_of默认含自己=全体友方
		...
		_skill_ring(o["pos"], Color(0.6, 0.85, 1.0, 0.55), 44.0)   # 六棱冰晶护盾泡VFX占位(美术L174)
...
func _sk_hunter_barrage(u: Dictionary, _tgt) -> void:          # 猎人龟·狩猎弹幕(封板·100龟能): 10绿箭·智能优先锁残血为方向·每箭可被挡直线弹道(命中路径第一敌/蛋·障碍穿·我方不挡·复用小龟_basic_first_blocker)·0.3A真实(共3A)·每箭<14%即处决
	for i in range(10):
		...
		_bolt_line(u["pos"], u["pos"] + dir * 520.0, Color(0.66, 1.0, 0.7))   # 绿箭直线占位(连珠速射扇面散射美术F5)
...
func _sk_cyber_cannon(u: Dictionary, tgt) -> void:              # 赛博龟·能量大炮(用户#15: 对一条直线蓄力后发长激光能量射线·线上敌各1A物理+0.1A×浮游炮数真伤·炮越多越猛)
	...
	var fire := func() -> void:                                 # 蓄力后·直线长激光能量射线(用户#15·非贴身跃击)
		...
		_bolt_line(uu["pos"], uu["pos"] + dir * 900.0, Color(0.6, 0.94, 1.0))                          # 长激光能量射线占位
...
func _on_unit_death(u: Dictionary, killer) -> void:
	...
	for f in _units:
		if f.get("alive", false) and f.get("id") == "fortune" and f != u:
			f["gold"] += 9
	# 海盗掠夺(被动·用户2026-07-09加回原版·死亡钩索): 任意敌人(非召唤)阵亡 → 存活海盗龟钩索最近敌·拉近90码+造伤 ★占位待确认PoC值+美术
		...
		for _pf in _units:
			if _pf.get("alive", false) and _pf.get("id") == "pirate" and _pf.get("side", "") != u.get("side", ""):
				...
				_pending_shots.append({"delay": 0.15, "src": _pf, "fn": func() -> void:
					...
					_bolt_line(_pk["pos"], _pt["pos"], Color(1.0, 0.85, 0.4))})
