...
const TYPES := {
	...
	"香火": {"tiers": [1], "stats": [{}]},
	## 斧头(2026-08-31 用户「为木斧专门设一个羁绊」): 只有 096 小木斧一件,
	## ⇒ **持有并装备 1 件即激活**, 照香火的形状(单档·无属性)。
	## ★效果不在属性上 —— 激活后改的是**商店出货概率**(3% + 本大轮激活后局数×0.1%, 封顶 10%),
	##   那条在四期实现; 本期先把类型立起来, 免得 096 挂在"遗物"下面把
	##   「顶档阈值 == 该类型件数」那条不变式(D5)撞红。
	"斧头": {"tiers": [1], "stats": [{}]},
...
const TIER_DESCS := {
	...
	"灵物": [
		"每件灵物额外提供 +5% 闪避率 · 1 个无敌触手登场 · 【拍击蓄能】触手每 {C:SpiritSynergySystem.SLAP_PERIOD} 秒获得 1 层拍击层数(无上限); 空闲且射程内有敌人时消耗 1 层, 朝最近的一名敌人拍下, 对沿途敌人造成 ({C:SpiritSynergySystem.HIT_HP_PCT%}% 目标最大生命 + {C:SpiritSynergySystem.HIT_FLAT}) 物理伤害 · 【亡灵】友方单位阵亡时原地召唤 1 只亡魂, 继承其 20% 攻击力与生命, 自动攻击最近敌人; 亡魂阵亡后不再循环",
		"每件灵物额外提供 +9% 闪避率 · 2 个触手 · 【闪避蓄能】己方任一单位闪避成功 → 每根触手各获得 1 层拍击层数(不设上限) · 【亡灵】继承 38% 攻击力与生命 · 亡魂阵亡后再循环 1 次(每次属性 ×0.9)",
		"每件灵物额外提供 +13% 闪避率 · 2 个触手, 拍击伤害 +50% · 闪避蓄能同上 · 【亡灵】继承 65% 攻击力与生命 · 亡魂阵亡后再循环 2 次",
		"每件灵物额外提供 +18% 闪避率 · 2 个触手, 拍击伤害 +100% · 闪避蓄能同上 · 【亡灵】继承 100% 攻击力与生命 · 亡魂阵亡后再循环 3 次",
	],
	...
	"斧头": [
		"装备小木斧即激活 · 商店里更容易再遇到它(概率随本大轮的局数增长, 有上限) · 重复购买会化作砍伐经验",
	],
	"遗物": [
