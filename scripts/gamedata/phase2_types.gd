...
#   · stats 的逐档数值取自方案书 §5 的十张表(每件给携带者)。效果文本见下方 TIER_DESCS。
const TYPES := {
	"剑":   {"tiers": [3, 6, 9],     "stats": [{"atk": 15}, {"atk": 38}, {"atk": 62}]},
	"奇械": {"tiers": [2, 5, 8, 10], "stats": [{"mr": 18}, {"mr": 40}, {"mr": 68}, {"mr": 105}]},
	"食物": {"tiers": [3, 6, 9],     "stats": [{}, {}, {}]},  # 团队级 maxHP(数【全队每件装备】而非本类型件数) — 特殊, apply 另写
	"盾":   {"tiers": [3, 6, 9],     "stats": [{"def": 8}, {"def": 20}, {"def": 38}]},
	"药水": {"tiers": [3, 6, 9],     "stats": [{"_maxEnergy": 18}, {"_maxEnergy": 36}, {"_maxEnergy": 60}]},
	"枪":   {"tiers": [3, 6, 9],     "stats": [{"armorPen": 8}, {"armorPen": 18}, {"armorPen": 30}]},
	"弓箭": {"tiers": [3, 6, 9],     "stats": [{"crit": 0.11}, {"crit": 0.20}, {"crit": 0.30}]},
	"法器": {"tiers": [2, 5, 8, 10], "stats": [{"magicPen": 8, "_maxEnergy": 10}, {"magicPen": 14, "_maxEnergy": 10}, {"magicPen": 22, "_maxEnergy": 10}, {"magicPen": 32, "_maxEnergy": 10}]},
	"灵物": {"tiers": [2, 5, 8, 10], "stats": [{}, {}, {}, {}]},  # 闪避走下方 _extraDodge 专属分支, 不放这里 —— 见 ★
	# ★灵物的 +5/9/13/18% 闪避【故意】不写进 stats: apply_team_start 下方已有 _extraDodge 专属分支,
	#   两边都写 = 同一份闪避加两次。这正是 p2eq_015 荆棘海胆踩过的坑(通用钩 + 专属分支各发一次,
	#   探针实测实发是名义值的 2.00×, equip_stats_apply.gd:50-61 有整段血泪注释)。
	#   批 4 真实装时二选一, 别两边都留。
	"遗物": {"tiers": [2, 5, 8, 10], "stats": [{"_lifestealPct": 5}, {"_lifestealPct": 10}, {"_lifestealPct": 20}, {"_lifestealPct": 32}]},
}
...
const TIER_DESCS := {
	"剑": [
		"全队每件剑额外提供 +15 攻击力 · 剑触发伤害效果后, 再以 50% 伤害回响 1 次(回响不触发任何装备效果) · 【血祭】全队每损失 1% 最大生命 → +0.6 攻击力",
		"全队每件剑额外提供 +38 攻击力 · 回响 2 次 · 【血祭】每损失 1% 最大生命 → +1.0 攻击力",
		"全队每件剑额外提供 +62 攻击力 · 回响 3 次, 且回响伤害 50% → 80% · 【血祭】每损失 1% 最大生命 → +1.5 攻击力 · 【背水】携带剑者生命首次跌破 30% 时, 获得等于其 100% 攻击力的护盾(每单位每场一次)",
	],
	"奇械": [
		"每件奇械额外提供 +18 魔法抗性 · 携带奇械者每 2.5 秒铸造 1 枚深海币",
		"每件奇械额外提供 +40 魔法抗性 · 每 2.5 秒铸 2 枚 · 【冰封】全队攻击 15% 概率冻结目标 1.5 秒(同一目标每 2.5 秒最多被冻 1 次, 解冻后 1.5 秒免疫冻结)",
		"每件奇械额外提供 +68 魔法抗性 · 每 2.5 秒铸 3 枚 · 【冰封】25% 概率 · 【僵硬】全队每段攻击为目标叠 1 层僵硬(持续 5 秒, 叠加刷新时长; 每层 -2% 攻击力, 最多 20 层)",
		"每件奇械额外提供 +105 魔法抗性 · 每 2.5 秒铸 3 枚 · 【冰封】40% 概率 · 【僵硬】同上 · 【易碎】被冻结或眩晕的敌人, 受到的伤害 +25%",
	],
	"食物": [
	...
	"盾": [
		"每件盾额外提供 +8 护甲 · 携带盾者每次受伤获得(身上盾件数)怒气, 满 10 怒气消耗全部, 对一名敌人释放冲击波, 造成 4% 自身最大生命值的真实伤害, 并为自己获得等量护盾值",
		"每件盾额外提供 +20 护甲 · 冲击波 6% · 【圣光】每 5 秒为携带者生成 50 × (1 + 0.4 × 携带者身上盾件数) 护盾",
		"每件盾额外提供 +38 护甲 · 冲击波 8% · 【圣光】90 × (1 + 0.4 × 件数) · 【反击】圣光护盾存在时, 受到的每段伤害对来源造成 3 × (1 + 0.5 × 件数) 点真实伤害 · 【收殓】敌方单位阵亡时, 为最近的携带盾者提供该单位 30% 最大生命值的护盾",
	],
	"药水": [
		"每件药水额外提供 +18 最大龟能 · 【猎物】每 2.5 秒自动将敌方当前生命值最高的 1 名单位标记为猎物(猎物死亡后下个周期重选); 全队对猎物造成的伤害 +15%",
		"每件药水额外提供 +36 最大龟能 · 【猎物】+25% · 【猎获】每击杀 1 名猎物 → 全队攻击力永久 +22(本场累积)",
		"每件药水额外提供 +60 最大龟能 · 【猎物】+40% · 【猎获】每击杀 1 名猎物 → 全队攻击力永久 +38 · 【斩首】攻击猎物时, 若其生命低于 20% → 直接处决",
	],
	"枪": [
		"每件枪额外提供 +8 护甲穿透 · 每把枪射满 4 发额外射出 1 发金弹(继承该枪全部子弹效果, 另加 60% 真实伤害; 不计入弹药计数)",
		"每件枪额外提供 +18 护甲穿透 · 每把枪射满 3 发 → 金弹(+80% 真伤) · 【火控】携带枪者额外造成 (4 + 携带者身上枪件数 × 2)% 真实伤害",
		"每件枪额外提供 +30 护甲穿透 · 每把枪射满 2 发 → 金弹(+100% 真伤) · 【火控】(6 + 携带者身上枪件数 × 3)% · 【炮台】我方阵营后方生成 1 座固定炮台: 每 2.5 秒朝敌方生命值最高者轰击, 对炮台与该敌直线上所有敌人造成 (80 + 4% 目标最大生命) 物理伤害, 并为我方最低血友军回复 30% × 造成伤害的生命",
	],
	"弓箭": [
		"每件弓箭额外提供 +11% 暴击率 · 携带弓箭者攻击时, 目标生命百分比低于斩杀线则直接处决(斩杀线 = 4% + 暴击率 × 0.1)",
		"每件弓箭额外提供 +20% 暴击率 · 斩杀线 = 7% + 暴击率 × 0.1 · 【腐蚀·穿透】全队攻击无视目标 20% 护甲与魔法抗性",
		"每件弓箭额外提供 +30% 暴击率 · 斩杀线 = 10% + 暴击率 × 0.1 · 【腐蚀·穿透】无视 38% 护甲与魔法抗性 · 【腐蚀叠层】每 2.5 秒, 所有敌方单位 +1 层腐蚀(每层使其受到的伤害 +5%, 最多 5 层); 腐蚀满 5 层的敌人, 其受到伤害的 25% 转化为真实伤害",
	],
	"法器": [
		"每件法器额外提供 +8 法术穿透 与 +10 最大龟能 · 每件法器拥有独立法力条, 满 100 触发其效果(每 2.5 秒 +25 法力, 另按造成的技能伤害 ×0.1 与受到的伤害 ×0.1 累积)",
		"每件法器额外提供 +14 法术穿透 与 +10 最大龟能 · 法力条满 80 · 【潮涌】每 2.5 秒, 全队回复已损失生命的 5%",
		"每件法器额外提供 +22 法术穿透 与 +10 最大龟能 · 法力条满 60 · 【潮涌】8% · 【退潮】友军受到治疗时, 额外获得等于治疗量 30% 的护盾",
		"每件法器额外提供 +32 法术穿透 与 +10 最大龟能 · 法力条满 50 · 【潮涌】12% · 【退潮】50% · 【大潮】每 7.5 秒掀起一次大潮: 全队每个友军回复 15% 最大生命值, 并净化 2 个减益",
	],
	"灵物": [
		"每件灵物额外提供 +5% 闪避率(总闪避上限 75%) · 1 个无敌触手登场, 每 2.5 秒朝一名敌人拍击, 对沿途敌人造成 (4% 目标最大生命 + 55) 物理伤害 · 【亡灵】友方单位阵亡时原地召唤 1 只亡魂, 继承其 20% 攻击力与生命, 自动攻击最近敌人; 亡魂阵亡后不再循环",
	...
	"遗物": [
		"每件遗物额外提供 +5% 生命偷取 · 携带遗物者生命 >50% 时额外 +3% 攻击力; 生命 <50% 时遗物提供的生命偷取翻倍(→10%)",
		"每件遗物额外提供 +10% 生命偷取 · >50% 时 +5% 攻击力(<50% 吸血 →20%) · 【远古之力】每 2.5 秒全队永久 +4 攻击力(本场累积, 软上限 +150) · 龟蛋获得 +500 最大生命值",
		"每件遗物额外提供 +20% 生命偷取 · >50% 时 +8% 攻击力(<50% →40%) · 【远古之力】每 2.5 秒 +7 攻击力(软上限 +250) · 龟蛋 +900 最大生命值",
		"每件遗物额外提供 +32% 生命偷取 · >50% 时 +12% 攻击力(<50% →64%) · 【远古之力】每 2.5 秒 +11 攻击力(软上限 +350) · 龟蛋 +1500 最大生命值 · 【觉醒】本路战斗开打满 20 秒后, 已累积的远古之力 +50%, 之后每 2.5 秒的累积翻倍",
	],
}
