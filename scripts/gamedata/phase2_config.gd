...
##
## 2026-09-25 用户两句话定的:
##   ①「我的档是 5 场次的, 我要和 6 场次的人打? 这不合理啊」
##   ②「不应该有这些东西啊粗格子: 0 / 1-2 / 3-4 / 5-7 / 8-11 / 12-16 / 17-21 /
##      22-27 / 28+。你 5 场次在『5-7』」
##
## ⇒ `Backend.bracket_for_battles` 那九格从此**只是池子的索引**, 不是匹配的尺子。
##   索引可以粗(它要让每个桶里有足够多的候选), **尺子必须细且对称**:
##   高档一格宽到六场(档7 = 22~27), 拿它当尺子的话 22 场的人会碰到 27 场的 ——
##   差着五轮升级 + 五次商店。
##
## 这**一个**常量同时被两处读, 不许各写一份(各写一份必然漂):
##   · `supabase.opponents_query()` —— 从服务器拉哪几格(网络那一侧)
##   · `Backend.find_opponent()`    —— 从池子里选哪一个(选靶那一侧)
## ══════════════════════════════════════════════════════════════════════
const MATCH_BATTLES_SPAN := 1
