...
const STAGES := [
	## ★木斧档的显示名就是**装备自己的名字**「小木斧」, 不是「木斧」。
	##   由来(2026-09-01): 门禁 verify_axe_shop_codex 抓到 —— 图鉴渲染 `eq["name"]`(小木斧)
	##   而商店 `_deco` 把它改成「木斧」⇒ **同一件装备两个名字**, 玩家在图鉴记住一个、
	##   到商店找不到它。需求里「木斧」指的是**进化档位**, 不是这件装备的名字。
	{"key": "wood",    "name": "小木斧", "need": 0,   "cost": 1},
...
## ★★用户 2026-09-15:「这个梯形本来是4秒内每0.5秒加100码是吧，现在改为每0.25加30码，
##   并且梯形的宽度减少20%，包括特效和实际伤害范围」
##   ⇒ 0.5 秒 / 100 码 → 0.25 秒 / 30 码(满蓄 800 → 480 码); 近边 300 → 240、远边 900 → 720。
##   演出(`AxePassiveVfx.half_w_at` / `field_corners` / 六边形铺法)与判定(`in_trapezoid`)读的是
##   同一组常量 —— 改这里两边一起变, 不存在"特效改了伤害没改"。
const CHARGE_TIME := 4.0         # 共蓄力 4 秒
const CHARGE_STEP := 0.25        # 每 0.25 秒
const CHARGE_H_PER_STEP := 30.0  # 梯形的【高】增加 30 码 ⇒ 满蓄 480 码
...
## 梯形的两条底边宽度。★需求原话没给(只说「梯形预警范围」+「高每 0.5 秒 +100 码」),
##   由我 2026-08-31 自拍: 近边 = 召唤物碰撞直径的两倍, 远边 = 近边的 3 倍。
##
## ★★2026-09-03 我一度按 LOL 塞恩 Q 的比例把它收窄到 130/300, **用户当场否掉**:
##     「梯形还是按我们的来因为参考的是长方形, 我们是梯形」
##   —— 说得对: 参考的预警区是**长方形**(从近到远等宽), 而需求这里明写「梯形」,
##   它本来就该张开。**形状不同的东西, 尺寸不能照抄。**
##   ⇒ 参考里该学的是**颜色/辉光/零填充/细线**(那些与形状无关), 不是宽度。
const TRAPEZOID_NEAR_W := 240.0   # 300 × 0.8(用户 2026-09-15「梯形的宽度减少20%」)
const TRAPEZOID_FAR_W := 720.0    # 900 × 0.8
