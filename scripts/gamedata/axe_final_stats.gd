...
## ★★下面两个是【演出时序常量, 不是平衡数值】(2026-09-15)。
##   用户:「7/9的回旋镖同样是在敷衍我啊，回旋镖是什么？以及特效和实际伤害范围完全不一样啊，也没有命中特效」
##   ⇒ 回旋镖改成「飞出去 → 折返 → 飞回斧头」, 伤害在镖真正经过敌人时结算。
##   伤害 / 灼烧 / 半宽 / 把数 / 施放时长一个没动; 这两个只决定镖在路上走多快、至少飞多远。
const SERAPH_BOOM_SPEED := 1600.0   # 码/秒: 750 码一去一回 ≈ 0.94 秒
const SERAPH_BOOM_MIN_OUT := 750.0  # 至少飞出多远(= 原演出默认射程 SERAPH_BOOM_R × 2.5); 身前判定带里有更远的敌人就飞到它
