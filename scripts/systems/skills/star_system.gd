...
## ★★2026-08-30 从【一帧内】改成【累计】—— 用户实测「近战小将有位移技却逃不出去」。
##   旧判据是"一帧内被挪走 >130 码"。它只抓得住**瞬移**;
##   而绝大多数位移技是**平滑**的(人体浪板拉己滑行 / 猎人翻滚 / 熔岩跃击),
##   实测每帧只被挪 **1.7 码** ⇒ 一帧内永远够不到 130, **一次都逃不掉**。
##   (旧门禁全绿是因为它用"直接改 pos"制造位移 = 瞬移, 恰好是唯一能过的那种。)
## ★为什么累计是安全的(实测剖面, 不是拍的): 捕获期 1971 帧里
##   **1433 帧漂移恰好 = 0.000 码**(虫洞每帧写死坐标, 没人动它就是 0),
##   538 帧 = 1.7 码(浪板在拉), **两者之间一帧都没有**。
##   ⇒ 没有噪声会把这个累加器攒满; 只有真被外力挪时它才涨。
## ★60 码 ≈ 35 个位移帧 ≈ 0.6 秒被拖走; 一次瞬移(130+)当帧就超, 两种都覆盖。
const WORM_ESCAPE_DIST := 60.0   # 【累计】被外力挪走这么多 ⇒ 脱离捕获(码)
	...
	var fire := func() -> void:
		...
		var step := func(d: float) -> void:
			...
			for cg2 in caught:
				...
				## 上一帧我们把它放在 `set`; 这一帧它离那儿多远, 就是**纯外力**挪的距离
				## (虫洞本帧还没动它)。逐帧累加 —— 平滑位移也攒得满, 瞬移当帧就超。
				cg2["acc"] = float(cg2.get("acc", 0.0)) 					+ (oc["pos"] as Vector2).distance_to(cg2["set"] as Vector2)
				if float(cg2["acc"]) > WORM_ESCAPE_DIST:
					## ★产品自己的账: 放走过几次。门禁量这个, 不量"它最后在哪"——
					##   虫洞飞到边界会自己结束, 那之后单位当然能自由移动,
					##   拿"最终位置"当判据会被【洞没了】满足而不是被【放人了】满足
					##   (2026-08-30 反向验证当场抓到: 退回旧判据门禁照样绿)。
					oc["_worm_freed_n"] = int(oc.get("_worm_freed_n", 0)) + 1
					freed.append(cg2)
