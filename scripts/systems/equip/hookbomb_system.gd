...
##   如果敌人带有炸弹的时候死亡，则会朝所有敌方单位发射钩索，眩晕1.1秒后把他们拉向自己，
...
## 所以本文件的规矩是:
##   · `_hb_pull_dest_at(...)` 纯几何 —— 验落点, 不等 tween 跑到
...
##   卡在 `if not _over` 里, 而"抽搐 CONVULSE_SEC(2.0) 秒后引爆"很可能正好跨过战斗结束那一刻
...
##   ⇒ 300/600/750 + 15%  →  **120/250/400 + 10%**。
##   原来这两行的注释是「★2026-08-29 ×1.5 补偿(见 BOMB_DPS_PCT 头注)」——
##   **那句话现在不再描述这两个数了**(新值不是从任何旧值 ×1.5 来的), 所以一并改掉;
##   但上面 `BOMB_DPS_PCT` 的头注**要保留** —— 它解释的是「为什么这些数要除以 0.6685」,
##   而那个理由仍然成立(这两段仍是**真物理**、仍吃护甲, 见 `:261` 的 `_phys_after_armor`)。
##   ⇒ 这里写的是**面板值**, 落地还要 ×0.6685 左右。
##     例: ★2 打 5000 血目标, 面板 250+500=750, 实际到手约 501。
const BLAST_FLAT := [120.0, 250.0, 400.0]     # 聚爆固定段(面板值·落地吃护甲)
const BLAST_MAXHP_PCT := 0.10                 # 聚爆额外 10% 最大生命(面板值·落地吃护甲)
...
## ⚠ `TEX_CHAIN` **零引用**(全仓只有这一行): 链节画法已被下方的触手网格 `_tendril_*` 取代。
##   素材 `assets/sprites/vfx/hookbomb-chain.png` 还留在盘上 —— 和上面"不留孤儿素材"那句相冲,
##   要么接回来用, 要么连素材一起删(素材不在本目录, 交主会话处理)。
...
func _bomb_vfx(o: Dictionary) -> void:
	...
	#   参照: 地图道具用 1.5~3.0 米, 龟身约 32px×WS≈0.77 米。炸弹取 0.62 米 ≈ 龟头大小。
...
##   横截面 7 条(暗边→肉→高光脊→肉→暗边, 见 `OFF`/`COL`)造出圆柱感, 两端锚在【身体中心高度】,
##   中段在 Y 上拱起 ⇒ 它是一根穿过空间的立体物件, 不是地面贴纸。
const TENT_W_ROOT := 9.5       # 根部【半】宽(码)。★正对镜头后不再被压扁, 19 码全宽 ≈ 龟身(SEP_RADIUS 92)的 21%
...
func _tendril_draw(from2d: Vector2, to2d: Vector2, t01: float, phase: float, tip_h: float = TENT_ANCHOR_H, prio: int = 6) -> void:
	...
	# ── ② 沿脊线铺【正对镜头】的管状带: 横截面 7 条, 暗边→肉→高光脊→肉→暗边
