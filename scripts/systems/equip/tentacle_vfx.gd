...
const INERTIA_MAXLAG := 62.0     # 滞后角上限(度)
const INERTIA_OMEGA_REF := 45.0  # 饱和参考角速度(度/秒)
const INERTIA_STIFF := 250.0      # 弹簧刚度 —— 越大回弹越快
const INERTIA_DAMP := 9.0        # 阻尼 —— 太小像橡皮, 太大就没有甩尾了
const INERTIA_P := 3.0
...
const INERTIA_GAIN_SLAM := 0.28

## ★★★2026-08-22【重力垂头】用户原话:「抬起到最高处时…头部会向后弯**由于重力**」
## 惯性(上面那套弹簧)只能给"追不上", 给不了"到顶之后自己倒过去" —— 后者是**重力**。
## 物理: 单摆力矩 ∝ sin(偏离竖直的角度)。梢端一旦**越过竖直**(仰角 > 90°),
##   重力就把它**继续往后按**(角度继续增大); 还没到竖直则把它往前按。
...
func _phase_at(t: Dictionary, ts: float) -> Array:
	...
	match st:
		...
		ST_WARN:
			...
			## ★★★2026-08-22 用户:「你现在是压根没有往后甩, 整个头部一直在前面吊着」
			## 抬起时头部吊在前面 = 惯性追不上, 那一半是对的; **错的是没有"甩回来"**。
			## 根因: v0.19.262 我为了治定格把抬起改成【匀速升到底】—— 等于**把顶点的减速删了**,
			##   角速度永远不归零, 惯性弹簧就永远等不到"停", 自然永远不甩回去。
			##   (第一次探针其实就打出「到顶后的甩尾过冲 +0.00°」, 我看见了没当回事。)
			## ⇒ 三段: 缓入 → 匀速 → **末尾一小段减速到停**。
			##   减速段很短(SETTLE), 所以不会退回"定格"那个病(门禁 LIFT_FLOW_MIN 焊着)。
			var wp: float = clampf(ts / T_WARN, 0.0, 1.0)
			const WARM := 0.22
			const SETTLE := 0.16          # 末尾这一段减速到停 —— 头部靠惯性甩过去
			var q: float
			var mid: float = 1.0 - SETTLE
			## 三段的斜率在接缝处相等(否则接缝上会有速度跳变, 那正是我在治的毛病)
			var k_lin: float = 2.0 / (2.0 - WARM - SETTLE)
			if wp < WARM:
				q = k_lin * wp * wp / (2.0 * WARM)
			elif wp < mid:
				q = k_lin * (wp - WARM * 0.5)
			else:
				var e: float = (wp - mid) / SETTLE
				q = k_lin * (mid - WARM * 0.5) + k_lin * SETTLE * (e - e * e * 0.5)
