...
##
## ── 本批（4-1）只做 🟢：现成钩子、零新机制 ───────────────────────────
## `Phase2Types.TYPES` 全部类型的**每件属性**（给携带者）+ 周期效果。
## ★周期效果现在本文件只剩【食物盛宴】一条: 法器潮涌→灵泉已搬去 staff_synergy_system.gd,
##   盾圣光 2026-08-03 已搬去 shield_synergy_system.gd（见文末那段记录）。
...
## ★这里原有 `const LIGHT_BASE := [0.0, 50.0, 90.0]`, 注释写"实发 = 基数 × (1 + 0.4 × 盾件数)"。
##   两条都删了: ① 全仓零引用 ② 公式与实装不符 —— 圣光 2026-08-03 已搬去
##   `shield_synergy_system.gd`(HOLY_AMOUNT=55 × 携带的 095 件数 × holy_bonus, 9 档 ×1.20),
##   那个 0.4/件 的系数全仓不存在。事实源看 shield_synergy_system.gd:58-61。
...
## 每帧节拍（挂主循环）。★本文件的周期效果只剩【食物盛宴】一条（_pulse）；
##   `_t_light` / `LIGHT` 是圣光搬走后留下的空转计时器，不驱动任何效果。
