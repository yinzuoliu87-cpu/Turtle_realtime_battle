...
## 类型原生是 `healAmp` / `shieldAmp` / `_maxEnergy`（每件 +6/12/20 治疗强度、+6/12/20 护盾强度、
## +2/4/6 最大龟能，逐档，见 `Phase2Types.TYPES["药水"]`）。★旧注释写的"只有 _maxEnergy 5/11/20"
## 是改表前的老账 —— 5/11/20 其实是法器的 magicPen 那一行。
##
## ── 三条的作用域 ─────────────────────────────────────────────────────
