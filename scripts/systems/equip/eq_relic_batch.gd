...
## ══════════════════════════════════════════════════════════════════════
## ·【A】(★2026-09-15 订正: 已接线) 批④ 六个系统的 `clear_all()` 现在由 `dual_lane_flow.gd` 的换路撤场循环
##   `for _b4ref in battle._equip_sys._b4_all(): _b4ref.clear_all()` 统一调用。
##   本文件的 `_sweep()` 自扫兜底(每 0.5 秒查立碑者还在不在 `battle._units` 里)仍保留, 但已不是唯一保证。
## ·【B】(★2026-09-15 订正: 已解耦) 批④ 全局 `tick` 现在由主循环每帧直接调 `EquipSystem.tick_global`,
##   不再挂在「单位身上有装备」之内 —— 场上没有带装备的活单位时碑照常推进。
