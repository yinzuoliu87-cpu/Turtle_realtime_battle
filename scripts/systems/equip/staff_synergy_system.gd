...
## ★事实源是上面的 `MANA_FULL_BY_TIER = [200, 180, 150, 120, 80]`(档 0~4 一一对应):
##   档 0(没激活羁绊)= 200, 首档(2 件)= 180 —— 档 0 与首档【不同值】。
##   羁绊的收益就是逐档把满值往下压 200→180→150→120→80。
##   (旧注释写的 "档0=100 / 首档=100 / 降到 80/60/50" 是 2026-08 改表之前的老账, 已订正。)
...
func add_mana(u: Dictionary, n: float) -> void:
	...
	##   文案承诺的主动永远兑现不了。⇒ 法力条是【每件法器自带】的, 未激活羁绊时满值 200;
	##   羁绊的作用是逐档把满值压下来 200→180→150→120→80(见 MANA_FULL_BY_TIER)。
