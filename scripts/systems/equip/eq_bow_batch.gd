...
## ══════════════════════════════════════════════════════════════════════
##   073 → `_eq_on_basic_attack`(藤蔓箭)      + 本文件 `tick()` 每帧推进(攻速 buff 到期 + 小球跟随)
##          ★不占 EQ_IV_BATCH1 的周期口 —— 排进去会把精度降到 0.25 秒, 见下方 §073 那段。
...
##    ⚠ 076 一次连射最多 26 发(全表最贵技能 210 龟能 ÷ ENERGY_PER_SHOT 8) × 每发贯穿数人,
##    回钩 on-hit 会直接自激炸掉。(旧注写 42 发, 那是"每 5 龟能一发"时代的数, 2026-08-12 已改 8。)
...
const RAIN_HOPS := 6
## 箭雨总持续(秒)。★写成【推导】而不是再抄一个 1.5 —— 改跳数或跳间隔它自动跟着走,
##   文案引用的也是它, 所以"文案说 1.5 秒"与"代码真的跳 1.5 秒"不可能分家。
const RAIN_SEC := RAIN_HOP_IV * RAIN_HOPS
...
func on_cast_076(u: Dictionary, si: int) -> void:
	...
	## "射了几发读得出是几下"不能只靠反冲节奏, 数字倒数是最硬的读数。逐发递减在 _tick_volleys。
