...
##    ⇒ 选靶全走「最近」, 本文件自己不掷随机; 伤害经 `_resolve_dmg` 按携带者暴击率掷暴击(走 battle._battle_rng, 确定性不受影响)。
...
## ══════════════════════════════════════════════════════════════════════
## · **088/089/090 打出的伤害【吃暴击】**(★用户 2026-09-15 拍板「88 89 90应该要吃暴击啊」)。
##   原来这里写「一律不掷暴击」, 而代码早就经 `_magic_after_mr` → `battle._resolve_dmg`(会掷暴击) ——
##   注释与代码对不上, 第十批调查(V1)查出来的; 以代码为准, 注释订正。
...
## · **090 的起跳→砸落 = `pestle_jump_sec()` = 2√(2h/g) ≈ 1.673 秒**(规格没给)。
##   ★因果方向是【先定峰高 h=PESTLE_APEX_M(7米) 与重力 g=PESTLE_G(20), 再推滞空 T】,
##   不是先定时间倒推重力(2026-08-08 用户「哪有这么快的跳」之后改的, 旧版才是 h=2.4/T=0.6)。
##   起跳直接写击飞物理字段(`vy = √(2gh)`), 由主循环那段 `airborne` 积分抛起再落回,
##   所以**落地与砸落同时**。
...
## ★2026-09-06 用户「500妈伤害改为1倍攻击力加150/300/4000」⇒ 系数 3.0 → 1.0。
...
## (旧头注「只做魔抗减免、不掷暴击」已作废: 2026-08-20 起直接转调 `_resolve_dmg`, 会掷暴击、也吃水晶共鸣; 用户 2026-09-15 拍板就该吃暴击。)
...
# ══════════════════════════════════════════════════════════════════
#  §090 镇海杵 (法器 · 5 费)
#  主动「法力条集满 → 跳到空中后向整个战场猛砸: PESTLE_RADIUS(500) 码内敌人受到
#        (规格原文写 1000, 2026-08-09 用户拍板改 500 —— 直径 2000 比战场长边还长, 圈落在画面外)
#        1 ATK + 150/300/4000 魔法伤害, 2/3/8 秒眩晕 + 1 秒击飞。
#        ★造成伤害后才重新开始记录法力值。」
#  被动「每第三次普攻发射一片浪潮, 从携带者飞向目标后不断跳向最近的、
#        未被本次浪潮飞过的友军或敌军, 最多跳 4/5/8 次;
#        打到敌人 40/70/140 魔法伤害, 打到友军 20/40/80 治疗。」
#  ⚠ 眩晕 3★ 仍是全表最大的一次星级跳跃(3→8 秒)。
#     用户原话「5000，8秒，就要8」——【8 秒这半句仍然作数, 别去动眩晕】。
#  ★★2026-09-06 用户改主意, 把伤害那一段削了:「500妈伤害改为1倍攻击力加150/300/4000」
#     ⇒ 3 ATK + 200/300/5000  →  1 ATK + 150/300/4000。
#     原来这里写着「不许在实施时调低」——【那是当时的拍板, 现在已被用户本人推翻】,
#     所以连同数值一起改掉。留着不改 = 给下一个人留一条与事实相反的铁律。
#     ★用户 2026-09-06 逐字:「我给你报的数值你为什么要质疑我, 实战里就是不平衡啊」
#       ⇒ 数值由实战定, 不由"星级曲线是否均匀"这种纸面对称性定。别再拿降幅去劝。
# ══════════════════════════════════════════════════════════════════
...
func _pestle_leap(u: Dictionary, si: int) -> void:
	...
	var flat: float = [150.0, 300.0, 4000.0][si]    # 猛砸的固定魔法伤害段(另加 1 ATK) ★2026-09-06: 原 200/300/5000
	...
	## 峰高 h 与重力 g 是【给定的】, 滞空 T = 2√(2h/g) 由它们推出(pestle_jump_sec)。
	## 换算过来就是 vy = √(2gh) = 4h/T、g = 8h/T² ⇒ 落地时刻恰好 = T = 砸落时刻。
